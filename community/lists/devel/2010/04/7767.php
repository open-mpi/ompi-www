...
?>
<!-- received="Fri Apr 16 21:59:32 2010" -->
<!-- isoreceived="20100417015932" -->
<!-- sent="Fri, 16 Apr 2010 19:59:21 -0600" -->
<!-- isosent="20100417015921" -->
<!-- name="Ralph Castain" -->
<!-- email="rhc_at_[hidden]" -->
<!-- subject="Re: [OMPI devel] problem when binding to socket on a single	socket	node" -->
<!-- id="47C6DEAC-2988-4A6B-9932-26E5DB3013E8_at_open-mpi.org" -->
<!-- charset="us-ascii" -->
<!-- inreplyto="1271158498.2561.418.camel_at_frecb014522.frec.bull.fr" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2010-04-16 21:59:21
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="7768.php">Ralph Castain: "Re: [OMPI devel] problem when binding to socket on a single	socket	node"</a>
<li><strong>Previous message:</strong> <a href="7766.php">&#213;&#197;&#190;&#167;: "[OMPI devel] &#180;&#240;&#184;&#180;: [OMPI devel] &#180;&#240;&#184;&#180;:  Migrate the OpenMPI to VxWorks"</a>
<li><strong>In reply to:</strong> <a href="7745.php">Nadia Derbey: "Re: [OMPI devel] problem when binding to socket on a single	socket	node"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="7768.php">Ralph Castain: "Re: [OMPI devel] problem when binding to socket on a single	socket	node"</a>
<li><strong>Reply:</strong> <a href="7768.php">Ralph Castain: "Re: [OMPI devel] problem when binding to socket on a single	socket	node"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<p>I have committed a solution to this issue in r22984 and r22985. I have tested it against a range of scenarios, but hardly an exhaustive test. So please do stress it.
<br>
...
<br>
<p>I couldn't use the provided patch for several reasons:
<br>
<p>* you cannot print a message out of an odls module after the fork occurs unless you also report an error - i.e., you cannot print a message out as a warning and then continue processing. If you do so, everything will appear correct when you are operating in an environment where no processes are local to mpirun - e.g., when running under slurm as it is normally configured. However, when processes are local to mpirun, then using orte_show_help after the fork causes the print statement to occur in separate process instances. This prevents mpirun from realizing that multiple copies of the message are being printed, and thus it cannot aggregate them.
<br>
<p>As a result, the provided patch generated one warning for every local process, plus one aggregated warning for all the remote processes. This isn't what we wanted users to see.
<br>
<p>The correct solution was to write an integer indicating the warning to be issued back to the parent process, and then let that process output the actual warning. This allows mpirun to aggregate the result.
<br>
<p>Nadia wasn't the only one to make this mistake. I found that I had also made it in an earlier revision when reporting the &quot;could not bind&quot; message. So it is easy to make, but one we need to avoid.
<br>
<p>* it didn't address the full range of binding scenarios - it only addressed bind-to-socket. While I know that solved Nadia's immediate problem, it helps if we try to address the broader issue when making such changes. Otherwise, we wind up with a piecemeal approach to the problem. So I added support for all the binding methods in the odls_default module.
<br>
<p>* it missed the use-case where processes are launched outside of mpirun with paffinity_alone or slot-list set - e.g., when direct-launching processes in slurm. In this case, MPI_Init actually attempts to set the process affinity - the odls is never called.
<br>
<p>Here is why it is important to remember that use-case. While implementing the warning message there, I discovered that the code in ompi_mpi_init.c would actually deal with Nadia's scenario incorrectly. It would identify the process as unbound because it had been &quot;bound&quot; to all available processors. Since paffinity_alone is set, it would then have automatically bound the process to a single core based on that process' node rank.
<br>
<p>So even though the odls had &quot;bound&quot; us to the socket, mpi_init would turn around and bind us to a core - which is not at all what Nadia wanted to have happen.
<br>
<p>The solution here was to pass a parameter to the spawned process indicating that mpirun had &quot;bound&quot; it, even if the &quot;binding&quot; was a no-op. This value is then checked in mpi_init - if set, mpi_init makes no attempt to re-bind the process. If not set, then mpi_init is free to do whatever it deems appropriate.
<br>
<p>So looking at all the use-cases can expose some unintended interactions. Unfortunately, I suspect that many people are unaware of this second  method of setting affinity, and so wouldn't realize that their intended actions were not getting the desired result.
<br>
<p>Again, no criticism intended here. Hopefully, the above explanation will help future changes!
<br>
Ralph
<br>
<p><p>On Apr 13, 2010, at 5:34 AM, Nadia Derbey wrote:
<br>
<p><span class="quotelev1">&gt; On Tue, 2010-04-13 at 01:27 -0600, Ralph Castain wrote:
</span><br>
<span class="quotelev2">&gt;&gt; On Apr 13, 2010, at 1:02 AM, Nadia Derbey wrote:
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; On Mon, 2010-04-12 at 10:07 -0600, Ralph Castain wrote:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; By definition, if you bind to all available cpus in the OS, you are
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; bound to nothing (i.e., &quot;unbound&quot;) as your process runs on any
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; available cpu.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; PLPA doesn't care, and I personally don't care. I was just explaining
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; why it generates an error in the odls.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; A user app would detect its binding by (a) getting the affiinity mask
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; from the OS, and then (b) seeing if the bits are set to '1' for all
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; available processors. If it is, then you are not bound - there is no
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; mechanism available for checking &quot;are the bits set only for the
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; processors I asked to be bound to&quot;. The OS doesn't track what you
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; asked for, it only tracks where you are bound - and a mask with all
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; '1's is defined as &quot;unbound&quot;.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; So the reason for my question was simple: a user asked us to &quot;bind&quot;
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; their process. If their process checks to see if it is bound, it will
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; return &quot;no&quot;. The user would therefore be led to believe that OMPI had
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; failed to execute their request, when in fact we did execute it - but
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; the result was (as Nadia says) a &quot;no-op&quot;.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; After talking with Jeff, I think he has the right answer. It is a
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; method we have used elsewhere, so it isn't unexpected behavior.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; Basically, he proposed that we use an mca param to control this
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; * default: generate an error message as the &quot;bind&quot; results in a no-op,
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; and this is our current behavior
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; * warn: generate a warning that the binding wound up being a &quot;no-op&quot;,
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; but continue working
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; * quiet: just ignore it and keep going
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; Excellent, I completely agree (though I would have put the 2nd star as
</span><br>
<span class="quotelev3">&gt;&gt;&gt; the default behavior, but never mind, I don't want to restart the
</span><br>
<span class="quotelev3">&gt;&gt;&gt; discussion ;-) )
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; I actually went back/forth on that as well - I personally think it might be better to just have warn and quiet, with warn being the default. The warning could be generated with orte_show_help so the messages would be consolidated across nodes. Given that the enhanced paffinity behavior is fairly new, and that no-one has previously raised this issue, I don't think the prior behavior is relevant.
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; Would that make sense? If so, we could extend that to the other binding options for consistency.
</span><br>
...
<span class="quotelev1">&gt; Patch proposal attached.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; Also this is a good opportunity to fix the other issue I talked about in
</span><br>
<span class="quotelev3">&gt;&gt;&gt; the first message in this thread: the tag
</span><br>
<span class="quotelev3">&gt;&gt;&gt; &quot;odls-default:could-not-bind-to-socket&quot; does not exist in
</span><br>
<span class="quotelev3">&gt;&gt;&gt; orte/mca/odls/default/help-odls-default.txt
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; I'll take that one - my fault for missing it. I'll cross-check the other messages as well. Thanks for catching it!
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; As for your other change: let me think on it. I -think- I understand your logic, but honestly haven't had time to really walk through it properly. Got an ORCM deadline to meet, but hope to break free towards the end of this week.
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt; Fairly trivial to implement, and Bull could set the default mca param
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; file to &quot;quiet&quot; to get what they want. I'm not sure if that's what the
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; community wants or not - like I said, it makes no diff to me so long
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; as the code logic is understandable.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt; On Apr 12, 2010, at 8:27 AM, Terry Dontje wrote:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; Ralph, I guess I am curious why is it that if there is only one
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; socket we cannot bind to it?  Does plpa actually error on this or is
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; this a condition we decided was an error at odls?
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; I am somewhat torn on whether this makes sense.  On the one hand it
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; is definitely useless as to the result if you allow it.  However if
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; you don't allow it and you have a script or running tests on
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; multiple systems it would be nice to have this run because you are
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; not really running into a resource starvation issue.
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; At a minimum I think the error condition/message needs to be spelled
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; out (defined).    As to whether we allow binding when only one
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; socket exist I could go either way slightly leaning towards allowing
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; such a specification to work.
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev1">&gt; &lt;003_bind_to_socket_on_single_socket.patch&gt;_______________________________________________
</span><br>
<span class="quotelev1">&gt; devel mailing list
</span><br>
<span class="quotelev1">&gt; devel_at_[hidden]
