...
?>
<!-- received="Fri Jun 10 08:51:23 2011" -->
<!-- isoreceived="20110610125123" -->
<!-- sent="Fri, 10 Jun 2011 06:51:13 -0600" -->
<!-- isosent="20110610125113" -->
<!-- name="Ralph Castain" -->
<!-- email="rhc_at_[hidden]" -->
<!-- subject="Re: [OMPI devel] RFC: Resilient ORTE" -->
<!-- id="15D66102-0A7A-4B4D-9D13-42651508C3BF_at_open-mpi.org" -->
<!-- charset="us-ascii" -->
<!-- inreplyto="BANLkTimHfRLoZ8_p45=F=9xAcO7u6CrMYg_at_mail.gmail.com" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2011-06-10 08:51:13
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="9353.php">Ralph Castain: "Re: [OMPI devel] RFC: Fix missing code in MPI_Abort functionality"</a>
<li><strong>Previous message:</strong> <a href="9351.php">Josh Hursey: "Re: [OMPI devel] RFC: Fix missing code in MPI_Abort functionality"</a>
<li><strong>In reply to:</strong> <a href="9350.php">Josh Hursey: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="9355.php">Josh Hursey: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<li><strong>Reply:</strong> <a href="9355.php">Josh Hursey: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<p><span class="quotelev1">&gt; Another problem with this patch, that I mentioned to Wesley and George
</span><br>
<span class="quotelev1">&gt; off list, is that it does not handle the case when mpirun/HNP is also
</span><br>
<span class="quotelev1">&gt; hosting processes that might fail. In my testing of the patch it
</span><br>
<span class="quotelev1">&gt; worked fine if mpirun/HNP was -not- hosting any processes, but once it
</span><br>
<span class="quotelev1">&gt; had to host processes then unexpected behavior occurred when a process
</span><br>
<span class="quotelev1">&gt; failed. So for those just listening to this thread, Wesley is working
</span><br>
<span class="quotelev1">&gt; on a revised patch to address this problem that he will post when it
</span><br>
<span class="quotelev1">&gt; is ready.
</span><br>
<p>See my other response to the patch - I think we need to understand why we are storing state in multiple places as it can create unexpected behavior when things are out-of-sync.
<br>
<p><p><span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; As far as the RML issue, doesn't the ORTE state machine branch handle
</span><br>
<span class="quotelev1">&gt; that case? If it does, then let's push the solution to that problem
</span><br>
<span class="quotelev1">&gt; until that branch comes around instead of solving it twice.
</span><br>
<p>No, it doesn't - in fact, it's what breaks the current method. Because we no longer allow event recursion, the RML message never gets out of the app. Hence my question.
<br>
<p>I honestly don't think we need to have orte be aware of the distinction between &quot;aborted by cmd&quot; and &quot;aborted by signal&quot; as the only diff is in the error message. There ought to be some other way of resolving this?
<br>
<p><p><span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev1">&gt; On Fri, Jun 10, 2011 at 8:22 AM, Ralph Castain &lt;rhc_at_[hidden]&gt; wrote:
</span><br>
<span class="quotelev2">&gt;&gt; Something else you might want to address in here: the current code sends an RML message from the proc calling abort to its local daemon telling the daemon that we are exiting due to the app calling &quot;abort&quot;. We needed to do this because we wanted to flag the proc termination as one induced by the app itself as opposed to something like a segfault or termination by signal.
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; However, the problem is that the app may be calling abort from within an event handler. Hence, the RML send (which is currently blocking) will never complete once we no longer allow event lib recursion (coming soon). If we use a non-blocking send, then we can't know for sure that the message has been sent before we terminate.
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; What we need is a non-messaging way of communicating that this was an ordered abort as opposed to a segfault or other failure. Prior to the current method, we had the app drop a file that the daemon looked for as an &quot;abort  marker&quot;, but that was ugly as it sometimes caused us to not properly cleanup the session directory tree.
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; I'm open to suggestion - perhaps it isn't actually all that critical for us to distinguish &quot;aborted by call to abort&quot; from &quot;aborted by signal&quot;, and we can just have the app commit suicide via self-imposed SIGKILL? It is only the message output  to the user at the end of the job that differs - and since MPI_Abort already provides a message indicating &quot;we called abort&quot;, is it really necessary that we have orte aware of that distinction?
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; 
