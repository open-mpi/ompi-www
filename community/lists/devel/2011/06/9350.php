...
?>
<!-- received="Fri Jun 10 08:38:57 2011" -->
<!-- isoreceived="20110610123857" -->
<!-- sent="Fri, 10 Jun 2011 08:38:52 -0400" -->
<!-- isosent="20110610123852" -->
<!-- name="Josh Hursey" -->
<!-- email="jjhursey_at_[hidden]" -->
<!-- subject="Re: [OMPI devel] RFC: Resilient ORTE" -->
<!-- id="BANLkTimHfRLoZ8_p45=F=9xAcO7u6CrMYg_at_mail.gmail.com" -->
<!-- charset="ISO-8859-1" -->
<!-- inreplyto="A6FA0979-E66F-41D7-8F66-7319DD60515B_at_open-mpi.org" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2011-06-10 08:38:52
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="9351.php">Josh Hursey: "Re: [OMPI devel] RFC: Fix missing code in MPI_Abort functionality"</a>
<li><strong>Previous message:</strong> <a href="9349.php">Ralph Castain: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<li><strong>In reply to:</strong> <a href="9343.php">Ralph Castain: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="9352.php">Ralph Castain: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<li><strong>Reply:</strong> <a href="9352.php">Ralph Castain: "Re: [OMPI devel] RFC: Resilient ORTE"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
is ready.
<br>
<p><p>As far as the RML issue, doesn't the ORTE state machine branch handle
<br>
...
<p>-- Josh
<br>
<p><p>On Fri, Jun 10, 2011 at 8:22 AM, Ralph Castain &lt;rhc_at_[hidden]&gt; wrote:
<br>
<span class="quotelev1">&gt; Something else you might want to address in here: the current code sends an RML message from the proc calling abort to its local daemon telling the daemon that we are exiting due to the app calling &quot;abort&quot;. We needed to do this because we wanted to flag the proc termination as one induced by the app itself as opposed to something like a segfault or termination by signal.
</span><br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt; However, the problem is that the app may be calling abort from within an event handler. Hence, the RML send (which is currently blocking) will never complete once we no longer allow event lib recursion (coming soon). If we use a non-blocking send, then we can't know for sure that the message has been sent before we terminate.
</span><br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt; What we need is a non-messaging way of communicating that this was an ordered abort as opposed to a segfault or other failure. Prior to the current method, we had the app drop a file that the daemon looked for as an &quot;abort &#160;marker&quot;, but that was ugly as it sometimes caused us to not properly cleanup the session directory tree.
</span><br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt; I'm open to suggestion - perhaps it isn't actually all that critical for us to distinguish &quot;aborted by call to abort&quot; from &quot;aborted by signal&quot;, and we can just have the app commit suicide via self-imposed SIGKILL? It is only the message output &#160;to the user at the end of the job that differs - and since MPI_Abort already provides a message indicating &quot;we called abort&quot;, is it really necessary that we have orte aware of that distinction?
</span><br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt;
