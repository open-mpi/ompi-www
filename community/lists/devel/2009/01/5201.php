...
?>
<!-- received="Thu Jan 15 01:50:39 2009" -->
<!-- isoreceived="20090115065039" -->
<!-- sent="Wed, 14 Jan 2009 22:54:47 -0800" -->
<!-- isosent="20090115065447" -->
<!-- name="Eugene Loh" -->
<!-- email="Eugene.Loh_at_[hidden]" -->
<!-- subject="[OMPI devel] RFC: Fragmented sm Allocations" -->
<!-- id="496EDDB7.1060807_at_sun.com" -->
<!-- charset="us-ascii" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2009-01-15 01:54:47
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="5202.php">Jeff Squyres: "[OMPI devel] This is why we test"</a>
<li><strong>Previous message:</strong> <a href="5200.php">Paul H. Hargrove: "Re: [OMPI devel] openmpi-1.3rc4 build failure with qsnet4.30"</a>
<!-- nextthread="start" -->
<!-- reply="end" -->
</ul>
<hr>
...
<tt>MPI_Init()</tt>, since the 3<i>n</i><sup>2</sup> CB allocations
during <tt>MPI_Init()</tt> all contend for a single
<tt>mca_common_sm_mmap->map_seg->seg_lock</tt> lock.  (To be fair,
...
<p>
In sum, for process <tt>myrank</tt>, the proposed FIFO allocation in
shared memory during <tt>MPI_Init()</tt> looks something like this:
<pre>
...
<li> <b>File</b>: <tt>ompi/class/ompi_fifo.h</tt><br>
     ...
     <p>
     <b>Function</b>: <tt>ompi_fifo_write_to_head()</tt><br>
     <p>
     ...
     in this case, the additional argument will be <tt>NULL</tt>.
<li> <b>File</b>: <tt>ompi/mca/btl/sm/btl_sm.c</tt><br>
     ...
     sender and receiver.
     <p>
     <b>Function</b>: <tt>sm_btl_first_time_init()</tt><br>
     <p>
     Increase the allocation of FIFOs (call to <tt>mpool_calloc</tt>)
     to include room for the CBs.
     <p>
     <b>Function</b>: <tt>mca_btl_sm_add_procs()</tt><br>
     <p>
...
Meanwhile, at large <i>n</i>, the file is insufficiently small.
...
The <tt>sm</tt> BTL should generate the desired file size, to
be used by the <tt>sm</tt> <tt>mpool</tt> in creating the shared
memory.  Sizing information can be passed from the BTL to the
<tt>mpool</tt> component by adding a <tt>size</tt> field to the
<tt>mca_mpool_base_resources_t</tt> data structure that is passed.
...
<li> <tt>mpool_sm_per_peer_size</tt>: parameter should be eliminated
</ul>

<p>
The changes are:
<ul>
<li> <b>File</b>: <tt>ompi/mca/mpool/sm/mpool_sm.h</tt><br>
     <b>Declaration</b>: <tt>mca_mpool_base_resources_t</tt><br>
     <p>
     Add a <tt>size_t size</tt> field to the data structure.
<li> <b>File</b>: <tt>ompi/mca/btl/sm/btl_sm.c</tt><br>
     <b>Function</b>: <tt>sm_btl_first_time_init()</tt><br>
     <p>
     Determine a reasonable size for the file to <tt>mmap</tt>
     into each process for point-to-point shared-memory messages.
     Set the new <tt>res.size</tt> field to this value before
     calling <tt>mca_mpool_base_module_create()</tt>.
<li> <b>File</b>: <tt>ompi/mca/mpool/sm/mpool_sm_component.c</tt><br>
     <b>Function</b>: <tt>mca_mpool_sm_init()</tt><br>
     <p>
     Instead of setting <tt>mca_mpool_sm_component.sm_size</tt>
     "arbitrarily", use the value <tt>resources->size</tt> passed
     in by the BTL.
     <p>
     <b>Function</b>: <tt>mca_mpool_sm_init()</tt><br>
     <b>Function</b>: <tt>mca_mpool_sm_open()</tt><br>
     <b>Declaration</b>: <tt>peer_size_param</tt> and <tt>default_peer</tt><br>
     <p>
     Reflect the changes in the MCA "min", "max", and "per-peer" parameters.
<li> <b>File</b>: <tt>ompi/mca/btl/sm/btl_sm_component.c</tt><br>
     <b>Function</b>: <tt>mca_btl_sm_component_open()</tt><br>
     <p>
     Change the default value of <tt>sm_extra_procs</tt> from
