...
?>
<!-- received="Fri Mar 18 19:06:02 2016" -->
<!-- isoreceived="20160318230602" -->
<!-- sent="Fri, 18 Mar 2016 23:05:50 +0000" -->
<!-- isosent="20160318230550" -->
<!-- name="Lane, William" -->
<!-- email="William.Lane_at_[hidden]" -->
<!-- subject="Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files	limits	&amp;gt;=	4096 still required?" -->
<!-- id="434F2BB0AF80484CA04DE0D8C0738BF2A29F2456_at_cshsmsgmbx01.CSMC.EDU" -->
<!-- charset="Windows-1252" -->
<!-- inreplyto="8AB3F5B7-5C8B-4A5D-90D0-3932A3D8CE5E_at_open-mpi.org" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2016-03-18 19:05:50
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="28757.php">Ralph Castain: "Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files	limits	&gt;=	4096 still required?"</a>
<li><strong>Previous message:</strong> <a href="28755.php">Andrus, Brian Contractor: "Re: [OMPI users] Dynamically throttle/scale processes"</a>
<li><strong>In reply to:</strong> <a href="28751.php">Ralph Castain: "Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files limits	&gt;=	4096 still required?"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="28757.php">Ralph Castain: "Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files	limits	&gt;=	4096 still required?"</a>
<li><strong>Reply:</strong> <a href="28757.php">Ralph Castain: "Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files	limits	&gt;=	4096 still required?"</a>
<li><strong>Reply:</strong> <a href="28773.php">Dave Love: "Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files	limits	&gt;=	4096 still required?"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
Ralph,
<br>
<p>For the following openMPI job submission:
<br>
<p>qsub -q short.q -V -pe make 84 -b y mpirun -np 84 --prefix /hpc/apps/mpi/openmpi/1.10.1/ --hetero-nodes --mca btl ^sm --mca plm_base_verbose 5 /hpc/home/lanew/mpi/openmpi/a_1_10_1.out
<br>
<p>I have some more information on this issue. All the server daemons are started without error and before
<br>
I ever see the
<br>
...
<br>
<p>qrsh throws the following error for various nodes taking part in the openmpi compute ring:
<br>
<p>unable to write to file /tmp/285507.1.short.q/qrsh_error: No space left on device[csclprd3-4-3:08052] [[24964,0],17] plm:rsh: using &quot;/opt/sge/bin/lx-amd64/qrsh -inherit -nostdin -V -verbose&quot; for launching
<br>
<p>Does each and every node taking part in the openMPI compute ring need to write to a temporary directory?
<br>
<p>Also, the submit node for this cluster has a soft open files limit of 1024 and a hard open files limit of 4096. All
<br>
compute nodes have a hard/soft open files limit of 4096.
<br>
<p>-Bill L.
<br>
...
<br>
Sent: Thursday, March 17, 2016 6:02 PM
<br>
To: Open MPI Users
<br>
Subject: Re: [OMPI users] OpenMPI 1.10.1 *ix hard/soft open files limits &gt;= 4096 still required?
<br>
<p>Yeah, it looks like something is wrong with the mmap backend for some reason. It gets used by both vader and sm, so no help there.
<br>
<p>I&#146;m afraid I&#146;ll have to defer to Nathan from here as he is more familiar with it than I.
<br>
<p><p>On Mar 17, 2016, at 4:55 PM, Lane, William &lt;William.Lane_at_[hidden]&lt;mailto:William.Lane_at_[hidden]&gt;&gt; wrote:
<br>
<p>I ran OpenMPI using the &quot;-mca btl ^vader&quot; switch Ralph recommended and I'm still getting the same errors
<br>
...
<p>________________________________
<br>
From: users [users-bounces_at_[hidden]&lt;mailto:users-bounces_at_[hidden]&gt;] on behalf of Lane, William [William.Lane_at_[hidden]&lt;mailto:William.Lane_at_[hidden]&gt;]
<br>
...
Subscription: <a href="http://www.open-mpi.org/mailman/listinfo.cgi/users">http://www.open-mpi.org/mailman/listinfo.cgi/users</a>
<br>
Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2016/03/28749.php">http://www.open-mpi.org/community/lists/users/2016/03/28749.php</a>
<br>
<p>IMPORTANT WARNING: This message is intended for the use of the person or entity to which it is addressed and may contain information that is privileged and confidential, the disclosure of which is governed by applicable law. If the reader of this message is not the intended recipient, or the employee or agent responsible for delivering it to the intended recipient, you are hereby notified that any dissemination, distribution or copying of this information is strictly prohibited. Thank you for your cooperation.
<br>
<p><p>
<p><hr>
<ul>
<li>text/html attachment: <a href="http://www.open-mpi.org/community/lists/users/att-28756/attachment">attachment</a>
</ul>
