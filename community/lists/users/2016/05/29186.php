...
?>
<!-- received="Thu May 12 03:54:43 2016" -->
<!-- isoreceived="20160512075443" -->
<!-- sent="Thu, 12 May 2016 03:54:38 -0400" -->
<!-- isosent="20160512075438" -->
<!-- name="dpchoudh ." -->
<!-- email="dpchoudh_at_[hidden]" -->
<!-- subject="Re: [OMPI users] No core dump in some cases" -->
<!-- id="CAHXxYDijRQ24W0g-DzyogKZTJ5FMmC7XxCgX_7gCLZi+W_33CA_at_mail.gmail.com" -->
<!-- charset="UTF-8" -->
<!-- inreplyto="CAAkFZ5t8bh2QZjDO+yDJdDnGNnZ1or=BL4Y=EaN0J8w4duC7yg_at_mail.gmail.com" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2016-05-12 03:54:38
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="29187.php">Gilles Gouaillardet: "Re: [OMPI users] No core dump in some cases"</a>
<li><strong>Previous message:</strong> <a href="29185.php">Gilles Gouaillardet: "Re: [OMPI users] No core dump in some cases"</a>
<li><strong>In reply to:</strong> <a href="29185.php">Gilles Gouaillardet: "Re: [OMPI users] No core dump in some cases"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="29187.php">Gilles Gouaillardet: "Re: [OMPI users] No core dump in some cases"</a>
<li><strong>Reply:</strong> <a href="29187.php">Gilles Gouaillardet: "Re: [OMPI users] No core dump in some cases"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
Hello Gilles
<br>
<p>I am not sure if I understand you correctly, but let me answer based on
<br>
...
&lt;/quote&gt;
<br>
<p>If this is true, then there is a bug in OMPI proper, since it is crashing
<br>
...
<br>
<p>As you can see, the crash happens inside the verbs library and the
<br>
...
<br>
<p>That sort of tells me the perhaps the signal handler does more than simply
<br>
...
<br>
<p>The MPI app that I am running is a simple program and it runs fine with the
<br>
...
&lt;/quote&gt;
<br>
<p>But during normal operation, this should not be an issue, right? The signal
<br>
handler, even if it points to unmapped memory, is being invoked in response
<br>
to an exception that will kill the process anyway. The only side effect of
<br>
this I see is that the stack will be misleading. In any case, I am
<br>
compiling with --disable-dlopen set, so my understanding is that since all
<br>
the components are slurped onto one giant .so file, the memory will not be
<br>
unmapped.
<br>
<p>&lt;quote&gt;
...
&lt;/quote&gt;
<br>
<p>I confirm that I have the latest from master (by running 'git pull'). Also,
<br>
I have a single Qlogic card with a single port and here is the output:
<br>
[durga_at_smallMPI ~]$ cat /sys/class/infiniband/qib0/ports/1/state
<br>
1: DOWN
<br>
<p>&lt;quote&gt;
...
&lt;/quote&gt;
<br>
<p>I did configure with --disable-dlopen, but why does that make a difference?
<br>
...
ageing.
<br>
<p>On Thu, May 12, 2016 at 12:38 AM, Gilles Gouaillardet &lt;
<br>
...
<span class="quotelev2">&gt; &gt; There must be a reason why PSM is installing its signal handlers; often
</span><br>
<span class="quotelev1">&gt; this
</span><br>
<span class="quotelev2">&gt; &gt; is done to modify the permission of a page in response to a SEGV and
</span><br>
...
<span class="quotelev3">&gt; &gt;&gt; #2  0x00007f1c1066d0ae in opal_obj_run_destructors (object=0x1c85b70) at
</span><br>
...
<span class="quotelev3">&gt; &gt;&gt; #8  0x00007f1c1173f675 in ompi_mpi_init (argc=1, argv=0x7ffd00dae008,
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; question, I am running it on a single node, so there are no other
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; MPI_Init() itself. The output from your code (I called it 'btrtest')
</span><br>
<span class="quotelev1">&gt; is as
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; Both my 'well-behaving' and crashing clusters run CentOS 7 (the
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; bug in the MPI app) has very simple network hardware: two different
</span><br>
<span class="quotelev1">&gt; NICs
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt; (one Broadcom GbE, one proprietary NIC that is currently being exposed
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; machine to the WAN link), it seems just the presence of these NICs
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; I do realize that my setup is very unusual (I am a quasi-developer of
</span><br>
<span class="quotelev1">&gt; MPI
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt; whereas most other folks in this list are likely end-users), but
</span><br>
<span class="quotelev1">&gt; somehow
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt; just disabling this 'execinfo' MCA would allow me to make progress
</span><br>
<span class="quotelev1">&gt; (and also
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt; find out why/where MPI_Init() is crashing!). Is there any way I can do
</span><br>
...
<span class="quotelev1">&gt; files for
</span><br>
...
<span class="quotelev2">&gt; &gt;&gt;&gt;&gt;&gt; you might wanna try to restore the signal handler for other signals
</span><br>
...
<span class="quotelev3">&gt; &gt;&gt;&gt;&gt;&gt; &gt; I also find it puzzling that nearly identical OMPI distro running
</span><br>
...
<span class="quotelev3">&gt; &gt;&gt;&gt;&gt;&gt; &gt; The surgeon general advises you to eat right, exercise regularly
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; should revisit it at some point but for now the only effective
</span><br>
<span class="quotelev1">&gt; way i
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; have
</span><br>
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; found to prevent it is to restore the default signal handlers
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; Thanks for your suggestion. But I am not using any resource
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; creates the 'btr; files. Strange thing is, I could not find
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; If you're using a resource manager to start the jobs (Torque,
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; I'm afraid I don't know what a .btr file is -- that is not
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; 'clusters' have two nodes only, connected back-to-back. The two
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt;&gt;&gt; &gt;&gt; more options than a static text file with addresses. How do I get
</span><br>
...
<p><hr>
<ul>
<li>text/html attachment: <a href="http://www.open-mpi.org/community/lists/users/att-29186/attachment">attachment</a>
</ul>
