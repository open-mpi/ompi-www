<?
$subject_val = "[OMPI users] Busy waiting [was Re:  (no subject)]";
include("../../include/msg-header.inc");
?>
<!-- received="Wed Apr 23 18:20:09 2008" -->
<!-- isoreceived="20080423222009" -->
<!-- sent="Wed, 23 Apr 2008 18:19:59 -0400" -->
<!-- isosent="20080423221959" -->
<!-- name="Barry Rountree" -->
<!-- email="rountree_at_[hidden]" -->
<!-- subject="[OMPI users] Busy waiting [was Re:  (no subject)]" -->
<!-- id="20080423221959.GA23971_at_eponymous" -->
<!-- charset="us-ascii" -->
...
<p class="headers">
<strong>Subject:</strong> [OMPI users] Busy waiting [was Re:  (no subject)]<br>
<strong>From:</strong> Barry Rountree (<em>rountree_at_[hidden]</em>)<br>
<strong>Date:</strong> 2008-04-23 18:19:59
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="5475.php">Alberto Giannetti: "Re: [OMPI users] Processor affinitiy"</a>
<li><strong>Previous message:</strong> <a href="5473.php">Jeff Squyres: "Re: [OMPI users] (no subject)"</a>
<li><strong>In reply to:</strong> <a href="5472.php">Ingo Josopait: "Re: [OMPI users] (no subject)"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="5481.php">Ingo Josopait: "Re: [OMPI users] Busy waiting [was Re:  (no subject)]"</a>
<li><strong>Reply:</strong> <a href="5481.php">Ingo Josopait: "Re: [OMPI users] Busy waiting [was Re:  (no subject)]"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<span class="quotelev1">&gt; I can think of several advantages that using blocking or signals to
</span><br>
...
<p>Not necessarily.  Any time the program ends up running longer, the
<br>
...
<br>
<p><span class="quotelev1">&gt; - Running additional background programs could be done far more efficiently
</span><br>
<p>It's usually more efficient -- especially in terms of cache -- to batch
<br>
up programs to run one after the other instead of running them
<br>
simultaneously.  
<br>
<p><span class="quotelev1">&gt; - It would be much simpler to examine the load balance.
</span><br>
<p>This is true, but it's still pretty trivial to measure load imbalance.
<br>
...
<br>
<p>int MPI_Barrier(MPI_Comm comm){
<br>
...
<br>
<p>I've got some code that does this for all of the MPI calls in OpenMPI
<br>
(ah, the joys of writing C code using python scripts).  Let me know if
<br>
you'd find it useful.
<br>
<p><span class="quotelev1">&gt; It may depend on the type of program and the computational environment,
</span><br>
...
<p>&lt;grin&gt;  I could use a few more benchmarks like that.  Seriously, if
<br>
...
<br>
<p>Doing this just for Barriers and Allreduces seems to speed up the
<br>
program a tiny bit, but I haven't done enough runs to make sure this
<br>
isn't an artifact.
<br>
<p>(This is my dissertation topic, so before asking any question be advised
<br>
that I WILL talk your ear off.)
<br>
&nbsp;
<br>
<span class="quotelev1">&gt; The &quot;spin for a while and then block&quot; method that you mentioned earlier
</span><br>
<span class="quotelev1">&gt; seems to be a good compromise. Just do polling for some time that is
</span><br>
<span class="quotelev1">&gt; long compared to the corresponding system call, and then go to sleep if
</span><br>
...
<p>I'm interested in seeing what this does for energy savings.  Are you
<br>
...
<span class="quotelev2">&gt; &gt; optimize spare CPU cycles for them.  As I said in a prior mail, adding  
</span><br>
...
<span class="quotelev2">&gt; &gt; latency.  Some of them will shift to blocking if nothing happens for a  
</span><br>
...
<span class="quotelev2">&gt; &gt; FWIW: I mentioned this in my other mail -- latency is quite definitely  
</span><br>
...
<span class="quotelev2">&gt; &gt; busy polling (i.e., they call yield() much less frequently, if at all).
</span><br>
<span class="quotelev2">&gt; &gt; 
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; communication, we have to alternate between polling shared memory and
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; latency.  MPI implementations are almost always first judged on their
</span><br>
...
<span class="quotelev4">&gt; &gt;&gt;&gt; We have plans for implementing the &quot;spin for a while and then block&quot;
</span><br>
