...
?>
<!-- received="Thu Mar  6 15:16:27 2014" -->
<!-- isoreceived="20140306201627" -->
<!-- sent="Thu, 06 Mar 2014 15:16:21 -0500" -->
<!-- isosent="20140306201621" -->
<!-- name="Gus Correa" -->
<!-- email="gus_at_[hidden]" -->
<!-- subject="Re: [OMPI users] OpenMPI job initializing problem" -->
<!-- id="5318D795.5050103_at_ldeo.columbia.edu" -->
<!-- charset="ISO-8859-1" -->
<!-- inreplyto="D67303F39678724DA65819E042CB369892A81D0B20_at_EXC2.ad.colorado.edu" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2014-03-06 15:16:21
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="23780.php">Beichuan Yan: "Re: [OMPI users] OpenMPI job initializing problem"</a>
<li><strong>Previous message:</strong> <a href="23778.php">Javier Garcia Blas: "[OMPI users] CFP: Workshop on Enhancing Parallel Scientific Applications with Accelerated HPC"</a>
<li><strong>In reply to:</strong> <a href="23777.php">Beichuan Yan: "Re: [OMPI users] OpenMPI job initializing problem"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="23780.php">Beichuan Yan: "Re: [OMPI users] OpenMPI job initializing problem"</a>
<li><strong>Reply:</strong> <a href="23780.php">Beichuan Yan: "Re: [OMPI users] OpenMPI job initializing problem"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
Hi Beichuan
<br>
<p>So, it looks like that now the program runs,
<br>
even though with specific settings depending on whether you're
<br>
using OMPI 1.6.5 or 1.7.4, right?
<br>
<p>It looks like the problem now is performance, right?
<br>
<p>System load affects performance, but unless the network is overwhelmed,
<br>
or perhaps the Lustre file system is hanging or too slow,
<br>
I would think that a walltime increase from 1min to 10min is not
<br>
related to system load, but something else.
<br>
<p>Do you remember the setup that gave you 1min walltime?
<br>
Was it the same that you sent below?
<br>
Do you happen to know which nodes?
<br>
Are you sharing nodes with other jobs, or are you running alone on
<br>
the nodes?
<br>
...
<br>
<p>I suggest that you do:
<br>
cat $PBS_NODEFILE
<br>
in your PBS script, just to document which nodes are
<br>
actually given to you.
<br>
<p>Also helpful to document/troubleshoot is to add
<br>
-v and -tag-output
<br>
to your mpiexec command line.
<br>
<p><p>The difference in walltime could be due to some malfunction
<br>
of IB HCAs on the nodes, for instance.
<br>
Since you are allowing (if I remember right) the use of TCP,
<br>
OpenMPI will try to use any interfaces that you did not rule out.
<br>
If your mpiexec command line doesn't make any restriction, it will use
<br>
anything available, if I remember right.
<br>
(Jeff will correct me in the next second.)
<br>
If your mpiexec command line has
<br>
mca btl_tcp_if_include 10.148.0.0/16
<br>
it will use the 10.148.0.0/16 subnet in with TCP transport, I think.
<br>
(Jeff will cut my list subscription after that one,
<br>
for spreading misinformation.)
<br>
<p>In either case my impression is that you may have left a door open to
<br>
the use of non-IB (and non-IB-verbs) transport.
<br>
<p>Is 10.148.0.0/16 the an Infiniband subnet or an Ethernet subnet?
<br>
<p>Did you remeber Jeff's suggestion from a while ago to avoid TCP (over 
<br>
Ethernet or over IB), and stick to IB verbs?
<br>
<p><p>Is 10.148.0.0/16 the IB or the Ethernet subnet?
<br>
<p>On 03/02/2014 02:38 PM, Jeff Squyres (jsquyres) wrote:
<br>
...
<p><p>You could force the use of IB verbs with
<br>
<p>-mca btl ^tcp
<br>
<p>or with
...
<br>
<p>on the mpiexec command line.
<br>
<p>In this case, if any of the IB HCAs on the nodes is bad,
<br>
the job will abort with an error message, instead of running too slow
<br>
(if it is using other networks).
<br>
<p>There are also ways to tell OMPI to do a more verbose output,
<br>
...
<br>
<p><p>Believe me, this did happen to me, i.e., to run MPI programs in a
<br>
...
Gus Correa
<br>
<p><p><p>On 03/06/2014 01:49 PM, Beichuan Yan wrote:
<br>
...
<span class="quotelev1">&gt; Warning: shared-memory, /work1/home/yanb/tmp/
</span><br>
<span class="quotelev1">&gt; Run, take 10 minutes, slow
</span><br>
<span class="quotelev1">&gt;
</span><br>
...
<span class="quotelev1">&gt; Warning: shared-memory /work3/yanb/605832.SPIRIT/
</span><br>
<span class="quotelev1">&gt; Run, take 10 minutess, slow
</span><br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt; So you see, a) openmpi 1.6.5 and 1.7.4 need different settings to run;
</span><br>
b) whether specifying TMPDIR, I got the shared memory warning.
<br>
<span class="quotelev1">&gt;
</span><br>
<span class="quotelev1">&gt; 3. But a few days ago, OpenMPI 1.6.5 worked great and took only 1 minute
</span><br>
...
<span class="quotelev1">&gt; [You could ask the sys admin about this, and perhaps what he recommends to use to replace /tmp.]
</span><br>
<span class="quotelev1">&gt;
</span><br>
...
<span class="quotelev2">&gt;&gt;                         1.2P  136T  1.1P  12% /work1 -bash-4.1$ mount
</span><br>
<span class="quotelev2">&gt;&gt; devpts on /dev/pts type devpts (rw,gid=5,mode=620) tmpfs on /tmp type
</span><br>
<span class="quotelev2">&gt;&gt; tmpfs (rw,size=150m) none on /proc/sys/fs/binfmt_misc type binfmt_misc
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; If the default TMPDIR is on Lustre (did you say this?, anyway I don't
</span><br>
...
<span class="quotelev2">&gt;&gt;&gt;&gt;&gt;&gt; cpupercent=0,cput=00:00:00,mem=7028kb,ncpus=128,vmem=495768kb,wall
</span><br>
