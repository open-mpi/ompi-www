...
?>
<!-- received="Thu Aug 21 06:44:26 2014" -->
<!-- isoreceived="20140821104426" -->
<!-- sent="Thu, 21 Aug 2014 12:44:17 +0200" -->
<!-- isosent="20140821104417" -->
<!-- name="Reuti" -->
<!-- email="reuti_at_[hidden]" -->
<!-- subject="Re: [OMPI users] Running a hybrid MPI+openMP program" -->
<!-- id="35DD363D-3935-4224-B874-758AAC636591_at_staff.uni-marburg.de" -->
<!-- charset="iso-8859-1" -->
...
<strong>Date:</strong> 2014-08-21 06:44:17
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="25109.php">Ralph Castain: "Re: [OMPI users] ORTE daemon has unexpectedly failed after launch"</a>
<li><strong>Previous message:</strong> <a href="25107.php">Reuti: "Re: [OMPI users] Running a hybrid MPI+openMP program"</a>
<li><strong>In reply to:</strong> <a href="25096.php">Oscar Mojica: "Re: [OMPI users] Running a hybrid MPI+openMP program"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="25019.php">Reuti: "Re: [OMPI users] Running a hybrid MPI+openMP program"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
Hi,
<br>
<p>Am 20.08.2014 um 20:08 schrieb Oscar Mojica:
<br>
<p><span class="quotelev1">&gt; Well, with qconf -sq one.q I got the following:
</span><br>
...
<span class="quotelev1">&gt; hostlist                 compute-1-30.local compute-1-2.local compute-1-3.local \
</span><br>
<span class="quotelev1">&gt;                       compute-1-4.local compute-1-5.local compute-1-6.local \
</span><br>
<span class="quotelev1">&gt;                       compute-1-7.local compute-1-8.local compute-1-9.local \
</span><br>
<span class="quotelev1">&gt;                       compute-1-10.local compute-1-11.local compute-1-12.local \
</span><br>
<span class="quotelev1">&gt;                       compute-1-13.local compute-1-14.local compute-1-15.local
</span><br>
...
<span class="quotelev1">&gt; the admin was who created this queue, so I have to speak to him to change the number of slots to number of threads that i wish to use. 
</span><br>
<p>Yep. I think it was his intention to allow an exclusive use of each node by this (this can be done in SGE by other means too). While one could do it, it doesn't reflect the proper amount of cores to SGE the user wants to use (it's more like the number of machines) and so any accounting won't work, or getting from `qacct` the correct information what the job requested at time he was submitted.
<br>
<p><p><span class="quotelev1">&gt; Then I could make use of: 
</span><br>
<span class="quotelev1">&gt; ===
</span><br>
...
<p>As mentioned by tmishima, it's sufficient to use:
<br>
<p>$ qsub -pe orte 80 ...
<br>
...
<br>
<p><p>=&gt; you get a proper binding here, either if you are alone on each machine or all jobs get proper binding and Open MPI stays inside it (not all versions of SGE support this though)
<br>
<p><p><span class="quotelev1">&gt; For now in my case this command line just would work for 10 processes and the work wouldn't be divided in threads, is it right?
</span><br>
<p>It works for 10 machines which you get exclusively, hence oversubscribing the granted single slot on each machine with &quot;-bind-to none&quot; what Ralph mentioned in the beginning is up to you (unless other users would get hurt as they are having there jobs too).
<br>
<p>$ qsub -pe orte 10 ...
<br>
<p>export OMP_NUM_THREADS=8
...
<br>
<p><p>=&gt; The OS will shift the processes around, while SGE doesn't know anything about the final number of slots/cores you want to use on each machine (or to leave free for others).
<br>
<p>===
<br>
<p>Both ways above work right now, but IMO it's not the optimum in a shared cluster for the SGE versions w/o hard-binding. In the second case Open MPI starts 1 process per node as we need it. In case you would request here `qsub -pe orte 80 ...` here too, Open MPI would start 80 processes. To avoid this I came up with altering the machinefile to give Open MPI a different information about the granted slots on each machine.
<br>
<p>$ qsub -pe orte 80 ...
<br>
<p>export OMP_NUM_THREADS=8
<br>
awk -v omp_num_threads=$OMP_NUM_THREADS '{ $2/=omp_num_threads; print }' $PE_HOSTFILE &gt; $TMPDIR/machines
<br>
...
<p>===
<br>
<p>I hope having all three versions in one email sheds some light on it.
<br>
<p>-- Reuti
<br>
<p><p><span class="quotelev1">&gt; can I set a maximum number of threads in the queue one.q (e.g. 15 ) and change the number in the 'export' for my convenience
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt; I discovered what was the error. I forgot include the '-fopenmp' when I compiled the objects in the Makefile, so the program worked but it didn't divide the job in threads. Now the program is working and I can use until 15 cores for machine in the queue one.q.
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt; c) I'm not sure how to do this step. I'd appreciate your help here. I can add some lines to the script to determine the PE_HOSTFILE path and contents, but i don't know how alter it 
</span><br>
<span class="quotelev2">&gt; &gt; 
</span><br>
...
<span class="quotelev2">&gt; &gt; - the &quot;start_proc_args&quot; would need to know the number of threads, i.e. OMP_NUM_THREADS must be defined by &quot;qsub -v ...&quot; outside of the jobscript (tricky scanning of the submitted jobscript for OMP_NUM_THREADS would be too nasty)
</span><br>
...
<span class="quotelev2">&gt; &gt; - each and every user must include it in all the jobscripts to adjust the list and export the pointer to the $PE_HOSTFILE, but he could change it forth and back for different steps of the jobscript though
</span><br>
<span class="quotelev2">&gt; &gt; 
</span><br>
<span class="quotelev2">&gt; &gt; 
...
<span class="quotelev4">&gt; &gt; &gt; &gt; Yes, here you got 10 slots (= cores) granted by SGE. So there is no free core left inside the allocation of SGE to allow the use of additional cores for your threads. If you use more cores than granted by SGE, it will oversubscribe the machines.
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev4">&gt; &gt; &gt; &gt; b) Although you specified $fill_up as allocation rule, it looks like $round_robin. Is there more than one slot defined in the queue definition of one.q to get exclusive access?
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; c) What version of SGE are you using? Certain ones use cgroups or bind processes directly to cores (although it usually needs to be requested by the job: first line of `qconf -help`).
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; In case you are alone in the cluster, you could bypass the allocation with b) (unless you are hit by c)). But having a mixture of users and jobs a different handling would be necessary to handle this in a proper way IMO:
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev4">&gt; &gt; &gt; &gt; c) copy and alter the $PE_HOSTFILE to show only (granted core count per machine) divided by (OMP_NUM_THREADS) per entry, change $PE_HOSTFILE so that it points to the altered file
</span><br>
<span class="quotelev4">&gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev1">&gt; &gt; &gt; &gt; &gt; So the job is using the 10 machines, Until here is all right OK. Do you think that changing the &quot;allocation_rule &quot; to a number instead $fill_up the MPI processes would divide the work in that number of threads?
</span><br>
<span class="quotelev1">&gt; &gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; I think this is a broader issue in case an MPI library is used in conjunction with threads while running inside a queuing system. First: whether your actual installation of Open MPI is SGE-aware you can check with:
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; AFAICS: for now there is no way in the Open MPI + SGE combination to specify the number of MPI processes and intended number of threads which are automatically read by Open MPI while staying inside the granted slot count and allocation. So it seems to be necessary to have the intended number of threads being honored by Open MPI too.
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; 
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; Hence specifying e.g. &quot;allocation_rule 8&quot; in such a setup while requesting 32 processes, would for now start 32 processes by MPI already, as Open MP reads the $PE_HOSTFILE and acts accordingly.
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; 
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; Open MPI would have to read the generated machine file in a slightly different way regarding threads: a) read the $PE_HOSTFILE, b) divide the granted slots per machine by OMP_NUM_THREADS, c) throw an error in case it's not divisible by OMP_NUM_THREADS. Then start one process per quotient.
</span><br>
<span class="quotelev2">&gt; &gt; &gt; &gt; &gt; &gt; 
</span><br>
...
<span class="quotelev1">&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/08/25096.php">http://www.open-mpi.org/community/lists/users/2014/08/25096.php</a>
</span><br>
<!-- body="end" -->
<hr>
<ul class="links">
