...
?>
<!-- received="Tue Sep  2 19:04:38 2014" -->
<!-- isoreceived="20140902230438" -->
<!-- sent="Tue, 2 Sep 2014 16:04:24 -0700" -->
<!-- isosent="20140902230424" -->
<!-- name="Ralph Castain" -->
<!-- email="rhc_at_[hidden]" -->
<!-- subject="Re: [OMPI users] Issues with OpenMPI 1.8.2, GCC 4.9.1, and SLURM Interactive Jobs" -->
<!-- id="AB610FDB-E31F-4498-8123-608C91790648_at_open-mpi.org" -->
<!-- charset="us-ascii" -->
...
<strong>Date:</strong> 2014-09-02 19:04:24
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="25235.php">Jeff Squyres (jsquyres): "Re: [OMPI users] Issues with OpenMPI 1.8.2, GCC 4.9.1, and SLURM Interactive Jobs"</a>
<li><strong>Previous message:</strong> <a href="25233.php">Jeff Squyres (jsquyres): "Re: [OMPI users] How does binding option affect network traffic?"</a>
...
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="25235.php">Jeff Squyres (jsquyres): "Re: [OMPI users] Issues with OpenMPI 1.8.2, GCC 4.9.1, and SLURM Interactive Jobs"</a>
<li><strong>Reply:</strong> <a href="25235.php">Jeff Squyres (jsquyres): "Re: [OMPI users] Issues with OpenMPI 1.8.2, GCC 4.9.1, and SLURM Interactive Jobs"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
<p>
I can answer that for you right now. The launch of the orted's is what is failing, and they are &quot;silently&quot; failing at this time. The reason is simple:
<br>
<p>1. we are failing due to truncation of the HNP uri at the first semicolon. This causes the orted to emit an ORTE_ERROR_LOG message and then abort with a non-zero exit status
<br>
<p>2. we throw away the error message unless someone adds --debug-daemons because we redirect the srun output to /dev/null. This is done because slurm spits out other things during our normal operation that confuse users
<br>
<p>3. srun detects the non-zero exit status of the orted and aborts the rest of the job.
<br>
<p>So when Matt adds --debug-daemons, he then sees the error messages. When he further adds the oob and plm verbosity, the true error is fully exposed.
<br>
<p><p>On Sep 2, 2014, at 2:35 PM, Jeff Squyres (jsquyres) &lt;jsquyres_at_[hidden]&gt; wrote:
<br>
...
<span class="quotelev1">&gt; Can you check one thing for me?  With the un-edited 1.8.2 tarball installation, I see that you're getting no output for commands that you run -- but also no errors.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev1">&gt; $ /discover/nobackup/mathomp4/MPI/gcc_4.9.1-openmpi_1.8.2-clean/bin/mpirun -np 8 script.sh
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; and then go &quot;ps&quot; on the back-end nodes and see if there is an &quot;orted&quot; process and N &quot;sleep 600&quot; processes running on them.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev1">&gt; The purpose of this test is that I want to see if OMPI is just totally erring out and not even running your job (which is quite unlikely; OMPI should be much more noisy when this happens), or whether we're simply not seeing the stdout from the job.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev2">&gt;&gt; On that machine, it would be SLES 11 SP1. I think it's soon transitioning to SLES 11 SP3.
</span><br>
...
<span class="quotelev2">&gt;&gt; Thanks - I expect we'll have to release 1.8.3 soon to fix this in case others have similar issues. Out of curiosity, what OS are you using?
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; 
...
<span class="quotelev3">&gt;&gt;&gt; Okay that seems to have done it here (well, minus the usual shmem_mmap_enable_nfs_warning that our system always generates):
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; (1033) $ setenv OMPI_MCA_shmem_mmap_enable_nfs_warning 0
</span><br>
<span class="quotelev3">&gt;&gt;&gt; (1034) $ /discover/nobackup/mathomp4/MPI/gcc_4.9.1-openmpi_1.8.2-debug-patch/bin/mpirun -np 8 ./helloWorld.182-debug-patch.x
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; I'll ask the admin to apply the patch locally...and wait for 1.8.3, I suppose.
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; On Sun, Aug 31, 2014 at 10:08 AM, Ralph Castain &lt;rhc_at_[hidden]&gt; wrote:
</span><br>
<span class="quotelev3">&gt;&gt;&gt; Hmmm....I may see the problem. Would you be so kind as to apply the attached patch to your 1.8.2 code, rebuild, and try again?
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; Much appreciate the help. Everyone's system is slightly different, and I think you've uncovered one of those differences.
</span><br>
<span class="quotelev3">&gt;&gt;&gt; Ralph
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt; (1002) $ /discover/nobackup/mathomp4/MPI/gcc_4.9.1-openmpi_1.8.2-debug/bin/mpirun --leave-session-attached --debug-daemons --mca oob_base_verbose 10 -mca plm_base_verbose 5 -np 8 ./helloWorld.182-debug.x
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Querying component [isolated]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Query of component [isolated] set priority to 0
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Querying component [rsh]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[INVALID],INVALID] plm:rsh_lookup on agent ssh : rsh path NULL
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Query of component [rsh] set priority to 10
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Querying component [slurm]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[INVALID],INVALID] plm:slurm: available for selection
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Query of component [slurm] set priority to 75
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca:base:select:(  plm) Selected component [slurm]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] plm:base:set_hnp_name: initial bias 3815 nodename hash 1757783593
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] plm:base:set_hnp_name: final jobfam 49163
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_open: opening oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] oob:tcp:init adding 10.1.24.63 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] WORKING INTERFACE 4 KERNEL INDEX 4 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] oob:tcp:init adding 10.12.24.63 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm add new daemon [[49163,0],1]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm assigning new daemon [[49163,0],1] to node borg01w064
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm add new daemon [[49163,0],2]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm assigning new daemon [[49163,0],2] to node borg01w065
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm add new daemon [[49163,0],3]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm assigning new daemon [[49163,0],3] to node borg01w069
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm add new daemon [[49163,0],4]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm assigning new daemon [[49163,0],4] to node borg01w070
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm add new daemon [[49163,0],5]
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:base:setup_vm assigning new daemon [[49163,0],5] to node borg01w071
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:slurm: launching on nodes borg01w064,borg01w065,borg01w069,borg01w070,borg01w071
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:slurm: Set prefix:/discover/nobackup/mathomp4/MPI/gcc_4.9.1-openmpi_1.8.2-debug
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:slurm: final top-level argv:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 	srun --ntasks-per-node=1 --kill-on-bad-exit --cpu_bind=none --nodes=5 --nodelist=borg01w064,borg01w065,borg01w069,borg01w070,borg01w071 --ntasks=5 orted -mca orte_debug_daemons 1 -mca orte_leave_session_attached 1 -mca orte_ess_jobid 3221946368 -mca orte_ess_vpid 1 -mca orte_ess_num_procs 6 -mca orte_hnp_uri 3221946368.0;tcp://10.1.24.63,172.31.1.254,10.12.24.63:41373 --mca oob_base_verbose 10 -mca plm_base_verbose 5
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w063:03815] [[49163,0],0] plm:slurm: reset PATH: /discover/nobackup/mathomp4/MPI/gcc_4.9.1-openmpi_1.8.2-debug/bin:/usr/local/other/SLES11/gcc/4.9.1/bin:/usr/local/other/SLES11.1/git/1.8.5.2/libexec/git-core:/usr/local/other/SLES11.1/git/1.8.5.2/bin:/usr/local/other/SLES11/svn/1.6.17/bin:/usr/local/other/SLES11/tkcvs/8.2.3/gcc-4.3.2/bin:.:/home/mathomp4/bin:/home/mathomp4/cvstools:/discover/nobackup/projects/gmao/share/dasilva/opengrads/Contents:/usr/local/other/Htop/1.0/bin:/usr/local/other/SLES11/gnuplot/4.6.0/gcc-4.3.2/bin:/usr/local/other/SLES11/xpdf/3.03-gcc-4.3.2/bin:/home/mathomp4/src/pdtoolkit-3.16/x86_64/bin:/discover/nobackup/mathomp4/WavewatchIII-GMAO/bin:/discover/nobackup/mathomp4/WavewatchIII-GMAO/exe:/usr/local/other/pods:/usr/local/other/SLES11.1/R/3.1.0/gcc-4.3.4/lib64/R/bin:.:/home/mathomp4/bin:/home/mathomp4/cvstools:/discover/nobackup/projects/gmao/share/dasilva/opengrads/Contents:/usr/local/other/Htop/1.0/bin:/usr/local/other/SLES11/gnuplot/4.6
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_open: opening oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] oob:tcp:init adding 10.1.24.65 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] WORKING INTERFACE 4 KERNEL INDEX 4 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] oob:tcp:init adding 10.12.24.65 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_open: opening oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] oob:tcp:init adding 10.1.24.70 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] WORKING INTERFACE 4 KERNEL INDEX 4 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] oob:tcp:init adding 10.12.24.70 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_open: opening oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] oob:tcp:init adding 10.1.24.64 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] WORKING INTERFACE 4 KERNEL INDEX 4 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] oob:tcp:init adding 10.12.24.64 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] mca: base: components_register: registering oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] mca: base: components_register: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] mca: base: components_register: component tcp register function successful
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_open: opening oob components
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] mca: base: components_open: found loaded component tcp
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] mca: base: components_open: component tcp open function successful
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] oob:tcp:init adding 10.1.24.71 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] oob:tcp:init adding 10.12.24.71 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] oob:tcp:init adding 10.1.24.69 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] WORKING INTERFACE 4 KERNEL INDEX 4 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] oob:tcp:init adding 172.31.1.254 to our list of V4 connections
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] WORKING INTERFACE 5 KERNEL INDEX 5 FAMILY: V4
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] oob:tcp:init adding 10.12.24.69 to our list of V4 connections
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] ORTE_ERROR_LOG: Bad parameter in file base/rml_base_contact.c at line 161
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] ORTE_ERROR_LOG: Bad parameter in file routed_binomial.c at line 498
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w070:12645] [[49163,0],4] ORTE_ERROR_LOG: Bad parameter in file base/ess_base_std_orted.c at line 539
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] ORTE_ERROR_LOG: Bad parameter in file base/rml_base_contact.c at line 161
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] ORTE_ERROR_LOG: Bad parameter in file routed_binomial.c at line 498
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w065:15893] [[49163,0],2] ORTE_ERROR_LOG: Bad parameter in file base/ess_base_std_orted.c at line 539
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w065]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w070]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] ORTE_ERROR_LOG: Bad parameter in file base/rml_base_contact.c at line 161
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] ORTE_ERROR_LOG: Bad parameter in file routed_binomial.c at line 498
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w064:16565] [[49163,0],1] ORTE_ERROR_LOG: Bad parameter in file base/ess_base_std_orted.c at line 539
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] ORTE_ERROR_LOG: Bad parameter in file base/rml_base_contact.c at line 161
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] ORTE_ERROR_LOG: Bad parameter in file routed_binomial.c at line 498
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w069:30276] [[49163,0],3] ORTE_ERROR_LOG: Bad parameter in file base/ess_base_std_orted.c at line 539
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w069]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] ORTE_ERROR_LOG: Bad parameter in file base/rml_base_contact.c at line 161
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] ORTE_ERROR_LOG: Bad parameter in file routed_binomial.c at line 498
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; [borg01w071:14879] [[49163,0],5] ORTE_ERROR_LOG: Bad parameter in file base/ess_base_std_orted.c at line 539
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w071]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w065]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; slurmd[borg01w069]: *** STEP 2347743.3 KILLED AT 2014-08-31T09:24:17 WITH SIGNAL 9 ***
...
<span class="quotelev4">&gt;&gt;&gt;&gt; sh: tcp://10.1.24.63,172.31.1.254,10.12.24.63:41373: No such file or directory
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; On Fri, Aug 29, 2014 at 3:18 PM, Ralph Castain &lt;rhc_at_[hidden]&gt; wrote:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; Rats - I also need &quot;-mca plm_base_verbose 5&quot; on there so I can see the cmd line being executed. Can you add it?
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/08/25197.php">http://www.open-mpi.org/community/lists/users/2014/08/25197.php</a>
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/08/25204.php">http://www.open-mpi.org/community/lists/users/2014/08/25204.php</a>
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
...
<span class="quotelev3">&gt;&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/08/25205.php">http://www.open-mpi.org/community/lists/users/2014/08/25205.php</a>
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
...
<span class="quotelev3">&gt;&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/09/25210.php">http://www.open-mpi.org/community/lists/users/2014/09/25210.php</a>
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; 
...
<span class="quotelev2">&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/09/25211.php">http://www.open-mpi.org/community/lists/users/2014/09/25211.php</a>
</span><br>
<span class="quotelev2">&gt;&gt; 
</span><br>
<span class="quotelev2">&gt;&gt; 
...
<span class="quotelev2">&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/09/25219.php">http://www.open-mpi.org/community/lists/users/2014/09/25219.php</a>
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; 
...
<span class="quotelev1">&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2014/09/25232.php">http://www.open-mpi.org/community/lists/users/2014/09/25232.php</a>
</span><br>
<!-- body="end" -->
<hr>
<ul class="links">
