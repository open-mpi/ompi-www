<?
$subject_val = "[OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code";
include("../../include/msg-header.inc");
?>
<!-- received="Mon Oct 27 08:41:14 2014" -->
<!-- isoreceived="20141027124114" -->
<!-- sent="Mon, 27 Oct 2014 12:41:10 +0000" -->
<!-- isosent="20141027124110" -->
<!-- name="Michael.Rachner_at_[hidden]" -->
<!-- email="Michael.Rachner_at_[hidden]" -->
<!-- subject="[OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code" -->
<!-- id="8337B490252F0944BF1D38541059627E173625A4_at_DLREXMBX01.intra.dlr.de" -->
<!-- charset="iso-8859-1" -->
...
<p class="headers">
<strong>Subject:</strong> [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code<br>
...
<strong>Date:</strong> 2014-10-27 08:41:10
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="25608.php">Gilles Gouaillardet: "Re: [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code"</a>
<li><strong>Previous message:</strong> <a href="25606.php">Gilles Gouaillardet: "Re: [OMPI users] OMPI users] Possible Memory Leak in simple PingPong-Routine with OpenMPI 1.8.3?"</a>
...
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="25608.php">Gilles Gouaillardet: "Re: [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code"</a>
<li><strong>Maybe reply:</strong> <a href="25608.php">Gilles Gouaillardet: "Re: [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code"</a>
<li><strong>Maybe reply:</strong> <a href="25609.php">Gilles Gouaillardet: "Re: [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code"</a>
<li><strong>Maybe reply:</strong> <a href="25611.php">Michael.Rachner_at_[hidden]: "Re: [OMPI users] WG: Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<p>We have now installed and tested the bugfixed OPENMPI Nightly Tarball  of 2014-10-24  (openmpi-dev-176-g9334abc.tar.gz) on Cluster5 .
<br>
As before (with OPENMPI-1.8.3 release version) the small Ftn-testprogram runs correctly on the login-node.
<br>
As before the program aborts on the compute node, but now with a different error message: 
<br>
<p>The following message appears when launching the program with 2 processes:     mpiexec -np 2 -bind-to core -tag-output ./a.out
<br>
************************************************************************************************
<br>
[1,0]&lt;stdout&gt;: ========on nodemaster: iwin=         685 :
<br>
[1,0]&lt;stdout&gt;:  total storage [MByte] alloc. in shared windows so far:   137.000000000000
<br>
[ [1,0]&lt;stdout&gt;: =========== allocation of shared window no. iwin=         686
<br>
[1,0]&lt;stdout&gt;:  starting now with idim_1=       50000
<br>
-------------------------------------------------------------------------
<br>
It appears as if there is not enough space for /tmp/openmpi-sessions-rachner_at_r5i5n13_0/48127/1/shared_window_688.r5i5n13 (the shared-memory backing
<br>
file). It is likely that your MPI job will now either abort or experience
<br>
performance degradation.
<br>
<p>&nbsp;&nbsp;Local host:  r5i5n13
<br>
...
<br>
[r5i5n13:26917] *** MPI_ERRORS_ARE_FATAL (processes in this communicator will now abort,
<br>
[r5i5n13:26917] ***    and potentially your MPI job)
<br>
rachner_at_r5i5n13:~/dat&gt;
<br>
************************************************************************************************
<br>
<p><p>When I repeat the run using 24 processes (on same compute node) the same kind of abort message occurs, but earlier:
<br>
************************************************************************************************
<br>
[1,0]&lt;stdout&gt;: ========on nodemaster: iwin=         231 :
<br>
[1,0]&lt;stdout&gt;:  total storage [MByte] alloc. in shared windows so far:   46.2000000000000
<br>
&nbsp;[1,0]&lt;stdout&gt;: =========== allocation of shared window no. iwin=         232
<br>
[1,0]&lt;stdout&gt;:  starting now with idim_1=       50000
<br>
-------------------------------------------------------------------------
<br>
It appears as if there is not enough space for /tmp/openmpi-sessions-rachner_at_r5i5n13_0/48029/1/shared_window_234.r5i5n13 (the shared-memory backing
<br>
file). It is likely that your MPI job will now either abort or experience
<br>
performance degradation.
...
<br>
[r5i5n13:26947] *** MPI_ERRORS_ARE_FATAL (processes in this communicator will now abort,
<br>
...
************************************************************************************************
<br>
<p>So the problem is not yet resolved.
<br>
<p>Greetings
<br>
&nbsp;Michael Rachner
...
An: 'Open MPI Users'
<br>
Betreff: AW: [OMPI users] Bug in OpenMPI-1.8.3: storage limition in shared memory allocation (MPI_WIN_ALLOCATE_SHARED) in Ftn-code
<br>
<p>Dear Mr. Squyres.
<br>
...
<br>
<p>Let me add the information that on the Laki each nodes has 16 GB of shared memory (there it worked), the login-node on Cluster 5 has 64 GB (there it worked too), whereas the compute nodes on Cluster5 have 128 GB (there it did not work).
<br>
