 ...
 *
 * Sadly it doesn't work as expected (it even breaks with an
 * error) due to the following reason.
 *
 ...
 *   mpijavac MatMultWithNproc2DarrayMain.java
 *
 * Usage:
 *   mpiexec [parameters] java [parameters] \
 *	MatMultWithNproc2DarrayMain
 *
 * Examples:
 *   mpiexec -np 4 java MatMultWithNproc2DarrayMain
 *   mpiexec -np 4 java -cp $HOME/mpi_classfiles \
 *	MatMultWithNproc2DarrayMain
 *
 *
 * File: MatMultWithNproc2DarrayMain.java	Author: S. Gross
 * Date: 15.01.2013
 *
 */
...
public class MatMultWithNproc2DarrayMain
{
						 ...
						 InterruptedException
  {
    ...
    if (ntasks != P)
    {
      ...
      {
	String className =
	  new MatMultWithNproc2DarrayMain().getClass().getName();
	System.err.printf ("\nI need %d processes.\n" +
			   "Usage:\n" +
			   "  mpiexec -np %d java %s\n\n",
			   P, P, className);
