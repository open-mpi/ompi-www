...
?>
<!-- received="Mon Nov 11 18:38:27 2013" -->
<!-- isoreceived="20131111233827" -->
<!-- sent="Mon, 11 Nov 2013 23:38:23 +0000" -->
<!-- isosent="20131111233823" -->
<!-- name="Jeff Squyres (jsquyres)" -->
<!-- email="jsquyres_at_[hidden]" -->
<!-- subject="Re: [OMPI users] Prototypes for Fortran MPI_ commands using 64-bit	indexing" -->
<!-- id="05CA88E3-B338-4659-B6D4-50812E6C298E_at_cisco.com" -->
<!-- charset="iso-8859-1" -->
<!-- inreplyto="CAEGKUywKTK6P=e2cdhJtAckG8zx3LC4MFgxURqG9AKmp3YQsOw_at_mail.gmail.com" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2013-11-11 18:38:23
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="22945.php">George Bosilca: "Re: [OMPI users] Prototypes for Fortran MPI_ commands using 64-bit indexing"</a>
<li><strong>Previous message:</strong> <a href="22943.php">Jeff Squyres (jsquyres): "Re: [OMPI users] MPI_File_write hangs on NFS-mounted filesystem"</a>
<li><strong>In reply to:</strong> <a href="22895.php">Jim Parker: "Re: [OMPI users] Prototypes for Fortran MPI_ commands using 64-bit indexing"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="22945.php">George Bosilca: "Re: [OMPI users] Prototypes for Fortran MPI_ commands using 64-bit indexing"</a>
<li><strong>Reply:</strong> <a href="22945.php">George Bosilca: "Re: [OMPI users] Prototypes for Fortran MPI_ commands using 64-bit indexing"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
Jim --
<br>
<p>This has been bugging me for a while.  I finally got to check today: it looks like we compute MPI_STATUS_SIZE correctly in both the 32 and 64 bit cases.
<br>
<p>That is, MPI_STATUS_SIZE exactly reflects the size of the C MPI_Status (4 int's and a size_t, or 4 * 4 + 8 = 24 bytes), regardless of whether Fortran is 32 or 64 bits.
<br>
<p>There are two implications here:
<br>
<p>1. OMPI should not be overwriting your status array if you're using MPI_STATUS_SIZE as its length.  Meaning: in the 32 bit case, MPI_STATUS_SIZE=6, and in the 64 bit case, MPI_STATUS_SIZE=3.
<br>
<p>2. In the 64 bit case, you'll have a difficult time extracting the MPI status values from the 8-byte INTEGERs in the status array in Fortran (because the first 2 of 3 each really be 2 4-byte integers).
<br>
<p>So while #2 is a little weird (and probably should be fixed), a properly-sized MPI_STATUS_SIZE array shouldn't be causing any problems.  So I'm still a little befuddled as to why you're seeing an error.  :-\
<br>
<p><p><p><p>On Nov 1, 2013, at 4:51 PM, Jim Parker &lt;jimparker96313_at_[hidden]&gt; wrote:
<br>
<p><span class="quotelev1">&gt; @Jeff,
</span><br>
<span class="quotelev1">&gt;  
</span><br>
<span class="quotelev1">&gt;  Well, it may have been &quot;just for giggles&quot;, but it Worked!! My helloWorld program ran as expected.  My original code ran through the initialization parts without overwriting data.  It will take a few days to finish computation and analysis to ensure it ran as expected.  I'll report back when I get done.
</span><br>
<span class="quotelev1">&gt;  
</span><br>
<span class="quotelev1">&gt; It looks like a good Friday!
</span><br>
<span class="quotelev1">&gt;  
</span><br>
...
<span class="quotelev1">&gt; On Thu, Oct 31, 2013 at 4:06 PM, Jeff Squyres (jsquyres) &lt;jsquyres_at_[hidden]&gt; wrote:
</span><br>
<span class="quotelev1">&gt; For giggles, try using MPI_STATUS_IGNORE (assuming you don't need to look at the status at all).  See if that works for you.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; Meaning: I wonder if we're computing the status size for Fortran incorrectly in the -i8 case...
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; 
...
<span class="quotelev2">&gt; &gt; Some additional info that may jog some solutions.  Calls to MPI_SEND do not cause memory corruption.  Only calls to MPI_RECV.  Since the main difference is the fact that MPI_RECV needs a &quot;status&quot; array and SEND does not, seems to indicate to me that something is wrong with status.
</span><br>
<span class="quotelev2">&gt; &gt;
</span><br>
<span class="quotelev2">&gt; &gt; Also, I can run a C version of the helloWorld program with no errors.  However, int types are only 4-byte.  To send 8byte integers, I define tempInt as long int and pass MPI_LONG as a type.
</span><br>
<span class="quotelev2">&gt; &gt;
</span><br>
...
<span class="quotelev2">&gt; &gt; On Wed, Oct 30, 2013 at 10:55 PM, Jim Parker &lt;jimparker96313_at_[hidden]&gt; wrote:
</span><br>
...
<span class="quotelev2">&gt; &gt; You need to set FC to either ifort or gfortran (those are the two compilers
</span><br>
...
<span class="quotelev2">&gt; &gt; must contain libtorque.so), if you are running jobs under torque; otherwise
</span><br>
...
<span class="quotelev2">&gt; &gt; (I don't have experience with that because we install hwloc independently).
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt;    8. ifconfig output from head node and computes listed as *-ifconfig.out
</span><br>
...
<span class="quotelev4">&gt; &gt; &gt;    &gt; I have compiled with -m64 -fdefault-integer-8 and even have assigned
</span><br>
<span class="quotelev3">&gt; &gt; &gt;    kind=8 to the integers (which would be the preferred method in my case)
</span><br>
<span class="quotelev4">&gt; &gt; &gt;    &gt;
</span><br>
...
<span class="quotelev4">&gt; &gt; &gt;    &gt; On Oct 30, 2013, at 4:35 PM, Jim Parker &lt;[5]jimparker96313_at_[hidden]&gt;
</span><br>
<span class="quotelev3">&gt; &gt; &gt;    wrote:
</span><br>
<span class="quotelev4">&gt; &gt; &gt;    &gt;
...
<span class="quotelev3">&gt; &gt; &gt;    [6]<a href="http://wiki.chem.vu.nl/dirac/index.php/How_to_build_MPI_libraries_fo">http://wiki.chem.vu.nl/dirac/index.php/How_to_build_MPI_libraries_fo</a>
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt;    correct in that all the MPI parameters are INTEGERs -- but you can tell
</span><br>
...
<span class="quotelev1">&gt; &gt; &gt;    &gt; &gt; I'm curious because some off my local variables get killed (set to
</span><br>
<span class="quotelev3">&gt; &gt; &gt;    null) upon my first call to MPI_RECV.  Typically, this is due (in
</span><br>
<span class="quotelev3">&gt; &gt; &gt;    Fortran) to someone not setting the 'status' variable to an appropriate
</span><br>
...
<span class="quotelev4">&gt; &gt; &gt;    &gt; If you *did* compile your application with -i8 and you're seeing this
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt;    during the compile makes the library support 64-bit indexing.  ie. int
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt;      &lt;mpi-test-64bit.tar.bz2&gt;____________________________________________
</span><br>
...
<span class="quotelev3">&gt; &gt; &gt;    6. <a href="http://wiki.chem.vu.nl/dirac/index.php/How_to_build_MPI_libraries_for_64-bit_integers">http://wiki.chem.vu.nl/dirac/index.php/How_to_build_MPI_libraries_for_64-bit_integers</a>
</span><br>
...
<span class="quotelev2">&gt; &gt; &lt;openmpi-1.6.5.config.tar.gz&gt;_______________________________________________
</span><br>
<span class="quotelev2">&gt; &gt; users mailing list
</span><br>
<span class="quotelev2">&gt; &gt; users_at_[hidden]
