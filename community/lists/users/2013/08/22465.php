...
?>
<!-- received="Sat Aug 10 15:39:33 2013" -->
<!-- isoreceived="20130810193933" -->
<!-- sent="Sat, 10 Aug 2013 15:39:22 -0400" -->
<!-- isosent="20130810193922" -->
<!-- name="Gustavo Correa" -->
<!-- email="gus_at_[hidden]" -->
<!-- subject="Re: [OMPI users] Error launching single-node tasks from	multiple-node job." -->
<!-- id="C0B54212-DD20-4393-A1C8-88D504EAD81E_at_ldeo.columbia.edu" -->
<!-- charset="us-ascii" -->
<!-- inreplyto="020301ce95fd$ed8453e0$c88cfba0$_at_stanford.edu" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2013-08-10 15:39:22
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="22466.php">Lee-Ping Wang: "Re: [OMPI users] Error launching single-node tasks	from	multiple-node job."</a>
<li><strong>Previous message:</strong> <a href="22464.php">Gustavo Correa: "Re: [OMPI users] Error launching single-node tasks from multiple-node job."</a>
<li><strong>In reply to:</strong> <a href="22463.php">Lee-Ping Wang: "Re: [OMPI users] Error launching single-node tasks from	multiple-node job."</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="22466.php">Lee-Ping Wang: "Re: [OMPI users] Error launching single-node tasks	from	multiple-node job."</a>
<li><strong>Reply:</strong> <a href="22466.php">Lee-Ping Wang: "Re: [OMPI users] Error launching single-node tasks	from	multiple-node job."</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<span class="quotelev1">&gt; Thank you for your reply.  I want to run MPI jobs inside a single node, but
</span><br>
<span class="quotelev1">&gt; due to the resource allocation policies on the clusters, I could get many
</span><br>
<span class="quotelev1">&gt; more resources if I submit multiple-node &quot;batch jobs&quot;.  Once I have a
</span><br>
<span class="quotelev1">&gt; multiple-node batch job, then I can use a command like &quot;pbsdsh&quot; to run
</span><br>
<span class="quotelev1">&gt; single node MPI jobs on each node that is allocated to me.  Thus, the MPI
</span><br>
<span class="quotelev1">&gt; jobs on each node are running independently of each other and unaware of one
</span><br>
<span class="quotelev1">&gt; another.
</span><br>
<p>Even if you use pbdsh to launch separate MPI jobs on individual nodes,
<br>
...
<br>
<p>Still quite confused because you didn't tell how your &quot;qsub&quot; command looks like,
<br>
...
<span class="quotelev1">&gt; jobs are failing immediately, then I only have a small window to view the
</span><br>
<span class="quotelev1">&gt; actual command through &quot;ps&quot; or something.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<br>
Note that the PBS_NODEFILE has one line per-node-per-core, *not* one line per node.
<br>
I have no idea how the trick above could be reconciled with the Q-Chem scripts, though.
<br>
<p>Overall, I don't understand why you would benefit from such a complicated scheme,
<br>
rather than lauching either a big MPI job across all nodes that you requested (if the problem
<br>
is large enough to benefit from  this many cores), 
<br>
or launch several small single-node jobs (if the problem is small enough to fit well a single node).
<br>
...
<br>
We run tons of parallel single-node jobs here, for problems that fit well a single node.
<br>
<p><p>My two cents
<br>
Gus Correa
...
<span class="quotelev1">&gt; &quot;That is to say, these tasks are intended to use OpenMPI parallelism on each
</span><br>
...
<span class="quotelev1">&gt; &quot;Since my jobs are only parallel over the node they're running on, I believe
</span><br>
...
<span class="quotelev1">&gt; command will look for the $PBS_NODEFILE to learn in which node(s) it should
</span><br>
...
<span class="quotelev1">&gt; that Q-Chem allows some level of standard mpiexec command options, you could
</span><br>
...
<span class="quotelev1">&gt; mpiexec -hostfile $PBS_NODEFILE -np 16 ./my-Q-chem-executable &lt;parameters to
</span><br>
...
<span class="quotelev1">&gt; multiple node job to the Torque batch system, and the job executes multiple
</span><br>
<span class="quotelev1">&gt; single-node parallel tasks.  That is to say, these tasks are intended to use
</span><br>
...
<span class="quotelev1">&gt; and this is the last known version of OpenMPI that this version of Q-Chem is
</span><br>
...
<span class="quotelev1">&gt; archives (<a href="http://www.open-mpi.org/community/lists/users/2010/03/12348.php">http://www.open-mpi.org/community/lists/users/2010/03/12348.php</a>),
</span><br>
...
<span class="quotelev1">&gt; multi-node job compared to a single-node job?  How does OpenMPI detect that
</span><br>
