...
?>
<!-- received="Mon Oct  5 13:01:54 2015" -->
<!-- isoreceived="20151005170154" -->
<!-- sent="Mon, 5 Oct 2015 10:01:45 -0700" -->
<!-- isosent="20151005170145" -->
<!-- name="Ralph Castain" -->
<!-- email="rhc_at_[hidden]" -->
<!-- subject="Re: [OMPI users] Process binding with SLURM and 'heterogeneous' nodes" -->
<!-- id="18042056-4A5F-4EC8-A161-AE68A4379F89_at_open-mpi.org" -->
...
<strong>Date:</strong> 2015-10-05 13:01:45
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="27801.php">Dave Love: "Re: [OMPI users] worse latency in 1.8 c.f. 1.6"</a>
<li><strong>Previous message:</strong> <a href="27799.php">marcin.krotkiewski: "Re: [OMPI users] Process binding with SLURM and 'heterogeneous' nodes"</a>
...
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="27807.php">Jeff Squyres (jsquyres): "Re: [OMPI users] Process binding with SLURM and 'heterogeneous'	nodes"</a>
<li><strong>Reply:</strong> <a href="27807.php">Jeff Squyres (jsquyres): "Re: [OMPI users] Process binding with SLURM and 'heterogeneous'	nodes"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<p>1. the warning needs to be emitted regardless of whether or not &#151;report-bindings was given. Not sure how that warning got &#147;covered&#148; by the option, but it is clearly a bug
<br>
<p>2. improve the warning to include binding info - relatively easy to do
<br>
<p>3. fix the mapping/binding under asymmetric topologies. Given further info and consideration, I&#146;m increasingly pushed towards the &#147;fallback to the map-by core default&#148; solution. It provides a predictable and consistent pattern. The other solution is technically viable, but leads to an unpredictable &#147;opportunistic&#148; result that might cause odd application behavior. If the user specifies a mapping option and we can&#146;t do it because of asymmetry, then error out.
<br>
<p>HTH
<br>
Ralph
<br>
<p><p><span class="quotelev1">&gt; On Oct 5, 2015, at 9:36 AM, marcin.krotkiewski &lt;marcin.krotkiewski_at_[hidden]&gt; wrote:
</span><br>
...
<span class="quotelev2">&gt;&gt; srun -N $SLURM_NNODES -n $SLURM_NNODES --cpu_bind=none -l grep Cpus_allowed_list /proc/self/status
</span><br>
...
<span class="quotelev1">&gt; mpirun --mca rmaps_base_verbose 10 --hetero-nodes --bind-to core --report-bindings --map-by socket -np 32 ./affinity
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; I have attached two output files: one for the original 1.10.1rc1, one for the patched version.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; When I said 'failed in one case' I was not precise. I got an error on node c1-8, which was the first one to have different number of MPI processes on the two sockets. It would also fail on some later nodes, just that because of the error we never got there.
</span><br>
<span class="quotelev1">&gt; 
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; called in rmaps_base_binding.c / bind_downwards being wrong. I do not know the reason, but I think I know when the problem happens (at least on 1.10.1rc1). It seems that by default openmpi maps by socket. The error happens when for a given compute node there is a different number of cores used on each socket. Consider previously studied case (the debug outputs I sent in last post). c1-8, which was source of error, has 5 mpi processes assigned, and the cpuset is the following:
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; Cores 0,5 are on socket 0, cores 9, 13, 14 are on socket 1. Binding progresses correctly up to and including core 13 (see end of file out.1.10.1rc2, before the error). That is 2 cores on socket 0, and 2 cores on socket 1. Error is thrown when core 14 should be bound - extra core on socket 1 with no corresponding core on socket 0. At that point the returned trg_obj points to the first core on the node (os_index 0, socket 0).
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; I have submitted a few other jobs and I always had an error in such situation. Moreover, if I now use --map-by core instead of socket, the error is gone, and I get my expected binding:
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev3">&gt;&gt;&gt; Using --map-by core seems to fix the issue on 1.8.8, 1.10.0 and 1.10.1rc1. However, there is still a difference in behavior between 1.10.1rc1 and earlier versions. In the SLURM job described in last post, 1.10.1rc1 fails to bind only in 1 case, while the earlier versions fail in 21 out of 32 cases. You mentioned there was a bug in hwloc. Not sure if it can explain the difference in behavior.
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; this could be pretty similar to a previous issue when ompi tried to bind to cores outside the cpuset.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; /* when a core has more than one thread, would ompi assume all the threads are available if the core is available ? */
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; On Sunday, October 4, 2015, Ralph Castain &lt; &lt;mailto:rhc_at_[hidden]&gt;rhc_at_[hidden] &lt;mailto:rhc_at_[hidden]&gt;&gt; wrote:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; Thanks - please go ahead and release that allocation as I&#146;m not going to get to this immediately. I&#146;ve got several hot irons in the fire right now, and I&#146;m not sure when I&#146;ll get a chance to track this down.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; Gilles or anyone else who might have time - feel free to take a gander and see if something pops out at you.
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt; Ralph
...
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; On Oct 3, 2015, at 11:05 AM, marcin.krotkiewski &lt; &lt;&gt;marcin.krotkiewski_at_[hidden] &lt;mailto:marcin.krotkiewski_at_[hidden]&gt;&gt; wrote:
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; 
</span><br>
<span class="quotelev1">&gt;&gt;&gt;&gt;&gt; 
...
<span class="quotelev4">&gt;&gt;&gt;&gt;&gt;&gt;&gt;&gt; On Oct 3, 2015, at 8:59 AM, marcin.krotkiewski &lt; &lt;mailto:marcin.krotkiewski_at_[hidden]&gt;marcin.krotkiewski_at_[hidden] &lt;mailto:marcin.krotkiewski_at_[hidden]&gt;&gt; wrote:
</span><br>
<span class="quotelev4">&gt;&gt;&gt;&gt;&gt;&gt;&gt;&gt; 
</span><br>
...
<span class="quotelev4">&gt;&gt;&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2015/10/27790.php">http://www.open-mpi.org/community/lists/users/2015/10/27790.php</a> &lt;<a href="http://www.open-mpi.org/community/lists/users/2015/10/27790.php">http://www.open-mpi.org/community/lists/users/2015/10/27790.php</a>&gt;
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
</span><br>
<span class="quotelev3">&gt;&gt;&gt; 
...
<span class="quotelev2">&gt;&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2015/10/27792.php">http://www.open-mpi.org/community/lists/users/2015/10/27792.php</a> &lt;<a href="http://www.open-mpi.org/community/lists/users/2015/10/27792.php">http://www.open-mpi.org/community/lists/users/2015/10/27792.php</a>&gt;
</span><br>
<span class="quotelev1">&gt; &lt;out.1.10.1rc1-patched&gt;&lt;out.1.10.1rc1-orig&gt;_______________________________________________
</span><br>
...
<span class="quotelev1">&gt; Link to this post: <a href="http://www.open-mpi.org/community/lists/users/2015/10/27799.php">http://www.open-mpi.org/community/lists/users/2015/10/27799.php</a> &lt;<a href="http://www.open-mpi.org/community/lists/users/2015/10/27799.php">http://www.open-mpi.org/community/lists/users/2015/10/27799.php</a>&gt;
</span><br>
<p><hr>
<ul>
<li>text/html attachment: <a href="http://www.open-mpi.org/community/lists/users/att-27800/attachment">attachment</a>
</ul>
