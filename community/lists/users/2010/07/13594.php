...
?>
<!-- received="Mon Jul 12 12:22:30 2010" -->
<!-- isoreceived="20100712162230" -->
<!-- sent="Mon, 12 Jul 2010 10:22:21 -0600" -->
<!-- isosent="20100712162221" -->
<!-- name="Ralph Castain" -->
...
<!-- id="086BADEB-3B50-45B4-9412-7669AE3A2AC2_at_open-mpi.org" -->
<!-- charset="us-ascii" -->
...
<strong>Date:</strong> 2010-07-12 12:22:21
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="13595.php">Brian Budge: "Re: [OMPI users] Dynamic process tutorials?"</a>
<li><strong>Previous message:</strong> <a href="13593.php">jody: "Re: [OMPI users] Dynamic process tutorials?"</a>
...
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="13687.php">Ralph Castain: "Re: [OMPI users] MPI process dies with a route error when using dynamic process calls to connect more than 2 clients to a server with InfiniBand"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<br>
<p>I'm afraid your application, as written, isn't going to work because the rendezvous protocol isn't correct. You cannot just write a port to a file and have the other side of a connect/accept read it. The reason for this is that OMPI needs to route its out-of-band communications, and needs some handshake to get that setup. If we don't route those communications, we consume way too many ports on nodes of large machines, and thus cannot run large jobs.
<br>
<p>If you want to do this, you need three things:
<br>
<p>1. you have to run our &quot;ompi-server&quot; program on a node where all MPI processes can reach it. This program serves as the central rendezvous point. See &quot;man ompi-server&quot; for info.
<br>
<p>2. you'll need a patch I provided to some other users that allows singletons to connect to ompi-server without first spawning their own daemon. Otherwise, you get an OMPI daemon (&quot;orted&quot;) started for every one of your clients.
<br>
<p>3. you'll need the patch I'm just completing that allows you to have more than 64 singletons connecting together, otherwise you'll just segfault. Each of your clients looks like a singleton to us because it wasn't started with mpiexec.
<br>
<p>I suspect your test works because (a) TCP interconnects differently than IB and doesn't talk via OOB to do it, and thus you made it further (but would still fail at some point when OOB was required), and (b) you were running fewer than 64 clients.
<br>
<p>HTH
<br>
Ralph
<br>
<p><p><p>On Jun 25, 2010, at 7:23 AM, Philippe wrote:
<br>
