...
?>
<!-- received="Wed Jul 21 10:44:56 2010" -->
<!-- isoreceived="20100721144456" -->
<!-- sent="Wed, 21 Jul 2010 08:44:46 -0600" -->
<!-- isosent="20100721144446" -->
<!-- name="Ralph Castain" -->
...
<!-- id="8515D8DD-248D-46A3-9FD4-2297AAE8AD2E_at_open-mpi.org" -->
<!-- charset="us-ascii" -->
<!-- inreplyto="AANLkTinxfFYoi3e773XDuvoTxrUat1AqQOsercia4W91_at_mail.gmail.com" -->
<!-- expires="-1" -->
<div class="center">
...
<strong>Date:</strong> 2010-07-21 10:44:46
</p>
<ul class="links">
<!-- next="start" -->
<li><strong>Next message:</strong> <a href="13723.php">Bill Rankin: "Re: [OMPI users] Partitioning problem set data"</a>
<li><strong>Previous message:</strong> <a href="13721.php">Jed Brown: "Re: [OMPI users] openmpi v1.5?"</a>
<li><strong>In reply to:</strong> <a href="13719.php">Philippe: "Re: [OMPI users] MPI process dies with a route error when using dynamic process calls to connect more than 2 clients to a server with InfiniBand"</a>
<!-- nextthread="start" -->
<li><strong>Next in thread:</strong> <a href="13743.php">Philippe: "Re: [OMPI users] MPI process dies with a route error when using dynamic process calls to connect more than 2 clients to a server with InfiniBand"</a>
<li><strong>Reply:</strong> <a href="13743.php">Philippe: "Re: [OMPI users] MPI process dies with a route error when using dynamic process calls to connect more than 2 clients to a server with InfiniBand"</a>
<!-- reply="end" -->
</ul>
<hr>
<!-- body="start" -->
...
<p>hmmm....that sounds like a bug in your program. let me know what you find
<br>
<p><span class="quotelev1">&gt; 
</span><br>
<span class="quotelev1">&gt; 
...
<p>We have talked in the past about creating a non-blocking MPI_Init as an extension to the standard. It would lock you to Open MPI, though...
<br>
<p>Regardless, at some point you would have to know how many processes are going to be part of the job so you can know when MPI_Init is complete. I would think you would require that info for the singleton wireup anyway - yes? Otherwise, how would you know when to quit running connect-accept?
<br>
<p><span class="quotelev1">&gt; 
</span><br>
...
<p>No real documentation - a lack I should probably fill. At the moment, we don't have a &quot;generic&quot; module for standalone launch, but I can create one as it is pretty trivial. I would then need you to pass each process envars telling it the total number of processes in the MPI job, its rank within that job, and a file where some rendezvous process (can be rank=0) has provided that port string. Armed with that info, I can wireup the job.
<br>
<p>Won't be as scalable as an mpirun-initiated startup, but will be much better than doing it from singletons.
<br>
<p>Or if you prefer, we could setup an &quot;infosphere&quot; module that we could customize for this system. Main thing here would be to provide us with some kind of regex (or access to a file containing the info) that describes the map of rank to node so we can construct the wireup communication pattern.
<br>
<p>Either way would work. The second is more scalable, but I don't know if you have (or can construct) the map info.
<br>
<p><span class="quotelev1">&gt; 
</span><br>
