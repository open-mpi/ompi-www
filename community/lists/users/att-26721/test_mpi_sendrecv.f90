program test
  ...
  integer :: ierr, info, datatype, nsize, window, iproc, nproc, size_of_double, i, tag, req
  ...
  write(*,'(3(a,i0))') 'Task ', iproc, ' post a non-blocking receive for ', nsize, ' elements from task ', modulo(iproc+1,nproc)
  call mpi_irecv(receivebuffer, nsize, mpi_double_precision, modulo(iproc+1,nproc), tag, &
       mpi_comm_world, req)
