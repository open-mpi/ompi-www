...
include_once("$topdir/software/hwloc/v1.0/version.inc");
$title = "Portable Hardware Locality (hwloc): Version $ver_v1_0";
...
if (isset($t_stable)) {
    ...
    $names = array("hwloc-$ver_v1_0.tar.gz", "hwloc-$ver_v1_0.tar.bz2",
                   "hwloc-win32-build-$ver_v1_0.zip");
...
if (preg_match("/[a-z]/i", $v)) {
    ...
    foreach ($names as $key => $n) {
        if (file_exists("downloads/$n")) {
            $t_prerelease->addFile($src, $n, $md5[$n], $sha1[$n]);
            $found = 1;
        }
    }
    if (!$found) {
...
if (isset($t_older)) {
    $versions = array("1.0.2", "1.0.1", "1.0");
...
<P><?php print("<a href=\"https://raw.github.com/open-mpi/hwloc/$ver_v1_0_dir/NEWS\">"); ?>This
file</a> contains a list of changes between the releases in the hwloc
releases in the <?php print($ver_v1_0_dir); ?> series</p>
