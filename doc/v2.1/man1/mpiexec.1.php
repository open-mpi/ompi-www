<?php
$topdir = "../../..";
$title = "mpiexec(1) man page (version 2.1.0)";
$meta_desc = "Open MPI v2.1.0 man page: mpiexec(1)";
...
<p>
Use
one of the following options to specify which hosts (nodes) of the cluster
...
and assigns MPI_COMM_WORLD ranks in a round-robin, "by node" manner.
</dd>
</dl>
<p>
...
<dd>Provide all output in XML format to
the specified file.   </dd>

<dt><b>-xterm, --xterm &lt;ranks&gt;</b> </dt>
<dd>Display the output from the processes
identified by their MPI_COMM_WORLD ranks in separate xterm windows. The
ranks are specified as a comma-separated list of ranges, with a -1 indicating
all. A separate window will be created for each specified process. <b>Note:</b>
xterm will normally terminate the window upon termination of the process
running within it. However, by adding a "!" to the end of the list of specified
ranks, the proper options will be provided to ensure that xterm keeps the
window open <i>after</i> the process terminates, thus allowing you to see the
process&rsquo; output. Each xterm window will subsequently need to be manually
closed. <b>Note:</b> In some environments, xterm may require that the executable
be in the user&rsquo;s path, or be specified in absolute or relative terms. Thus,
it may be necessary to specify a local executable as "./foo" instead of
just "foo". If xterm fails to find the executable, mpirun will hang, but
still respond correctly to a ctrl-c. If this happens, please check that the
executable is being specified correctly and try again.     </dd>
</dl>
<p>
To manage files
...
<dd>&lt;path&gt; that will be used when
attempting to locate the requested executables.  This is used prior to using
the local PATH setting.   </dd>

<dt><b>--prefix &lt;dir&gt;</b> </dt>
<dd>Prefix directory that will be used
to set the <i>PATH</i> and <i>LD_LIBRARY_PATH</i> on the remote node before invoking
...
<dd>Invoke the user-level debugger indicated by the
<i>orte_base_user_debugger</i> MCA parameter.   </dd>

<dt><b>--get-stack-traces</b> </dt>
<dd>When paired with
the <b>--timeout</b> option, <i>mpirun</i> will obtain and print out stack traces from
all launched processes that are still alive when the timeout expires.  Note
that obtaining stack traces can take a little time and produce a lot of
output, especially for large process-count jobs.   </dd>

<dt><b>-debugger, --debugger &lt;args&gt;</b>
</dt>
...
<dd>Allow <i>mpirun</i>
to run when executed by the root user (<i>mpirun</i> defaults to aborting when
launched as the root user).   </dd>

<dt><b>--app &lt;appfile&gt;</b> </dt>
<dd>Provide an appfile, ignoring all
other command line options.   </dd>
...
<dd>Disable recovery (resets all recovery
options to off).   </dd>

<dt><b>-do-not-launch, --do-not-launch</b> </dt>
<dd>Perform all necessary operations
to prepare to launch the application, but do not actually launch it.   </dd>

<dt><b>-do-not-resolve,
--do-not-resolve</b> </dt>
<dd>Do not attempt to resolve interfaces.   </dd>

<dt><b>-enable-recovery, --enable-recovery</b>
</dt>
<dd>Enable recovery from process failure [Default = disabled].   </dd>

<dt><b>-index-argv-by-rank,
--index-argv-by-rank</b> </dt>
<dd>Uniquely index argv[0] for each process using its rank.
  </dd>

<dt><b>-leave-session-attached, --leave-session-attached</b> </dt>
<dd>Do not detach OmpiRTE daemons
used by this application. This allows error messages from the daemons as
well as the underlying environment (e.g., when failing to launch a daemon)
to be output.   </dd>

<dt><b>-max-restarts, --max-restarts &lt;num&gt;</b> </dt>
<dd>Max number of times to restart
a failed process.   </dd>

<dt><b>-ompi-server, --ompi-server &lt;uri or file&gt;</b> </dt>
<dd>Specify the URI of
the Open MPI server (or the mpirun to be used as the server), the name
of the file (specified as file:filename) that contains that info, or the
PID (specified as pid:#) of the mpirun to be used as the server. The Open
MPI server is used to support multi-application data exchange via the MPI-2
<a href="../man3/MPI_Publish_name.3.php">MPI_Publish_name</a> and <a href="../man3/MPI_Lookup_name.3.php">MPI_Lookup_name</a> functions.   </dd>

<dt><b>-personality, --personality
&lt;list&gt;</b> </dt>
<dd>Comma-separated list of programming model, languages, and containers
being used (default="ompi").   </dd>

<dt><b>--ppr &lt;list&gt;</b> </dt>
<dd>Comma-separated list of number of
processes on a given resource type [default: none].   </dd>

<dt><b>-report-child-jobs-separately,
--report-child-jobs-separately</b> </dt>
<dd>Return the exit status of the primary job only.
  </dd>

<dt><b>-report-events, --report-events &lt;URI&gt;</b> </dt>
<dd>Report events to a tool listening at the
specified URI.   </dd>

<dt><b>-report-pid, --report-pid &lt;channel&gt;</b> </dt>
<dd>Print out mpirun&rsquo;s PID during
startup. The channel must be either a &rsquo;-&rsquo; to indicate  that the pid is to be
output to stdout, a &rsquo;+&rsquo; to indicate that the pid is to be  output to stderr,
or a filename to which the pid is to be written.   </dd>

<dt><b>-report-uri, --report-uri
&lt;channel&gt;</b> </dt>
<dd>Print out mpirun&rsquo;s URI during startup. The channel must be either
a &rsquo;-&rsquo; to indicate  that the URI is to be output to stdout, a &rsquo;+&rsquo; to indicate
that the URI is to be  output to stderr, or a filename to which the URI
is to be written.   </dd>

<dt><b>-show-progress, --show-progress</b> </dt>
<dd>Output a brief periodic report
on launch progress.   </dd>

<dt><b>-use-hwthread-cpus, --use-hwthread-cpus</b> </dt>
<dd>Use hardware threads
as independent cpus.   </dd>

<dt><b>-use-regexp, --use-regexp</b> </dt>
<dd>Use regular expressions for
launch.     </dd>
</dl>
<p>
The following options are useful for developers; they are not
generally useful to most ORTE and/or MPI users:
<dl>

<dt><b>-d, --debug-devel</b> </dt>
<dd>Enable debugging
of the OmpiRTE (the run-time layer in Open MPI). This is not generally useful
for most users.   </dd>

<dt><b>--debug-daemons</b> </dt>
<dd>Enable debugging of any OmpiRTE daemons used
by this application.   </dd>

<dt><b>--debug-daemons-file</b> </dt>
<dd>Enable debugging of any OmpiRTE
daemons used by this application, storing output in files.   </dd>

<dt><b>-display-devel-allocation,
--display-devel-allocation</b> </dt>
<dd>Display a detailed list of the allocation being
used by this job.   </dd>

<dt><b>-display-devel-map, --display-devel-map</b> </dt>
<dd>Display a more detailed
table showing the mapped location of each process prior to launch.   </dd>

<dt><b>-display-diffable-map,
--display-diffable-map</b> </dt>
<dd>Display a diffable process map just before launch.
</dd>

<dt><b>-display-topo, --display-topo</b> </dt>
<dd>Display the topology as part of the process map
just before launch.   </dd>
...
<dd>When paired with the <b>--timeout</b> command line option,
report the run-time subsystem state of each process when the timeout expires.
  </dd>
</dl>
<p>
...
<dd>The maximum number of seconds that <i>mpirun</i> (<i>mpiexec</i>)<i></i> will
run.  After this many seconds, <i>mpirun</i> will abort the launched job and exit.
     </dd>
</dl>

<h2><a name='sect5' href='#toc5'>Description</a></h2>
 One invocation of <i>mpirun</i> starts an MPI application running
under Open MPI. If the application is single process multiple data (SPMD),
the application can be specified on the <i>mpirun</i> command line.
...
-np).
<h3><a name='sect6' href='#toc6'>Specifying Host Nodes</a></h3>
 Host nodes can be identified on the <i>mpirun</i>
command line with the <i>-host</i> option or in a hostfile.  <p>
For example,
<dl>
...
<h3><a name='sect7' href='#toc7'>Specifying
