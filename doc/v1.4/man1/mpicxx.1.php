<?php
$topdir = "../../..";
$title = "mpicxx(1) man page (version 1.4.5)";
$meta_desc = "Open MPI v1.4.5 man page: mpicxx(1)";
...
<dt>-showme  </dt>
<dd>Do not invoke the underlying compiler.  Instead, show the
command line that would be executed to compile the program.  <b>NOTE:</b> If a
non-filename argument is passed on the command line, the <i>-showme</i> option will
<i>not</i> display any additional flags.  For example, both "mpicxx --showme" and
"mpicxx --showme my_source.c" will show all the wrapper-supplied flags.  But
"mpicxx -showme -v" will only show the underlying compiler name and "-v". </dd>

<dt>-showme:compile
</dt>
<dd>Do not invoke the underlying C++ compiler.  Instead, show the compiler flags
that would be supplied to the C++ compiler. </dd>

<dt>-showme:link </dt>
<dd>Do not invoke the
underlying C++ compiler.  Instead, show the linker flags that would be supplied
to the C++ compiler. </dd>
</dl>
<p>
...
<p>
Conceptually, the
role of these commands is quite simple: transparently add relevant compiler
and linker flags to the user&rsquo;s command line that are necessary to compile
...
(Open MPI). There are wrapper compilers for each layer; each layer&rsquo;s wrapper
...
<dt>OMPI </dt>
<dd><i>mpicc</i>, <i>mpic++</i>, <i>mpicxx</i>, <i>mpiCC</i> (only on systems
with case-senstive file systems), <i>mpif77</i>, and <i>mpif90</i>.  Note that <i>mpic++</i>,
<i>mpicxx</i>, and <i>mpiCC</i> all invoke the same underlying C++ compiler with the
same options.  All are provided as compatibility with other MPI implementations.
 </dd>
</dl>
<p>
The Fortran wrapper compilers for MPI (<i>mpif77</i> and <i>mpif90</i>) will be inoperative
...
<i>mpicxx</i> is a convenience wrappers
...
of the standard search directories of <i>ld(1)</i>.  It also often requires the
...
the -I, -L and -l options required by Open MPI programs.  <p>
The Open MPI Team
<i>strongly</i> encourages using the wrapper compilers instead of attempting to
...
use <i>mpicxx</i>.  When it is not possible to use the wrappers directly, the <i>-showme:compile</i>
...
by Open MPI and installed to <i>$pkgdata/mpicxx-wrapper-data.txt</i>, where <i>$pkgdata</i>
