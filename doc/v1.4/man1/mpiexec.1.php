<?php
$topdir = "../../..";
$title = "mpiexec(1) man page (version 1.4.5)";
$meta_desc = "Open MPI v1.4.5 man page: mpiexec(1)";
...
window will be created for each specified rank. <b>Note:</b> In some environments,
...
<dd>Copy the specified executable(s) to remote
...
<dd>Preload the comma separated list of files
to the current working directory of the remote machines where processes
will be launched prior to starting those processes.   </dd>

<dt><b>--preload-files-dest-dir
&lt;path&gt;</b> </dt>
<dd>The destination directory to be used for preload-files, if other than
the current working directory. By default, the absolute and relative paths
provided by --preload-files are used.   </dd>

<dt><b>--tmpdir &lt;dir&gt;</b> </dt>
<dd>Set the root for the session
directory tree for mpirun only.   </dd>

<dt><b>-wd &lt;dir&gt;</b> </dt>
<dd>Synonym for <i>-wdir</i>.   </dd>

<dt><b>-wdir &lt;dir&gt;</b> </dt>
<dd>Change
to the directory &lt;dir&gt; before the user&rsquo;s program executes. See the "Current
Working Directory" section for notes on relative paths. <b>Note:</b> If the <i>-wdir</i>
option appears both on the command line and in an application context,
the context will take precedence over the command line.   </dd>

<dt><b>-x &lt;env&gt;</b> </dt>
<dd>Export the
specified environment variables to the remote nodes before executing the
program.  Only one environment variable can be specified per <i>-x</i> option.  Existing
environment variables can be specified or new variable names specified
with corresponding values.  For example:     <b>%</b> mpirun -x DISPLAY -x OFILE=/tmp/out
...<br>
...
&lt;uri or file&gt;</b> </dt>
<dd>Specify the URI of the Open MPI server, or the name of the
file (specified as file:filename) that contains that info. The Open MPI
server is used to support multi-application data exchange via the MPI-2 <a href="../man3/MPI_Publish_name.3.php">MPI_Publish_name</a>
and <a href="../man3/MPI_Lookup_name.3.php">MPI_Lookup_name</a> functions.   </dd>

<dt><b>-wait-for-server, --wait-for-server</b> </dt>
<dd>Pause mpirun
...
by an <i>mpirun</i> command that wishes to connect to it. Mpirun will pause until
either the specified ompi-server is contacted or the server-wait-time is exceeded.
  </dd>

<dt><b>-server-wait-time, --server-wait-time &lt;secs&gt;</b> </dt>
<dd>The max amount of time (in seconds)
...
<h3><a name='sect8' href='#toc8'>Mapping Processes to Nodes:  Using Arbitrary Mappings</a></h3>
...
by line, assigning processes to nodes in whatever order the hostfile specifies.
 Use the <i>-mca rmaps seq</i> option.  For example, using the same hostfile as
before
<dl>

<dt>mpirun -hostfile myhostfile ./a.out </dt>
<dd>will launch three processes, on
ranks aa, bb, and cc, respectively. The slot counts don&rsquo;t matter;  one process
is launched per line on whatever node is listed on the line.  </dd>
</dl>
<p>
Another way
...
<h3><a name='sect9' href='#toc9'>Process
Binding</a></h3>
...
forms, <i>mpirun</i> looks on the command line for <i>--app</i> option.  If it is specified,
...
specified by the <i>--path</i> option.  If there is no <i>--path</i> option set or if the
file is not found at the <i>--path</i> location, then Open MPI will search the user&rsquo;s
PATH environment variable as defined on the source node(s). <p>
If a relative
...
invocation of <i>mpirun</i>.
<h3><a name='sect13' href='#toc13'>Current Working Directory</a></h3>
 The <i>-wdir</i> mpirun option
(and its synonym, <i>-wd</i>) allows the user to change to an arbitrary directory
...
<h3><a name='sect16' href='#toc16'>Process
...
<p>
Note that <i>--prefix</i> can be set on a per-context basis, allowing for different
values for different nodes. <p>
The <i>--prefix</i> option is not sufficient if the installation
...
equivalent to specifying <i>--prefix</i> without the last subdirectory in the absolute
pathname to <i>mpirun</i>.  For example:
<p>     <b>%</b> /usr/local/bin/mpirun ...<br>
...
<dt>mpirun -np 4 -mca btl ib,tcp,self
prog1 </dt>
<dd>Run 4 copies of prog1 using the "ib", "tcp", and "self" BTL&rsquo;s for
the transport of MPI messages.   </dd>
...
<li><a name='toc8' href='#sect8'>Mapping Processes to Nodes:  Using Arbitrary Mappings</a></li>
<li><a name='toc9' href='#sect9'>Process Binding</a></li>
