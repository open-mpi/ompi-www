<?php
$topdir = "../../..";
$title = "MPI_File_read_all_end(3) man page (version 1.4.5)";
$meta_desc = "Open MPI v1.4.5 man page: MPI_FILE_READ_ALL_END(3)";
...
<b>MPI_File_read_all_end</b> - Reads a file starting at the locations specified
...
is the ending part of a split collective operation that stores the number
of elements actually read from the file associated with  <i>fh</i> (at the current
individual file pointer position maintained by the system)  into the user&rsquo;s
buffer  <i>buf</i> in  <i>status.</i> The data is taken out of those parts of the file
specified by the current view. All other fields of  <i>status</i> are undefined.

<p>
<h2><a name='sect7' href='#toc7'>Notes</a></h2>
All the nonblocking collective routines for data access are "split"
into two routines, each with _begin or _end as a suffix. These split collective
routines are subject to the semantic rules described in Section 9.4.5 of
the MPI-2 standard.
<p>
