<?php
$topdir = "../../..";
$title = "MPI_Put(3) man page (version 1.4.5)";
$meta_desc = "Open MPI v1.4.5 man page: MPI_PUT(3)";
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;MPI::Datatype&amp; origin_datatype, int target_rank, MPI::Aint
<tt> </tt>&nbsp;<tt> </tt>&nbsp;target_disp, int target_count, const MPI::Datatype&amp;
<tt> </tt>&nbsp;<tt> </tt>&nbsp;target_datatype) const
</pre>
...
MPI_Put
transfers <i>origin_count</i> successive entries of the type specified by <i>origin_datatype</i>,
starting at address <i>origin_addr</i> on the origin node to the target node specified
by the <i>win</i>, <i>target_rank</i> pair. The data are written in the target buffer
at address <i>target_addr</i> = <i>window_base</i> + <i>target_disp</i> x <i>disp_unit</i>, where <i>window_base</i>
and <i>disp_unit</i> are the base address and window displacement unit specified
at window initialization, by the target process. <p>
The target buffer is specified
by the arguments <i>target_count</i> and <i>target_datatype</i>. <p>
The data transfer is
the same as that which would occur if the origin process executed a send
operation with arguments <i>origin_addr</i>, <i>origin_count</i>, <i>origin_datatype</i>, <i>target_rank</i>,
<i>tag</i>, <i>comm</i>, and the target process executed a receive operation with arguments
<i>target_addr</i>, <i>target_count</i>, <i>target_datatype</i>, <i>source</i>, <i>tag</i>, <i>comm</i>, where <i>target_addr</i>
is the target buffer address computed as explained above, and <i>comm</i> is a
communicator for the group of <i>win</i>. <p>
The communication must satisfy the same
constraints as for a similar message-passing communication. The <i>target_datatype</i>
...
the same buffer can access the target window. <p>
The <i>target_datatype</i> argument
...
The <i>target_datatype</i> argument is
...
Section 2.4 of the MPI-2 Standard). <p>
...
a target window in memory allocated by <a href="../man3/MPI_Alloc_mem.3.php">MPI_Alloc_mem</a> may be much faster
...
<p>
<a href="../man3/MPI_Get.3.php">MPI_Get</a> <br>

<p><a href="../man3/MPI_Accumulate.3.php">MPI_Accumulate</a>
<p> <p>
