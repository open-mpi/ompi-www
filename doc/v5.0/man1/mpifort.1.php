<?php
$topdir = "../../..";
$title = "mpifort(1) man page (version 5.0.0rc1)";
$meta_desc = "Open MPI v5.0.0rc1 man page: mpifort(1)";
...
flags that would have been supplied to the Fortran compiler. </dd>

<dt>--showme:link
</dt>
<dd>Output the linker flags that would have been supplied to the Fortran compiler.
</dd>

<dt>--showme:command </dt>
<dd>Outputs the underlying Fortran compiler command (which may
...
<dd>Outputs a space-delimited (but otherwise undecorated)
...
<dd>Outputs a space-delimited (but otherwise undecorated) list of library names
that the wrapper compiler would have used to link an application.  For example:
"mpi open-rte open-pal util". </dd>

<dt>--showme:version </dt>
<dd>Outputs the version number of
Open MPI. </dd>
...
user&rsquo;s command line that are necessary to compile / link Open MPI programs,
...
and OMPI (Open MPI). There are wrapper compilers for each layer; each layer&rsquo;s
...
<dt>OPAL </dt>
<dd><i>opalcc</i> and <i>opalc++</i>
 </dd>

<dt>OMPI </dt>
<dd><i>mpicc</i>, <i>mpic++</i>, <i>mpicxx</i>, <i>mpiCC</i> (only on systems with case-senstive file
systems), and <i>mpifort</i> (and its legacy/deprecated names <i>mpif77</i> and <i>mpif90</i>).
 Note that <i>mpic++</i>, <i>mpicxx</i>, and <i>mpiCC</i> all invoke the same underlying C++
compiler with the same options.  All are provided as compatibility with
other MPI implementations.   </dd>
</dl>

<h3><a name='sect5' href='#toc5'>Fortran Notes</a></h3>
<p>
The Fortran wrapper compiler for
MPI (<i>mpifort</i>, and its legacy/deprecated names <i>mpif77</i> and <i>mpif90</i>) can compile
and link MPI applications that use any/all of the MPI Fortran bindings:
<i>mpif.h</i>, the <i>mpi</i> module, and the <i>mpi_f08</i> module (assuming Open MPI was installed
...
use <i>mpif.h</i> vs. applications that use the <i>mpi</i> module -- just use <i>mpifort</i> for
all Fortran MPI applications.  <p>
...
is the underlying Fortran compiler, <i>-qfixed</i> may be necessary to compile
fixed-format Fortran source files.  <p>
Finally, note that <i>mpifort</i> will be inoperative
and will return an error on use if Fortran support was not built into the
MP Ilayer.
<h3><a name='sect6' href='#toc6'>Overview</a></h3>
<i>mpifort</i> is a convenience wrappers for the underlying
Fortran compiler.  Translation of an Open MPI program requires the linkage
of the Open MPI-specific libraries which may not reside in one of the standard
...
<i>mpifort</i> passes
its arguments to the underlying Fortran compiler along with the -I, -L and
-l options required by Open MPI programs.  <p>
The Open MPI Team <i>strongly</i> encourages
...
to change without forcing changes to linker directives in users&rsquo; Makefiles.
...
between different installations of the same version of Open MPI.  <p>
Indeed,
since the wrappers are simply thin shells on top of an underlying compiler,
there are very, very few compelling reasons <i>not</i> to use <i>mpifort</i>.  When it
is not possible to use the wrappers directly, the <i>-showme:compile</i> and <i>-showme:link</i>
...
shell$ cc
-c file2.c &lsquo;mpicc -showme:compile&lsquo;  <p>
shell$ cc file1.o file2.o &lsquo;mpicc -showme:link&lsquo;
-o my_mpi_program
...
by Open MPI and installed to <i>$pkgdata/mpifort-wrapper-data.txt</i>, where <i>$pkgdata</i>
