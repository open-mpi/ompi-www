<?php
$topdir = "../../..";
$title = "MPI_File_read_at(3) man page (version 5.0.0rc3)";
$meta_desc = "Open MPI v5.0.0rc3 man page: MPI_FILE_READ_AT(3)";
...
<i>datatype</i> type into the user&rsquo;s buffer <i>buf.</i> The <i>offset</i> is in <i>etype</i> units relative
to the current view. That is, holes are not counted when locating an offset.
The data is taken out of those parts of the file specified by the current
view. MPI_File_read_at stores the number of <i>datatype</i> elements actually read
in <i>status.</i> All other fields of <i>status</i> are undefined. It is erroneous to call
this function if MPI_MODE_SEQUENTIAL mode was specified when the file was
opened.
<p>
<h2><a name='sect8' href='#toc8'>Fortran 77 Notes</a></h2>
The MPI standard prescribes portable Fortran syntax
for the <i>OFFSET</i> argument only for Fortran 90. FORTRAN 77 users may use the
