<?php
$topdir = "../../..";
$title = "MPI_Win_lock_all(3) man page (version 5.0.0rc1)";
$meta_desc = "Open MPI v5.0.0rc1 man page: MPI_WIN_LOCK_ALL(3)";
...
<dd>No other processes
will hold or attempt to acquire a conflicting lock while the caller holds
the window lock. <p>
...
In a client/server environment in which clients connect
to a server and create windows that span both the client and the server,
if a client or server that has obtained a lock on such a window and then
terminates abnormally, the server or other clients may hang in a MPI_Win_lock_all
