<?php
$topdir = "../../..";
$title = "MPI_Comm_spawn(3) man page (version 5.0.0rc2)";
$meta_desc = "Open MPI v5.0.0rc2 man page: MPI_COMM_SPAWN(3)";
...
argument list is terminated by NULL in C and an empty string in Fortran
(note that it is the MPI application&rsquo;s responsibility to ensure that the
last entry of the <i>argv</i> array is an empty string; the compiler will not
automatically insert it). In Fortran, leading and trailing spaces are always
stripped, so that a string consisting of all spaces is considered an empty
string. The constant MPI_ARGV_NULL may be used in C and Fortran to indicate
an empty argument list. In C, this constant is the same as NULL. <p>
In C, the
MPI_Comm_spawn argument <i>argv</i> differs from the <i>argv</i> argument of <i>main</i> in
two respects. First, it is shifted by one element. Specifically, <i>argv</i>[0]
of <i>main</i>  contains the name of the program (given by <i>command</i>). <i>argv</i>[1] of
<i>main</i> corresponds to <i>argv</i>[0] in MPI_Comm_spawn, <i>argv</i>[2] of <i>main</i> to <i>argv</i>[1]
...
<p>
The <i>info</i> argument is an opaque handle of type MPI_Info in C and INTEGER
in Fortran. It is a container for a number of user-specified (<i>key,value</i>)
pairs. <i>key</i> and <i>value</i> are strings (null-terminated char* in C, character*(*)
in Fortran). Routines to create and manipulate the <i>info</i> argument are described
in Section 4.10 of the MPI-2 standard. <p>
For the SPAWN calls, <i>info</i> provides
additional, implementation-dependent instructions to MPI and the runtime
system on how to start processes. An application may pass MPI_INFO_NULL
...
Completion of MPI_Comm_spawn
in the parent does not necessarily mean that <a href="../man3/MPI_Init.3.php">MPI_Init</a> has been called in
