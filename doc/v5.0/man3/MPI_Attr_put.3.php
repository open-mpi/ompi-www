<?php
$topdir = "../../..";
$title = "MPI_Attr_put(3) man page (version 5.0.0rc1)";
$meta_desc = "Open MPI v5.0.0rc1 man page: MPI_ATTR_PUT(3)";
...
have a mpi_f08 binding. <p>
MPI_Attr_put stores the stipulated attribute value
attribute_val for subsequent retrieval by <a href="../man3/MPI_Attr_get.3.php">MPI_Attr_get</a>. If the value is
already present, then the outcome is as if <a href="../man3/MPI_Attr_delete.3.php">MPI_Attr_delete</a> was first called
to delete the previous value (and the callback function delete_fn was executed),
...
value. The call will fail if the delete_fn function returned an error code
other than MPI_SUCCESS.
<p>
<h2><a name='sect7' href='#toc7'>Notes</a></h2>
Values of the permanent attributes MPI_TAG_UB,
MPI_HOST, MPI_IO, and MPI_WTIME_IS_GLOBAL may not be changed. <p>
The type of
the attribute value depends on whether C or Fortran is being used. In C,
an attribute value is a pointer (void *); in Fortran, it is a single integer
(not a pointer, since Fortran has no pointers and there are systems for
which a pointer does not fit in an integer, e.g., any  32-bit address system
that uses 64 bits for Fortran DOUBLE PRECISION). <p>
If an attribute is already
present, the delete function (specified when the corresponding keyval was
