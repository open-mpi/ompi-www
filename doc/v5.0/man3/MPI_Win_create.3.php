<?php
$topdir = "../../..";
$title = "MPI_Win_create(3) man page (version 5.0.0rc3)";
$meta_desc = "Open MPI v5.0.0rc3 man page: MPI_WIN_CREATE(3)";
...
one initiator to one target on the same window memory location are strictly
ordered. If the info key accumulate_ordering is set to <i>none</i>, no ordering
of accumulate operations guaranteed. They key can also be a comma-separated
list of required orderings consisting of <i>rar</i>, <i>war</i>, <i>raw</i>, and <i>waw</i> for read-after-read,
write-after-read, read-after-write, and write-after-write, respectively.  Looser
ordering constraints are likely to result in improved performance. <p>
</dd>

<dt>accumulate_ops
</dt>
<dd>If set to <i>same_op</i>, the implementation will assume that all concurrent accumulate
calls to the same target address will use the same operation.  If set to
<i>same_op_no_op</i>, then the implementation will assume that all concurrent
accumulate calls to the same target address will use the same operation
or MPI_NO_OP.  The default is <i>same_op_no_op</i>. <p>
</dd>

<dt>same_size </dt>
<dd>If set to <i>true</i>, then
the implementation may assume that the argument <i>size</i> is identical on all
processes, and that all processes have provided this info key with the
same value. <p>
</dd>

<dt>same_disp_unit </dt>
<dd>If set to <i>true</i>, then the implementation may assume
that the argument <i>disp_unit</i> is identical on all processes, and that all
processes have provided this info key with the same value. <p>
</dd>
</dl>

<h2><a name='sect8' href='#toc8'>Notes</a></h2>
Common choices
for <i>disp_unit</i> are 1 (no scaling), and (in C syntax) <i>sizeof(type)</i>, for a
window that consists of an array of elements of type <i>type</i>. The later choice
will allow one to use array indices in RMA calls, and have those scaled
correctly to byte displacements, even in a heterogeneous environment. <p>
Use
memory allocated by <a href="../man3/MPI_Alloc_mem.3.php">MPI_Alloc_mem</a> to guarantee properly aligned window
...
for the <i>SIZE</i> argument only for Fortran 90. FORTRAN 77 users may use the
