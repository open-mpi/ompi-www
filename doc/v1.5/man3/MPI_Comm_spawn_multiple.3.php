<?php
$topdir = "../../..";
$title = "MPI_Comm_spawn_multiple(3) man page (version 1.5.5)";
$meta_desc = "Open MPI v1.5.5 man page: MPI_COMM_SPAWN_MULTIPLE(3)";
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;char **array_of_argv[], int array_of_maxprocs[], MPI_Info
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;const char* array_of_commands[], const char** array_of_argv[],
<tt> </tt>&nbsp;<tt> </tt>&nbsp;const int array_of_maxprocs[], const MPI::Info array_of_info[],
<tt> </tt>&nbsp;<tt> </tt>&nbsp;int root, int array_of_errcodes[])
MPI::Intercomm MPI::Intracomm::Spawn_multiple(int count,
<tt> </tt>&nbsp;<tt> </tt>&nbsp;const char* array_of_commands[], const char** array_of_argv[],
<tt> </tt>&nbsp;<tt> </tt>&nbsp;const int array_of_maxprocs[], const MPI::Info array_of_info[],
<tt> </tt>&nbsp;<tt> </tt>&nbsp;int root)
</pre>
...
MPI_Comm_spawn_multiple is identical to <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i>
except that it can specify multiple executables. The first argument, <i>count</i>,
indicates the number of executables. The next three arguments are arrays
of the corresponding arguments in <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i>. The next argument,
<i>array_of_info</i>, is an array of <i>info</i> arguments, one for each executable. See
the INFO ARGUMENTS section for more information. <p>
For the Fortran version
of <i>array_of_argv</i>, the element <i>array_of_argv</i>(i,j) is the jth argument to
command number i.  <p>
...
elements of <i>array_of_argv</i> to MPI_ARGV_NULL is not defined. To specify arguments
for some commands but not others, the commands without arguments should
have a corresponding <i>argv</i> whose first element is null ((char *)0 in C and
empty string in Fortran).  <p>
...
third have ranks m1+m2, m1+m2+1, ..., m1+m2+m3-1, etc. <p>
The <i>array_of_errcodes</i>
argument is 1-dimensional array of size  <p>
<br>
...
The following keys for <i>info</i> are recognized in "Open MPI".
...
others are set to false (or are unset), MPI_ERR_INFO will be returned.
 <br>
 <p>
Note that in "Open MPI", the first array location in <i>array_of_info</i> is
applied to all the commands in <i>array_of_commands</i>.
<p>
<h2><a name='sect9' href='#toc9'>Notes</a></h2>
The argument <i>count</i>
is interpreted by MPI only at the root, as is <i>array_of_argv</i>. Since the leading
dimension of <i>array_of_argv</i> is <i>count</i>, a nonpositive value of <i>count</i> at a
nonroot node could theoretically cause a runtime bounds check error, even
though <i>array_of_argv</i> should be ignored by the subroutine. If this happens,
you should explicitly supply a reasonable value of <i>count</i> on the nonroot
nodes.  <p>
Similar to <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i>, it is the application&rsquo;s responsibility
to terminate each individual set of argv in the <i>array_of_argv</i> argument.
...
array). <p>
Other restrictions apply to the  <i>array_of_argv</i> parameter; see <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i>&rsquo;s
description of the <i>argv</i> parameter for more details. <p>
Calling <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i>
...
instead of calling <i><a href="../man3/MPI_Comm_spawn.3.php">MPI_Comm_spawn</a>(3)</i> several times.
<p>
