<?php
$topdir = "../../..";
$title = "MPI_Unpack_external(3) man page (version 1.8.1)";
$meta_desc = "Open MPI v1.8.1 man page: MPI_UNPACK_EXTERNAL(3)";
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;const void* inbuf, MPI::Aint insize,
<tt> </tt>&nbsp;<tt> </tt>&nbsp;MPI_Aint&amp; position, void *outbuf,
<tt> </tt>&nbsp;<tt> </tt>&nbsp;int outcount) const
</pre>
...
<dt>inbuf       </dt>
<dd>Input
buffer start (choice). </dd>

<dt>insize       </dt>
<dd>Size of input buffer, in bytes (integer).
</dd>
...
by <i>inbuf</i> containing <i>insize</i> bytes. The output buffer can be any communication
buffer allowed in <a href="../man3/MPI_Recv.3.php">MPI_Recv</a>, and is specified by <i>outbuf</i>, <i>outcount</i>, and <i>datatype</i>.
<p>
The input value of <i>position</i> is the first position in <i>inbuf</i> to be read for
unpacking (measured in bytes, not elements, relative to the start of the
buffer). When the function returns, <i>position</i> is incremented by the size
of the packed message, so that it points to the first location in <i>inbuf</i>
...
<a href="../man3/MPI_Recv.3.php">MPI_Recv</a> and MPI_Unpack_external: In <a href="../man3/MPI_Recv.3.php">MPI_Recv</a>, the <i>count</i> argument specifies
the maximum number of items that can be received. In MPI_Unpack_external,
the <i>outcount</i> argument specifies the actual number of items that are to
...
Fortran files or sscanf in C for a similar function.) <p>
Several messages can
be successively packed into one packing unit. This is effected by several
successive related calls to <a href="../man3/MPI_Pack_external.3.php">MPI_Pack_external</a>, where the first call provides
<i>position</i>=0, and each successive call inputs the value of <i>position</i> that
was output by the previous call, along with the same values for <i>outbuf</i>
and <i>outcount</i>. This packing unit now contains the equivalent information
that would have been stored in a message by one send call with a send buffer
that is the "concatenation" of the individual send buffers. <p>
A packing unit
can be sent using type MPI_PACKED. Any point-to-point or collective communication
...
are relaxed for messages sent with type MPI_PACKED. <p>
A packing unit can be
unpacked into several successive messages. This is effected by several successive
related calls to MPI_Unpack_external, where the first call provides <i>position</i>=0,
and each successive call inputs the value of position that was output by
the previous call, and the same values for <i>inbuf</i> and <i>insize</i>. <p>
The concatenation
