<?php
$topdir = "../../..";
$title = "MPI_Bsend(3) man page (version 1.8.1)";
$meta_desc = "Open MPI v1.8.1 man page: MPI_BSEND(3)";
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;                  MPI.Datatype datatype, int dest, int tag)
</pre>
...
<dt>offset </dt>
<dd>Offset
of starting point in buffer (Java-only). </dd>

<dt>count </dt>
<dd>Number of entries in send
buffer (nonnegative integer). </dd>

<dt>datatype </dt>
<dd>Datatype of each send buffer element
(handle). </dd>
...
to send messages without worrying about where they are buffered (because
the user must have provided buffer space with <a href="../man3/MPI_Buffer_attach.3.php">MPI_Buffer_attach</a>). <p>
In deciding
how much buffer space to allocate, remember that the buffer space is not
available for reuse by subsequent <i>MPI_Bsend</i>s unless you are certain that
the message has been received (not just that it should have been received).
