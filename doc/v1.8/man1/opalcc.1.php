<?php
$topdir = "../../..";
$title = "opalcc(1) man page (version 1.8.4)";
$meta_desc = "Open MPI v1.8.4 man page: opalcc(1)";
...
flags.  For example, both "opalcc --showme" and "opalcc --showme my_source.c"
will show all the wrapper-supplied flags.  But "opalcc --showme -v" will only
...
line that are necessary to compile / link Open PAL programs, and then invoke
...
<i>opalcc</i> is a
convenience wrappers for the underlying C compiler.  Translation of an Open
PAL program requires the linkage of the Open PAL-specific libraries which
may not reside in one of the standard search directories of <i>ld(1)</i>.  It also
often requires the inclusion of header files what may also not be found
in a standard location.  <p>
<i>opalcc</i> passes its arguments to the underlying C
compiler along with the -I, -L and -l options required by Open PAL programs.
 <p>
The Open PAL Team <i>strongly</i> encourages using the wrapper compilers instead
of attempting to link to the Open PAL libraries manually.  This allows the
specific implementation of Open PAL to change without forcing changes to
linker directives in users&rsquo; Makefiles.  Indeed, the specific set of flags
and libraries used by the wrapper compilers depends on how Open PAL was
configured and built; the values can change between different installations
of the same version of Open PAL.  <p>
Indeed, since the wrappers are simply
thin shells on top of an underlying compiler, there are very, very few
compelling reasons <i>not</i> to use <i>opalcc</i>.  When it is not possible to use the
...
by Open PAL and installed to <i>$pkgdata/opalcc-wrapper-data.txt</i>, where <i>$pkgdata</i>
is typically <i>$prefix/share/openmpi</i>, and <i>$prefix</i> is the top installation
directory of Open PAL.  <p>
...
the compilers that were selected when Open PAL was configured.  These compilers
...
environment variables of the form "OPAL_value".  <p>
