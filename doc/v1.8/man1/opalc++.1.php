<?php
$topdir = "../../..";
$title = "opalc++(1) man page (version 1.8.4)";
$meta_desc = "Open MPI v1.8.4 man page: opalc++(1)";
...
flags.  For example, both "opalc++ --showme" and "opalc++ --showme my_source.c"
will show all the wrapper-supplied flags.  But "opalc++ --showme -v" will only
...
command line that are necessary to compile / link Open PAL programs, and
then invoke the underlying compiler to actually perform the command.  <p>
As
such, these commands are frequently referred to as "wrapper" compilers
...
Open PAL program requires the linkage of the Open PAL-specific libraries
...
<i>opalc++</i> passes its arguments to the underlying
C++ compiler along with the -I, -L and -l options required by Open PAL programs.
 <p>
...
compelling reasons <i>not</i> to use <i>opalc++</i>.  When it is not possible to use the
...
by Open PAL and installed to <i>$pkgdata/opalc++-wrapper-data.txt</i>, where <i>$pkgdata</i>
