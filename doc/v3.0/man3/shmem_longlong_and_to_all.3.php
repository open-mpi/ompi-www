<?php
$topdir = "../../..";
$title = "shmem_longlong_and_to_all(3) man page (version 3.0.5)";
$meta_desc = "Open MPI v3.0.5 man page: shmem_longlong_and_to_all(3)";
...
<p> <i><i>shmem_int_and_to_all</i>(3)</i>, <i><i>shmem_int4_and_to_all</i>(3)</i>, <i><i>shmem_int8_and_to_all</i>(3)</i>,
<i><i>shmem_long_and_to_all</i>(3)</i>, <i><i>shmem_longlong_and_to_all</i>(3)</i>, <i><i>shmem_short_and_to_all</i>(3)</i>
...
The nreduce argument
determines the number of separate reductions to perform. The source array
on all PEs in the active set provides one element for each reduction. The
results of the reductions are placed in the target array on all PEs in
the active set. The active set is defined by the PE_start, logPE_stride,
PE_size triplet. <p>
The source and target arrays may be the same array, but
they may not be overlapping arrays. As with all SHMEM collective routines,
each of these routines assumes that only PEs in the active set call the
routine. If a PE not in the active set calls a SHMEM collective routine,
...
<dt>target </dt>
<dd>A symmetric
array, of length nreduce elements, to receive the result of the reduction
operations. The data type of target varies with the version of the reduction
routine being called. When calling from C/C++, refer to the SYNOPSIS section
for data type information. When calling from Fortran, the target data types
are as follows: <blockquote></dd>

<dt><b>shmem_int8_and_to_all</b>: Integer, with an element size of
8 bytes </dt>
<dd></dd>

<dt><b>shmem_int4_and_to_all</b>: Integer, with an element size of 4 bytes
</dt>
<dd></dd>
</dl>
</blockquote>
...
<dt>source </dt>
<dd>A symmetric array, of length nreduce elements, that contains one
element for each separate reduction operation. The source argument must
have the same data type as target. </dd>

<dt>nreduce </dt>
<dd>The number of elements in the
target and source arrays. nreduce must be of type integer. If you are using
Fortran, it must be a default integer value. </dd>

<dt>PE_start </dt>
<dd>The lowest virtual
...
<dt>pWrk </dt>
<dd>A symmetric work array. The pWrk argument must
...
max(nreduce/2 + 1, SHMEM_REDUCE_MIN_WRKDATA_SIZE) elements. </dd>

<dt>pSync </dt>
<dd>A symmetric
work array. In C/C++, pSync must be of type long and size _SHMEM_REDUCE_SYNC_SIZE.
...
set enter the reduction routine. </dd>
</dl>
<p>
...
must be passed to all PEs in the active set. <p>
...
PEs in the active set is ready to accept the results of the reduction. <p>
Upon
...
<p> The terms collective, symmetric, and cache
aligned are defined in <i><i>intro_shmem</i>(3)</i>. All SHMEM reduction routines reset
...
to avoid the following situations: If the pSync array is initialized at
run time, some type of synchronization is needed to ensure that all PEs
in the working set have initialized pSync before any of them enter a SHMEM
routine called with the pSync synchronization array. A pSync or pWrk array
can be reused in a subsequent reduction routine call only if none of the
PEs in the active set are still processing a prior reduction routine call
that used the same pSync or pWrk arrays. In general, this can be assured
only by doing some type of synchronization. However, in the special case
of reduction routines being called with the same active set, you can allocate
two pSync and pWrk arrays and alternate between them on successive calls.
<p>

<h2><a name='sect4' href='#toc4'>Examples</a></h2>

<p> <b>Example 1</b>: This Fortran example statically initializes the pSync
...
IF ( MOD(MY_PE(),2) .EQ. 0) THEN
  ...
  &amp; PWRK, PSYNC)
  PRINT *, &rsquo;Result on PE &rsquo;, MY_PE(), &rsquo; is &rsquo;, FOOAND
ENDIF
</pre> <b>Example 2</b>: Consider the following C call: <br>
<pre>shmem_int_and_to_all( target, source, 3, 0, 0, 8, pwrk, psync );
</pre> The preceding call is more efficient, but semantically equivalent to,
the combination of the following calls: <br>
<pre>shmem_int_and_to_all(&amp;(target[0]), &amp;(source[0]), 1, 0, 0, 8,
  pwrk1, psync1);
...
<p> <i><i>f90</i>(1)</i>, <i><i>intro_shmem</i>(3)</i>
 <p>
