<?php
$topdir = "../../..";
$title = "shmem_longdouble_min_to_all(3) man page (version 3.0.6)";
$meta_desc = "Open MPI v3.0.6 man page: shmem_longdouble_min_to_all(3)";
...
<p> <i><i>shmem_double_min_to_all</i>(3)</i>, <i><i>shmem_float_min_to_all</i>(3)</i>, <i><i>shmem_int_min_to_all</i>(3)</i>,
<i><i>shmem_int4_min_to_all</i>(3)</i>, <i><i>shmem_int8_min_to_all</i>(3)</i>, <i><i>shmem_long_min_to_all</i>(3)</i>,
<i><i>shmem_longdouble_min_to_all</i>(3)</i>, <i><i>shmem_longlong_min_to_all</i>(3)</i>, <i><i>shmem_real4_min_to_all</i>(3)</i>,
<i><i>shmem_real8_min_to_all</i>(3)</i>, <i><i>shmem_real16_min_to_all</i>(3)</i>, <i><i>shmem_short_min_to_all</i>(3)</i>
- Performs a minimum function reduction across a set of processing elements
(PEs)
...
<dt><b>shmem_int4_min_to_all</b>: Integer, with an element size of
4 bytes </dt>
<dd></dd>

<dt><b>shmem_int8_min_to_all</b>: Integer, with an element size of 8 bytes
</dt>
<dd></dd>

<dt><b>shmem_real4_min_to_all</b>: Real, with an element size of 4 bytes </dt>
<dd></dd>

<dt><b>shmem_real8_min_to_all</b>:
Real, with an element size of 8 bytes </dt>
<dd></dd>

<dt><b>shmem_real16_min_to_all</b>: Real, with
an element size of 16 bytes </dt>
<dd></dd>

<dt><b>source A symmetric array</b>: of length nreduce
elements, that contains one </dt>
<dd>element for each separate reduction operation.
The source argument must have the same data type as target. </dd>
</dl>
</blockquote>
<blockquote><p>
</blockquote>
...
<dt>nreduce </dt>
<dd>The
number of elements in the target and source arrays. nreduce must be of type
...
The pWrk argument must have the same data type as target. In C/C++, this
contains max(nreduce/2 + 1, _SHMEM_REDUCE_MIN_WRKDATA_SIZE) elements. In
Fortran, this contains max(nreduce/2 + 1, SHMEM_REDUCE_MIN_WRKDATA_SIZE)
elements. </dd>
...
routine call that used the same pSync or pWrk arrays. In general, this can
...
pSync array and finds the minimum value of real variable FOO across all
the even PEs. <br>
<pre>INCLUDE "mpp/shmem.fh"
INTEGER PSYNC(SHMEM_REDUCE_SYNC_SIZE)
...
IF ( MOD(MY_PE(),2) .EQ. 0) THEN
  ...
  &amp; PWRK, PSYNC)
  PRINT *, &rsquo;Result on PE &rsquo;, MY_PE(), &rsquo; is &rsquo;, FOOMIN
ENDIF
</pre> <b>Example 2:</b> Consider the following C/C++ call: <br>
<pre>shmem_int_min_to_all( target, source, 3, 0, 0, 8, pwrk, psync );
</pre> The preceding call is more efficient, but semantically equivalent to,
the combination of the following calls: <br>
<pre>shmem_int_min_to_all(&amp;(target[0]), &amp;(source[0]), 1, 0, 0, 8,
  pwrk1, psync1);
