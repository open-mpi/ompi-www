<?php
$topdir = "../../..";
$title = "oshCC(1) man page (version 3.0.0)";
$meta_desc = "Open MPI v3.0.0 man page: oshCC(1)";
...
flags.  For example, both "oshCC --showme" and "oshCC --showme my_source.c" will
show all the wrapper-supplied flags.  But "oshCC --showme -v" will only show
...
<i>oshCC</i> is a convenience
wrappers for the underlying C++ compiler.  Translation of an Open SHMEM
program requires the linkage of the Open SHMEM-specific libraries which
...
<i>oshCC</i> passes its arguments to the underlying C++
compiler along with the -I, -L and -l options required by Open SHMEM programs.
 <p>
...
compelling reasons <i>not</i> to use <i>oshCC</i>.  When it is not possible to use the
...
by Open SHMEM and installed to <i>$pkgdata/oshCC-wrapper-data.txt</i>, where <i>$pkgdata</i>
