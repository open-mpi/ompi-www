<?php
$topdir = "../../..";
$title = "shmemcc(1) man page (version 3.0.5)";
$meta_desc = "Open MPI v3.0.5 man page: shmemcc(1)";
...
flags.  For example, both "shmemcc --showme" and "shmemcc --showme my_source.c"
will show all the wrapper-supplied flags.  But "shmemcc --showme -v" will only
...
SHMEM program requires the linkage of the Open SHMEM-specific libraries
...
<i>shmemcc</i> passes its arguments to the underlying
...
compelling reasons <i>not</i> to use <i>shmemcc</i>.  When it is not possible to use the
...
by Open SHMEM and installed to <i>$pkgdata/shmemcc-wrapper-data.txt</i>, where <i>$pkgdata</i>
