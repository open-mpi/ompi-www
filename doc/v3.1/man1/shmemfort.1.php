<?php
$topdir = "../../..";
$title = "shmemfort(1) man page (version 3.1.2)";
$meta_desc = "Open MPI v3.1.2 man page: shmemfort(1)";
...
shmemfort
[-showme|-showme:compile|-showme:link] ...
<h2><a name='sect2' href='#toc2'>Options</a></h2>

<dl>

<dt>--showme </dt>
<dd>This option comes in
...
compiler would have been invoked had <i>--showme</i> not been used. The basic <i>--showme</i>
option outputs the command line that would be executed to compile the program.
 <b>NOTE:</b> If a non-filename argument is passed on the command line, the <i>-showme</i>
option will <i>not</i> display any additional flags.  For example, both "shmemfort
...
<dd>Output the compiler flags that would have been
supplied to the Fortran compiler. </dd>

<dt>--showme:link </dt>
<dd>Output the linker flags that
would have been supplied to the Fortran compiler. </dd>
...
Fortran compiler to indicate where relevant header files are located. </dd>
...
simple: transparently add relevant compiler and linker flags to the user&rsquo;s
...
of <i>ld(1)</i>.  It also often requires the inclusion of header files what may
also not be found in a standard location.  <p>
<i>shmemfort</i> passes its arguments
to the underlying Fortran compiler along with the -I, -L and -l options required
by Open SHMEM programs.  <p>
The Open SHMEM Team <i>strongly</i> encourages using the
...
different installations of the same version of Open SHMEM.  <p>
Indeed, since
the wrappers are simply thin shells on top of an underlying compiler, there
are very, very few compelling reasons <i>not</i> to use <i>shmemfort</i>.  When it is
not possible to use the wrappers directly, the <i>-showme:compile</i> and <i>-showme:link</i>
...
by Open SHMEM and installed to <i>$pkgdata/shmemfort-wrapper-data.txt</i>, where
<i>$pkgdata</i> is typically <i>$prefix/share/openmpi</i>, and <i>$prefix</i> is the top installation
