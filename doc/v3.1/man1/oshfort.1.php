<?php
$topdir = "../../..";
$title = "oshfort(1) man page (version 3.1.5)";
$meta_desc = "Open MPI v3.1.5 man page: oshfort(1)";
...
flags.  For example, both "oshfort --showme" and "oshfort --showme my_source.c"
will show all the wrapper-supplied flags.  But "oshfort --showme -v" will only
...
user&rsquo;s command line that are necessary to compile / link Open SHMEM programs,
...
an Open SHMEM program requires the linkage of the Open SHMEM-specific libraries
...
<i>oshfort</i> passes its arguments to the underlying
Fortran compiler along with the -I, -L and -l options required by Open SHMEM
programs.  <p>
The Open SHMEM Team <i>strongly</i> encourages using the wrapper compilers
instead of attempting to link to the Open SHMEM libraries manually.  This
allows the specific implementation of Open SHMEM to change without forcing
changes to linker directives in users&rsquo; Makefiles.  Indeed, the specific set
of flags and libraries used by the wrapper compilers depends on how Open
SHMEM was configured and built; the values can change between different
installations of the same version of Open SHMEM.  <p>
Indeed, since the wrappers
are simply thin shells on top of an underlying compiler, there are very,
very few compelling reasons <i>not</i> to use <i>oshfort</i>.  When it is not possible
to use the wrappers directly, the <i>-showme:compile</i> and <i>-showme:link</i> options
should be used to determine what flags the wrappers would have used.  For
example:  <p>
shell$ cc -c file1.c &lsquo;mpicc -showme:compile&lsquo;  <p>
shell$ cc -c file2.c &lsquo;mpicc
-showme:compile&lsquo;  <p>
...
is, the libraries and includes specified may differ based on the compiler
flags specified (for example, with the GNU compilers on Linux, a different
library path may be used if -m32 is seen versus -m64 being seen).  This is
not the default behavior in a standard build, but can be activated (for
example, in a binary package providing both 32 and 64 bit support).  More
...
by Open SHMEM and installed to <i>$pkgdata/oshfort-wrapper-data.txt</i>, where <i>$pkgdata</i>
