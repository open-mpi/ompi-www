<?php
$topdir = "../../..";
$title = "MPI_Ibcast(3) man page (version 1.7.4)";
$meta_desc = "Open MPI v1.7.4 man page: MPI_IBCAST(3)";
...
<br>
<pre>import mpi.*;
void MPI.COMM_WORLD.Bcast(Object buffer, int offset, int count,
<tt> </tt>&nbsp;<tt> </tt>&nbsp;<tt> </tt>&nbsp;<tt> </tt>&nbsp;<tt> </tt>&nbsp;<tt> </tt>&nbsp;  MPI.Datatype datatype, int root)
</pre>
<h2><a name='sect6' href='#toc6'>Input/Output Parameters</a></h2>
...
<dd>Offset of starting point in buffer (Java-only) </dd>
...
<dt>datatype </dt>
<dd>Data type of buffer (handle). </dd>

<dt>root </dt>
<dd>Rank of broadcast
root (integer). </dd>
...
<a href="../man3/MPI_Bcast.3.php">MPI_Bcast</a> broadcasts a message from the process with rank
root to all processes of the group, itself included. It is called by all
members of group using the same arguments for comm, root. On return, the
contents of root&rsquo;s communication buffer has been copied to all processes.
   <p>
...
count, datatype at the root. This implies that the amount of data sent must
be equal to the amount received, pairwise between each process and the
root. <a href="../man3/MPI_Bcast.3.php">MPI_Bcast</a> and all other data-movement collective routines make this
restriction. Distinct type maps between sender and receiver are still allowed.
 <p>
<b>Example:</b> Broadcast 100 ints from process 0 to every process in the group.
<br>
<pre>    MPI_Comm comm;
    int array[100];
    int root=0;
...
<li><a name='toc6' href='#sect6'>Input/Output Parameters</a></li>
