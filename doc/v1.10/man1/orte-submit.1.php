<?php
$topdir = "../../..";
$title = "orte-submit(1) man page (version 1.10.0)";
$meta_desc = "Open MPI v1.10.0 man page: orte-submit(1)";
...
<h2><a name='sect0' href='#toc0'>Name</a></h2>
 orte-submit, ompi-submit - Execute serial and parallel jobs
in Open MPI using a DVM.
<p> <b>Note:</b> <i>ompi-submit</i> and <i>orte-submit</i> are synonyms for
each other.  Using either of the names will produce the same behavior.

<h2><a name='sect1' href='#toc1'>Synopsis</a></h2>
 <p>
Single Process Multiple Data (SPMD) Model:
<p> <b>ompi-submit</b> [ options
] <b>&lt;program&gt;</b> [ &lt;args&gt; ] <p>

<p> Multiple Instruction Multiple Data (MIMD) Model:
<p> <b>ompi-submit</b>
[ global_options ]        [ local_options1 ]<br>
 <b>&lt;program1&gt;</b> [ &lt;args1&gt; ] :        [ local_options2 ]<br>
 <b>&lt;program2&gt;</b> [ &lt;args2&gt; ] :        ... :<br>
        [ local_optionsN ]<br>
 <b>&lt;programN&gt;</b> [ &lt;argsN&gt; ] <p>

<p> Note that in both models, invoking <i>ompi-submit</i> via
an absolute path name is equivalent to specifying the <i>--prefix</i> option with
a <i>&lt;dir&gt;</i> value equivalent to the directory where <i>ompi-submit</i> resides, minus
its last subdirectory.  For example:
<p>     <b>%</b> /usr/local/bin/ompi-submit ...<br>

<p> is equivalent to
<p>     <b>%</b> ompi-submit --prefix /usr/local<br>

<p>
<h2><a name='sect2' href='#toc2'>Quick Summary</a></h2>
 <b></b> Use of <i>orte-submit</i> requires that you first start the
Distributed Virtual Machine (DVM) using <i>orte-dvm</i>. <p>
If you are simply looking
for how to run an MPI application, you probably want to use a command line
of the following form:
<p>     <b>%</b> ompi-submit [ -np X ] [ --hostfile &lt;filename&gt; ]
 &lt;program&gt;<br>

<p> This will run X copies of <i>&lt;program&gt;</i> in your current run-time environment
(if running under a supported resource manager, Open MPI&rsquo;s <i>ompi-submit</i> will
usually automatically use the corresponding resource manager process starter,
as opposed to, for example, <i>rsh</i> or <i>ssh</i>, which require the use of a hostfile,
...
<h2><a name='sect3' href='#toc3'>Options</a></h2>
 <i>ompi-submit</i> will send the name of the
...
<dt><b>&lt;args&gt;</b> </dt>
<dd>Pass these run-time arguments to every new process.  These must always
be the last arguments to <i>ompi-submit</i>. If an app context file is used, <i>&lt;args&gt;</i>
will be ignored.   </dd>
...
<dt><b>-q, --quiet</b> </dt>
<dd>Suppress
informative messages from orte-submit during application execution.   </dd>
...
<dt><b>-V, --version</b> </dt>
<dd>Print version number.  If no other arguments
are given, this will also cause orte-submit to exit.     </dd>
</dl>
<p>
...
<dt><b></b> -hostfile, --hostfile &lt;hostfile&gt;
</dt>
...
<dt><b>-c, -n, --n, -np &lt;#&gt;</b> </dt>
<dd>Run this many copies of the
...
otherwise.   </li><b>-&lt;&gt;</b><li>Launch N times the number of objects of the specified type
on each node.   </dd>

<dt><b>-npersocket, --npersocket &lt;#persocket&gt;</b> </dt>
<dd>On each node, launch this
many processes times the number of processor sockets on the node. The <i>-npersocket</i>
option also turns on the <i>-bind-to-socket</i> option. (deprecated in favor of --map-by
ppr:n:socket)   </dd>

<dt><b>-npernode, --npernode &lt;#pernode&gt;</b> </dt>
<dd>On each node, launch this many
processes. (deprecated in favor of --map-by ppr:n:node)   </dd>

<dt><b>-pernode, --pernode</b>
</dt>
<dd>On each node, launch one process -- equivalent to <i>-npernode</i> 1. (deprecated
in favor of --map-by ppr:1:node)     </dd>
</dl>
<p>
To map processes:
<dl>
...
<dd>Map to
the specified object, defaults to <i>socket</i>. Supported options include slot,
...
<dd>Map processes by core (deprecated
in favor of --map-by core)  </dd>
...
<dd>Alias for <i>-cpus-per-proc</i>. (deprecated in
favor of --map-by &lt;obj&gt;:PE=n)  </dd>
...
<dt><b>-bind-to-none,
--bind-to-none</b> </dt>
<dd>Do not bind processes  (deprecated in favor of --bind-to none)
 </dd>
...
<dd>List of processor IDs to be used for binding
MPI processes. The specified bindings will be applied to all MPI processes.
See explanation below for syntax.     </dd>
</dl>
<p>
For rankfiles:
<dl>

<dt><b>-rf, --rankfile &lt;rankfile&gt;</b>
</dt>
...
<dt><b>-output-filename, --output-filename
&lt;filename&gt;</b> </dt>
<dd>Redirect the stdout, stderr, and stddiag of all processes to a
...
filename.id, where the id will be the processes&rsquo; rank in MPI_COMM_WORLD,
left-filled with zero&rsquo;s for correct ordering in listings.   </dd>

<dt><b>-stdin, --stdin &lt;rank&gt;</b>
</dt>
...
<dt><b>-tag-output, --tag-output</b>
</dt>
<dd>Tag each line of output to stdout, stderr, and stddiag with <b>[jobid, MCW_rank]&lt;stdxxx&gt;</b>
indicating the process jobid and MPI_COMM_WORLD rank of the process that
generated the output, and the channel which generated it.   </dd>

<dt><b>-timestamp-output,
--timestamp-output</b> </dt>
<dd>Timestamp each line of output to stdout, stderr, and stddiag.
  </dd>

<dt><b>-xml, --xml</b> </dt>
<dd>Provide all output to stdout, stderr, and stddiag in an xml
format.   </dd>

<dt><b>-xterm, --xterm &lt;ranks&gt;</b> </dt>
<dd>Display the output from the processes identified
...
window will be created for each specified process. <b>Note:</b> xterm will normally
...
names specified with corresponding values.  For example:     <b>%</b> ompi-submit
-x DISPLAY -x OFILE=/tmp/out ...<br>
...
<dd>Allow <i>ompi-submit</i>
to run when executed by the root user (<i>ompi-submit</i> defaults to aborting
when launched as the root user).   </dd>

<dt><b>-aborted, --aborted &lt;#&gt;</b> </dt>
<dd>Set the maximum number
of aborted processes to display.   </dd>

<dt><b>--app &lt;appfile&gt;</b> </dt>
<dd>Provide an appfile, ignoring
all other command line options.   </dd>

<dt><b>-cf, --cartofile &lt;cartofile&gt;</b> </dt>
<dd>Provide a cartography
file.   </dd>

<dt><b>--hetero</b> </dt>
<dd>Indicates that multiple app_contexts are being provided that
are a mix of 32/64-bit binaries.   </dd>

<dt><b>-ompi-server, --ompi-server &lt;uri or file&gt;</b> </dt>
<dd>Specify
...
<dd>Enable debugging of the OmpiRTE (the
run-time layer in Open MPI). This is not generally useful for most users.
   </dd>
</dl>
<p>
There may be other options listed with <i>ompi-submit --help</i>.
<h3><a name='sect4' href='#toc4'>Environment
Variables</a></h3>

<dl>

<dt><b>MPIEXEC_TIMEOUT</b> </dt>
<dd>The maximum number of seconds that <i>ompi-submit</i>
(<i>mpiexec</i>)<i></i> will run.  After this many seconds, <i>ompi-submit</i> will abort the
launched job and exit.      </dd>
</dl>

<h2><a name='sect5' href='#toc5'>Description</a></h2>
 One invocation of <i>ompi-submit</i> starts
an MPI application running under Open MPI. If the application is single
process multiple data (SPMD), the application can be specified on the <i>ompi-submit</i>
command line.
...
multiple <i>ompi-submit</i> command lines, less the command name itself.  The ability
...
Nodes</a></h3>
 Host nodes can be identified on the <i>ompi-submit</i> command line with
the <i>-host</i> option or in a hostfile.  <p>
For example,
<dl>

<dt>ompi-submit -H aa,aa,bb ./a.out
</dt>
<dd>launches two processes on node aa and one on bb.  </dd>
</dl>
<p>
...
<p>  <p>
Since the DVM was started with <i>orte-dvm</i>, <i>orte-submit</i> will ignore any slots
arguments in the hostfile. Values provided via hostfile to <i>orte-dvm</i> will
control the behavior.  <p>

<dl>

<dt>ompi-submit -hostfile myhostfile ./a.out </dt>
<dd>will launch
two processes on each of the three nodes.  </dd>

<dt>ompi-submit -hostfile myhostfile
-host aa ./a.out </dt>
<dd>will launch two processes, both on node aa.  </dd>

<dt>ompi-submit -hostfile
...
<dt>ompi-submit -H aa,bb -npersocket
2 ./a.out </dt>
...
<dt>ompi-submit -H aa,bb -npernode 1 ./a.out </dt>
<dd>launches one process
per host node.  </dd>

<dt>ompi-submit -H aa,bb -pernode ./a.out </dt>
<dd>is the same as <i>-npernode</i>
1.   </dd>
</dl>
<p>
...
<dt>ompi-submit -hostfile myhostfile -np 6 ./a.out </dt>
<dd>will launch processes
...
can also be controlled with various <i>ompi-submit</i> options that describe mapping
policies.   <p>
Consider the same hostfile as above, again with <i>-np</i> 6:
<p>
                      node aa      node bb      node cc<br>

<p>   ompi-submit                  0 1 2 3      4 5<br>

<p>   ompi-submit --map-by node    0 3          1 4          2 5<br>

<p>   ompi-submit -nolocal                      0 1 2 3      4 5<br>
  <p>
...
node aa).  While <i>ompi-submit</i> typically consumes few system resources, <i>-nolocal</i>
can be helpful for launching very large jobs where <i>ompi-submit</i> may actually
...
<dt>ompi-submit -hostfile myhostfile -np 14 -nooversubscribe ./a.out
</dt>
...
<dt>ompi-submit -hostfile myhostfile -np 14 ./a.out
</dt>
<dd>causes the first 12 processes to be launched as before, but the remaining
...
<dt>ompi-submit -H aa,bb -np 8 ./a.out </dt>
<dd>launches
...
<dt>ompi-submit -H aa -np 1
hostname : -H bb,cc -np 2 uptime </dt>
<dd>will launch process 0 running <i>hostname</i> on
node aa and processes 1 and 2 each running <i>uptime</i> on nodes bb and cc, respectively.
 </dd>
</dl>
...
consider a node with two processor sockets, each comprising four cores.
 We run <i>ompi-submit</i> with <i>-np 4 --report-bindings</i> and the following additional
options:
<p>  % ompi-submit ... --map-by core --bind-to core<br>
...
<p>  % ompi-submit ... --map-by socket -0bind-to socket<br>
...
<p>  % ompi-submit ... --map-by core:PE=2 -bind-to core<br>
...
of <i>ompi-submit</i> options. On the other hand, MCA parameters can be set not
only on the <i>ompi-submit</i> command line, but alternatively in a system or user
mca-params.conf file or as environment variables, as described in the MCA
section below. Some examples include:  <p>
...
<p>     $ cat myrankfile<br>
     ...
     $ ompi-submit -H aa,bb,cc,dd -rf myrankfile ./a.out<br>
  <p>
Means that
...
of hostnames (e.g., by ompi-submit&rsquo;s --host argument, a hostfile, or a job scheduler).
 <p>
...
<h3><a name='sect11' href='#toc11'>Application Context or Executable Program?</a></h3>
 To distinguish the
two different forms, <i>ompi-submit</i> looks on the command line for <i>--app</i> option.
...
working directory from the invocation of <i>ompi-submit</i>.
<h3><a name='sect13' href='#toc13'>Current Working
Directory</a></h3>
 The <i>-wdir</i> ompi-submit option (and its synonym, <i>-wd</i>) allows the user
...
If the <i>-wdir</i> option appears
both in a context file and on the command line, the context file directory
will override the command line value. <p>
If the <i>-wdir</i> option is specified, Open
MPI will attempt to change to the specified directory on all of the remote
nodes. If this fails, <i>ompi-submit</i> will abort. <p>
If the <i>-wdir</i> option is <b>not</b> specified,
Open MPI will send the directory name where <i>ompi-submit</i> was invoked to each
...
from <i>ompi-submit</i>. <b>Note:</b> The node that invoked <i>ompi-submit</i> need not be the
same as the node where the MPI_COMM_WORLD rank 0 process resides. Open MPI
handles the redirection of <i>ompi-submit</i>&rsquo;s standard input to the rank 0 process.
<p>
Open MPI directs UNIX standard output and error from remote nodes to the
node that invoked <i>ompi-submit</i> and prints it on the standard output/error
of <i>ompi-submit</i>. Local processes inherit the standard output/error of <i>ompi-submit</i>
and transfer to it directly. <p>
Thus it is possible to redirect standard I/O
for Open MPI applications by using the typical shell redirection procedure
on <i>ompi-submit</i>.
<p>       <b>%</b> ompi-submit -np 2 my_app &lt; my_input &gt; my_output<br>
...
then sending all processes in the job a SIGKILL.  <p>
SIGUSR1 and SIGUSR2 signals
received by orte-submit are propagated to all processes in the job.  <p>
One
...
to ompi-submit will cause a SIGCONT sent.  <p>
Other signals are not currently
propagated by orte-submit.
<h3><a name='sect16' href='#toc16'>Process Termination / Signal Handling</a></h3>
 During
the run of an MPI application, if any process dies abnormally (either exiting
before invoking <i><a href="../man3/MPI_Finalize.3.php">MPI_FINALIZE</a></i>, or dying as the result of a signal), <i>ompi-submit</i>
will print out an error message and kill the rest of the MPI application.
<p>
User signal handlers should probably avoid trying to cleanup MPI state
(Open MPI is currently not async-signal-safe; see <i><a href="../man3/MPI_Init_thread.3.php">MPI_Init_thread</a>(3)</i> for
details about <i>MPI_THREAD_MULTIPLE</i> and thread safety).  For example, if a
segmentation fault occurs in <i><a href="../man3/MPI_Send.3.php">MPI_SEND</a></i> (perhaps because a bad buffer was
passed in) and a user signal handler is invoked, if this user handler attempts
to invoke <i><a href="../man3/MPI_Finalize.3.php">MPI_FINALIZE</a></i>, Bad Things could happen since Open MPI was already
"in" MPI when the error occurred.  Since <i>ompi-submit</i> will notice that the
...
from the user&rsquo;s shell.  On remote nodes, the exact environment is determined
by the boot MCA module used.  The <i>rsh</i> launch module, for example, uses either
<i>rsh</i>/<i>ssh</i> to launch the Open RTE daemon on remote nodes, and typically executes
one or more of the user&rsquo;s shell-setup files before launching the Open RTE
daemon.  When running dynamically linked applications which require the
<i>LD_LIBRARY_PATH</i> environment variable to be set, care must be taken to ensure
that it is correctly set when booting Open MPI. <p>
See the "Remote Execution"
section for more details.
<h3><a name='sect18' href='#toc18'>Remote Execution</a></h3>
 Open MPI requires that the
<i>PATH</i> environment variable be set to find executables on remote nodes (this
is typically only necessary in <i>rsh</i>- or <i>ssh</i>-based environments -- batch/scheduled
environments typically copy the current environment to the execution of
remote jobs, so if the current environment has <i>PATH</i> and/or <i>LD_LIBRARY_PATH</i>
...
<p>     <b>%</b> ompi-submit --prefix /remote/node/directory<br>
...
Note
that executing <i>ompi-submit</i> via an absolute pathname is equivalent to specifying
<i>--prefix</i> without the last subdirectory in the absolute pathname to <i>ompi-submit</i>.
 For example:
<p>     <b>%</b> /usr/local/bin/ompi-submit ...<br>
...
to <i>ompi-submit</i> has been deprecated, but the syntax of the MCA param follows
...
<dt>ompi-submit -mca btl tcp,self -np 1 foo </dt>
<dd>Tells Open MPI to use the "tcp" and
"self" BTLs, and to run a single copy of "foo" an allocated node.  </dd>

<dt>ompi-submit
-mca btl self -np 1 foo </dt>
<dd>Tells Open MPI to use the "self" BTL, and to run
a single copy of "foo" an allocated node.  </dd>
</dl>
<p>
The <i>-mca</i> switch can be used multiple
times to specify different <i>&lt;key&gt;</i> and/or <i>&lt;value&gt;</i> arguments.  If the same <i>&lt;key&gt;</i>
is specified more than once, the <i>&lt;value&gt;</i>s are concatenated with a comma (",")
separating them. <p>
Note that the <i>-mca</i> switch is simply a shortcut for setting
environment variables. The same effect may be accomplished by setting corresponding
environment variables before running <i>ompi-submit</i>. The form of the environment
...
as environment variable -- they are not checked (by <i>ompi-submit</i>) for correctness.
...
<h3><a name='sect21' href='#toc21'>Running as root</a></h3>
 The Open MPI team
strongly advises against executing <i>ompi-submit</i> as the root user.  MPI applications
...
add the <i>--allow-run-as-root</i> option to the <i>ompi-submit</i> command line.
<h3><a name='sect22' href='#toc22'>Exit status</a></h3>

There is no standard definition for what <i>ompi-submit</i> should return as an
exit status. After considerable discussion, we settled on the following
method for assigning the <i>ompi-submit</i> exit status (note: in the following
...
<dt>[bu]</dt>
<dd>if one or more processes in the primary job normally
terminate with non-zero exit status, we return the exit status of the process
with the lowest MPI_COMM_WORLD rank to have a non-zero status </dd>
...
<dt>[bu]</dt>
<dd>if the cmd line option --report-child-jobs-separately is set, we will
...
<dt>ompi-submit -np 4 -mca btl ib,tcp,self prog1
</dt>
<dd>Run 4 copies of prog1 using the "ib", "tcp", and "self" BTL&rsquo;s for the transport
of MPI messages.   </dd>

<dt>ompi-submit -np 4 -mca btl tcp,sm,self </dt>
<dd><br>
...
<h2><a name='sect24' href='#toc24'>Return Value</a></h2>
 <i>ompi-submit</i> returns 0 if all processes started by <i>ompi-submit</i>
exit after calling <a href="../man3/MPI_Finalize.3.php">MPI_FINALIZE</a>.  A non-zero value is returned if an internal
error occurred in ompi-submit, or one or more processes exited before calling
<a href="../man3/MPI_Finalize.3.php">MPI_FINALIZE</a>.  If an internal error occurred in ompi-submit, the corresponding
...
process that <i>ompi-submit</i> first notices died before calling <a href="../man3/MPI_Finalize.3.php">MPI_FINALIZE</a>
...
<i><a href="../man3/MPI_Init_thread.3.php">MPI_Init_thread</a>(3)</i>
<p>
