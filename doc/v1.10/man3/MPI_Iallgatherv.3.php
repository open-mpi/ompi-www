<?php
$topdir = "../../..";
$title = "MPI_Iallgatherv(3) man page (version 1.10.0)";
$meta_desc = "Open MPI v1.10.0 man page: MPI_IALLGATHERV(3)";
...
<dt>recvcount     </dt>
<dd>Integer array (of length group
size) containing the number of elements that are received from each process.
</dd>

<dt>displs     </dt>
<dd>Integer array (of length group size). Entry i specifies the displacement
(relative to recvbuf) at which to place the incoming data from process
i. </dd>
...
<a href="../man3/MPI_Allgatherv.3.php">MPI_Allgatherv</a> is similar to <a href="../man3/MPI_Allgather.3.php">MPI_Allgather</a> in that
all processes gather data from all other processes, except that each process
can send a different amount of data. The block of data sent from the jth
process is received by every process and placed in the jth block of the
buffer  <i>recvbuf.</i> <p>
The type signature associated with sendcount, sendtype,
at process j must be equal to the type signature associated with recvcounts[j],
recvtype at any other process.  <p>
The outcome is as if all processes executed
calls to       <br>
