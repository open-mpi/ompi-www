<?php
$topdir = "../../..";
$title = "MPI_File_open(3) man page (version 1.10.0)";
$meta_desc = "Open MPI v1.10.0 man page: MPI_FILE_OPEN(3)";
...
<pre>    INCLUDE &rsquo;mpif.h&rsquo;
   ...
   <tt> </tt>&nbsp;<tt> </tt>&nbsp; CHARACTER*(*)    FILENAME
    <tt> </tt>&nbsp;<tt> </tt>&nbsp; INTEGER      COMM, AMODE, INFO, FH, IERROR
</pre>
...
MPI_File_open opens the file identified
by the filename  <i></i> filename on all processes in the  <i>comm</i> communicator group.
MPI_File_open is a collective routine; all processes must provide the same
value for <i>amode,</i> and all processes must provide filenames that reference
the same file and which are textually identical. A process can open a file
independently of other processes by using the MPI_COMM_SELF communicator.
The file handle returned,  <i>fh,</i> can be subsequently used to access the file
until the file is closed using <a href="../man3/MPI_File_close.3.php">MPI_File_close</a>. Before calling <a href="../man3/MPI_Finalize.3.php">MPI_Finalize</a>,
the user is required to close (via <a href="../man3/MPI_File_close.3.php">MPI_File_close</a>) all files that were
opened with MPI_File_open. Note that the communicator  <i>comm</i>  is unaffected
by MPI_File_open and continues to be usable in all MPI routines. Furthermore,
use of  <i>comm</i>  will not interfere with I/O behavior. <p>
Initially, all processes
view the file as a linear byte stream; that is, the  <i>etype</i>  and  <i>filetype</i>
are both MPI_BYTE. The file view can be changed via the <a href="../man3/MPI_File_set_view.3.php">MPI_File_set_view</a>
routine. <p>
The following access modes are supported (specified in amode, in
a bit-vector OR in one of the following integer constants): <br>
...
in a communicator group reside on more than one node.  This setting can
be changed using <a href="../man3/MPI_File_set_atomicity.3.php">MPI_File_set_atomicity</a>. <p>
