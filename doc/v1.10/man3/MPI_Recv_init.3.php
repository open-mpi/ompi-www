<?php
$topdir = "../../..";
$title = "MPI_Recv_init(3) man page (version 1.10.1)";
$meta_desc = "Open MPI v1.10.1 man page: MPI_RECV_INIT(3)";
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;int source, int tag, MPI_Comm comm, MPI_Request *request)
</pre>
...
<tt> </tt>&nbsp;<tt> </tt>&nbsp;Datatype&amp; datatype, int source, int tag) const
</pre>
...
<dd>Type of each entry (handle). </dd>
...
<i>buf</i> is marked as OUT because the user gives permission to write on the
receive buffer by passing the argument to MPI_Recv_init.   <p>
A persistent
communication request is inactive after it is created -- no active communication
is attached to the request.  <p>
A communication (send or receive) that uses
a persistent request is initiated by the function <a href="../man3/MPI_Start.3.php">MPI_Start</a> or <a href="../man3/MPI_Startall.3.php">MPI_Startall</a>.
