<?php
$topdir = "../../..";
$title = "mpicc(1) man page (version 1.6.4)";
$meta_desc = "Open MPI v1.6.4 man page: mpicc(1)";
...
flags to the user&rsquo;s command line that are necessary to compile / link Open
...
<i>mpicc</i> is a convenience wrappers for the underlying C compiler.
...
<i>mpicc</i> passes its arguments to
the underlying C compiler along with the -I, -L and -l options required by
Open MPI programs.  <p>
The Open MPI Team <i>strongly</i> encourages using the wrapper
...
forcing changes to linker directives in users&rsquo; Makefiles.  Indeed, the specific
...
installations of the same version of Open MPI.  <p>
Indeed, since the wrappers
are simply thin shells on top of an underlying compiler, there are very,
very few compelling reasons <i>not</i> to use <i>mpicc</i>.  When it is not possible to
use the wrappers directly, the <i>-showme:compile</i> and <i>-showme:link</i> options should
