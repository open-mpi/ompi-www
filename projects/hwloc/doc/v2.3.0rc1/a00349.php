...
<p>If the application does not want the CPU binding to change when changing the memory policy, it needs to use the <a class="el" href="a00158.php#ggab00475fd98815bf4fb9aaf752030e7d2aad6b9eaf2ee324ca58dc8f58094b9997" title="Avoid any effect on CPU binding.">HWLOC_MEMBIND_NOCPUBIND</a> flag to prevent hwloc from using OS functions which would change the CPU binding. Additionally, <a class="el" href="a00157.php#gga217dc8d373f8958cc93c154ebce1c71ca41ce440443cc3087caed95ab60edcad6" title="Avoid any effect on memory binding.">HWLOC_CPUBIND_NOMEMBIND</a> can be passed to CPU binding function to prevent hwloc from using OS functions would change the memory binding policy. Of course, using these flags will reduce hwloc's overall support for binding, so their use is discouraged.</p>
...
<li>
a function that allocates memory bound to specific node set without changing the current memory binding policy (if supported): <a class="el" href="a00158.php#ga04736461780fadcf193af218c0122273" title="Allocate some memory on NUMA memory nodes specified by set.">hwloc_alloc_membind()</a>. </li>
<li>
a helper which, if needed, changes the current memory binding policy of the process in order to obtain memory binding: <a class="el" href="a00158.php#gab1b77b8408bacaf03c7e8878f7577922" title="Allocate some memory on NUMA memory nodes specified by set.">hwloc_alloc_membind_policy()</a>. </li>
</ul>
...
<p>See <a class="el" href="a00157.php">CPU binding</a> and <a class="el" href="a00158.php">Memory binding</a> for hwloc's API functions regarding CPU and memory binding, respectively. There are some examples under doc/examples/ in the source tree. </p>
</div></div><!-- contents -->
</div><!-- PageDoc -->
<?php
