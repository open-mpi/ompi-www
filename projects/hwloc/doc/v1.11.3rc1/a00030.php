...
<div class="contents">
<div class="textblock"><h1><a class="anchor" id="faq_indexes"></a>
Should I use logical or physical/OS indexes? and how?</h1>
...
0,2,5
</pre><h1><a class="anchor" id="faq_xml"></a>
...
<p><a class="el" href="a00060_source.php" title="Topology differences. ">hwloc/diff.h</a> (see also <a class="el" href="a00100.php">Topology differences</a>) offers the ability to compute topology differences, apply or unapply them, or export/import to/from XML. However this feature is limited to basic differences such as attribute changes. It does not support complex modifications such as adding or removing some objects.</p>
...
<p>The utility <code>hwloc-dump-hwdata</code> is also involved in gathering privileged information at boot time and making it available to non-privileged users. However it only applies to Intel Knights Landing Xeon Phi for now (see <a class="el" href="a00030.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for caches on Intel Knights Landing Xeon Phi?</a>). See also <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00006.php">Environment Variables</a> for details about the location of dumped files.</p>
...
</pre><h1><a class="anchor" id="faq_groups"></a>
What are these Group objects in my topology?</h1>
...
<h1><a class="anchor" id="faq_asymmetric"></a>
...
<h1><a class="anchor" id="faq_knl_numa"></a>
How do I find the local MCDRAM NUMA node on Intel Knights Landing Xeon Phi?</h1>
<p>Intel Knights Landing Xeon Phi processors introduce a new memory architecture by possibly having two distinct local memories: some normal memory (DDR) and some high-bandwidth on-package memory (MCDRAM). Processors can be configured in various clustering modes to have up to 4 <em>Clusters</em>. Moreover, each <em>Cluster</em> (quarter, half or whole processor) of the processor may have its own local parts of the DDR and of the MCDRAM.</p>
...
<h1><a class="anchor" id="faq_knl_dump"></a>
Why do I need hwloc-dump-hwdata for caches on Intel Knights Landing Xeon Phi?</h1>
<p>Intel Knights Landing Xeon Phi processors may use the on-package memory (MCDRAM) as either memory or a memory-side cache (currently reported as a L3 cache by hwloc). Details about this cache (especially its size) are currently only available to privileged users.</p>
<p>The hwloc-dump-hwdata utility may be used to dump this privileged binary information into human-readable and world-accessible files that the hwloc library will later load. The utility should usually run as root once during boot, in order to update dumped information (stored under /var/run/hwloc by default) in case the MCDRAM configuration changed between reboots.</p>
