...
<p>lstopo options <code>-l</code> and <code>-p</code> may be used to switch between logical indexes (prefixed with <code>L#</code>) and physical/OS indexes (<code>P#</code>). Converting one into the other may also be achieved with hwloc-calc which may manipulate either logical or physical indexes as input or output. See also <a class="el" href="a00004.php#cli_hwloc_calc">hwloc-calc</a>.</p>
...
0,2,5
</pre><h2><a class="anchor" id="faq_onedim"></a>
...
<p>hwloc comes with a set of predefined object types (Core, Package, NUMA node, Caches) that match the vast majority of hardware platforms. The <a class="el" href="a00076.php#ggacd37bb612667dc437d66bfb175a8dc55a5269ef95be72f88465559d35c9b7ad56" title="Group objects. Objects which do not fit in the above but are detected by hwloc and are useful to take...">HWLOC_OBJ_GROUP</a> type was designed for cases where this set is not sufficient. Groups may be used anywhere to add more structure information to the topology, for instance to show that 2 out of 4 NUMA nodes are actually closer than the others. When applicable, the <code>Type</code> info attribute describes why a Group was actually added (see also <a class="el" href="a00016.php#attributes_info">Custom string infos</a>).</p>
...
<p>In practice, asymmetric topologies mostly appear when intermediate groups are added for I/O affinity: on a 4-package machine, an I/O bus may be connected to 2 packages. These packages are below an additional Group object, while the other packages are not (see also <a class="el" href="a00030.php#faq_groups">What are these Group objects in my topology?</a>).</p>
...
<p>First, see <a class="el" href="a00030.php#faq_nosmt">What happens to my topology if I disable symmetric multithreading, hyper-threading, etc. in the system?</a> for more information about multithreading.</p>
...
<p>The utility <code>hwloc-dump-hwdata</code> is also involved in gathering privileged information at boot time and making it available to non-privileged users (note that this may require a specific SELinux MLS policy module). However it only applies to Intel Knights Landing Xeon Phi for now (see <a class="el" href="a00030.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for memory on Intel Knights Landing Xeon Phi?</a>). See also <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00006.php">Environment Variables</a> for details about the location of dumped files.</p>
...
#endif
</pre><p>You should not try to remain compatible with very old releases such as 1.1.x or earlier because <code><a class="el" href="a00074.php#ga8f4dfb8eef138af55dd1a0fa802e5476" title="Indicate at build time which hwloc API version is being used. ">HWLOC_API_VERSION</a></code> was added in 1.0.0 and <code><a class="el" href="a00074.php#ga9c0b50c98add1adf57ed1ce85bb5190d" title="Indicate at runtime which hwloc API version was used at build time. ">hwloc_get_api_version()</a></code> came only in 1.1.1. Also do not use the old cpuset API since it was deprecated and superseded by the bitmap API in 1.1, and later removed in 1.5.</p>
...
<p>Intel Knights Landing Xeon Phi processors introduce a new memory architecture by possibly having two distinct local memories: some normal memory (DDR) and some high-bandwidth on-package memory (MCDRAM). Processors can be configured in various clustering modes to have up to 4 <em>Clusters</em>. Moreover, each <em>Cluster</em> (quarter, half or whole processor) of the processor may have its own local parts of the DDR and of the MCDRAM. This memory and clustering configuration may be probed by looking at MemoryMode and ClusterMode attributes, see <a class="el" href="a00016.php#attributes_info">Custom string infos</a> and doc/examples/get-knl-modes.c in the source directory.</p>
...
<p>See <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00006.php">Environment Variables</a> for details about the location of dumped files.</p>
