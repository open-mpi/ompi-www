...
<p>If any configuration function such as <a class="el" href="a00055.php#ga93efcc8a962afe1ed23393700682173f" title="Enable XML-file based topology. ">hwloc_topology_set_xml()</a> is used before loading the topology, the corresponding component is enabled first. Then, as usual, hwloc enables any other component (based on priorities) that does not conflict.</p>
<p>Certain components that manage a virtual topology, for instance XML topology import, synthetic topology description, or custom building, conflict with all other components. Therefore, one of them may only be loaded (e.g. with <code><a class="el" href="a00055.php#ga93efcc8a962afe1ed23393700682173f" title="Enable XML-file based topology. ">hwloc_topology_set_xml()</a></code>) if no other component is enabled.</p>
...
<p>Certain component names (<code>xml</code> and <code>synthetic</code>) accept an argument (e.g. <code>xml=file.xml</code>). These arguments behave exactly as if the corresponding string had been passed to <code><a class="el" href="a00055.php#ga93efcc8a962afe1ed23393700682173f" title="Enable XML-file based topology. ">hwloc_topology_set_xml()</a></code> or <code><a class="el" href="a00055.php#ga2fcb52181b586c20f001b7a999550324" title="Enable synthetic topology. ">hwloc_topology_set_synthetic()</a></code>.</p>
...
<p>Also note that plugins should carefully be enabled and used when embedding hwloc in another project, see <a class="el" href="a00013.php#embed">Embedding hwloc in Other Software</a> for details.</p>
<h1><a class="anchor" id="plugins_adding"></a>
Adding new discovery components and plugins</h1>
<p>The types and functions cited below are declared in the <a class="el" href="a00048_source.php" title="Public interface for building hwloc plugins. ">hwloc/plugins.h</a> header. Components are supposed to only use hwloc public headers (<a class="el" href="a00040_source.php" title="The hwloc API. ">hwloc.h</a> and anything under the <code>include/hwloc</code> subdirectory) and nothing from the <code>include/private</code> subdirectory in the source tree.</p>
<h2><a class="anchor" id="plugins_disc_basic"></a>
Basics of discovery components</h2>
<p>Each discovery component is defined by a <code><a class="el" href="a00019.php" title="Discovery component structure. ">hwloc_disc_component</a></code> structure which contains an <code>instantiate()</code> callback. This function is invoked when this component is actually used by a topology. It fills a new <code><a class="el" href="a00015.php" title="Discovery backend structure. ">hwloc_backend</a></code> structure that usually contains <code>discover()</code> and/or <code>notify_new_object()</code> callbacks taking care of the actual topology discovery.</p>
...
<p>Registering components to the hwloc core relies on a <code><a class="el" href="a00018.php" title="Generic component structure. ">hwloc_component</a></code> structure. Its <code>data</code> field points to the previously defined <code><a class="el" href="a00019.php" title="Discovery component structure. ">hwloc_disc_component</a></code> structure while its <code>type</code> should be HWLOC_COMPONENT_TYPE_DISC. This structure should be named <code>hwloc_&lt;name&gt;_component</code>.</p>
...
<dl class="section note"><dt>Note</dt><dd>The symbol name of the <code><a class="el" href="a00018.php" title="Generic component structure. ">hwloc_component</a></code> structure is independent of the name of the discovery component mentioned in the previous section.</dd></dl>
...
<dt>bgq </dt>
<dd>This component is specific to IBM BlueGene/Q compute node (running CNK). It is built and enabled by default when <code>&ndash;host=powerpc64-bgq-linux</code> is passed to configure (see <a class="el" href="a00014.php#faq_bgq">How do I build hwloc for BlueGene/Q?</a>).  </dd>
<dt>no_os </dt>
