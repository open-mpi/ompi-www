...
<h2><a class="anchor" id="multinode_cpusets"></a>
Multi-node Objects Specifities</h2>
...
<div class="fragment"><pre class="fragment">
  ...
  if (machine_obj)
    ...
    return NULL;
</pre></div><p> The hwloc assembler scripts (see below) also add <code>AssemblerName</code> and <code>AssemblerIndex</code> info attributes to the Machine objects to identify the corresponding host name and index during assembly.</p>
<h2><a class="anchor" id="multinode_tools"></a>
Assembling topologies with command-line tools</h2>
<p>One way to manipulate multinode topologies is to retrieve other nodes' topologies as XML files and combine them as a global XML topology. It may then be loaded with <a class="el" href="a00044.php#ga93efcc8a962afe1ed23393700682173f" title="Enable XML-file based topology.">hwloc_topology_set_xml()</a> or with the HWLOC_XMLFILE environment variable.</p>
<p>The hwloc-assembler and hwloc-assembler-remote utilities offer the ability to combine XML topologies or remote nodes' topologies (see <a class="el" href="a00002.php">Command-Line Tools</a>).</p>
<h2><a class="anchor" id="multinode_interface"></a>
Assembling topologies with the programming interface</h2>
<p>The hwloc programming interface offers the ability to build multinode topologies using the <em>custom</em> interface. A new multinode topology has to be initialized with <a class="el" href="a00043.php#ga5c2d6f476af87005c7bd0811d4548b9f" title="Allocate a topology context.">hwloc_topology_init()</a> and then set to custom with <a class="el" href="a00044.php#ga12024fec46f9368fb8fc5c624089c5ec" title="Prepare the topology for custom assembly.">hwloc_topology_set_custom()</a>. Topologies and objects mat then be assembled. Later, the custom topology is finalized as usual with <a class="el" href="a00043.php#ga91e2e6427b95fb7339c99dbbef996e71" title="Build the actual topology.">hwloc_topology_load()</a>.</p>
<p>A custom topology starts with a single root object of type System. It may be modified by inserting a new child object with <a class="el" href="a00051.php#gac1037fe389bfa7c2bf0e3739b7e20844" title="Insert a new group object inside a custom topology.">hwloc_custom_insert_group_object_by_parent()</a> or by duplicating another topology with <a class="el" href="a00051.php#ga59ccf6a63cb23d63940e8b782059d9cb" title="Insert an existing topology inside a custom topology.">hwloc_custom_insert_topology()</a>. Both of these operations require to specify the parent object in the custom topology where the insertion will take place. This parent may be either the root (returned by <a class="el" href="a00053.php#gadbf58f6e187efbdb3cd9a8e30311b7d7" title="Returns the top-object of the topology-tree.">hwloc_get_root_obj()</a>) or an already-inserted object (returned by <a class="el" href="a00051.php#gac1037fe389bfa7c2bf0e3739b7e20844" title="Insert a new group object inside a custom topology.">hwloc_custom_insert_group_object_by_parent()</a>).</p>
<p>Ideally, any existing object in the custom topology could be the parent. However, special care should be taken when traversing the topology to find such an object because most links between objects (children, siblings, cousins) are not setup until <a class="el" href="a00043.php#ga91e2e6427b95fb7339c99dbbef996e71" title="Build the actual topology.">hwloc_topology_load()</a> is invoked.</p>
<h2><a class="anchor" id="multinode_example"></a>
Example of assembly with the programming interface</h2>
...
...
</pre></div><p>If a intermediate object such as a switch should be inserted above one of the host topologies: </p>
<div class="fragment"><pre class="fragment">
...
