...
<p>hwloc objects have many generic attributes in the <a class="el" href="a00197.php" title="Structure of a topology object.">hwloc_obj</a> structure, for instance their <code>logical_index</code> or <code>os_index</code> (see <a class="el" href="a00359.php#faq_indexes">Should I use logical or physical/OS indexes? and how?</a>), <code>depth</code> or <code>name</code>.</p>
...
<li>
NUMA nodes: subtype <code>DRAM</code> (for usual main memory), <code>HBM</code> (high-bandwidth memory), <code>SPM</code> (specific-purpose memory, usually reserved for some custom applications), <code>NVM</code> (non-volatile memory when used as main memory), <code>MCDRAM</code> (on KNL), <code>GPUMemory</code> (on POWER architecture with NVIDIA GPU memory shared over NVLink), <code>CXL-DRAM</code> or <code>CXL-NVM</code> for CXL DRAM or non-volatile memory. Note that some of these subtypes are guessed by the library, they might be missing or slightly wrong in some corner cases. See <a class="el" href="a00352.php">Heterogeneous Memory</a> for details, and HWLOC_MEMTIERS and HWLOC_MEMTIERS_GUESS in <a class="el" href="a00346.php">Environment Variables</a> for tuning these.  </li>
<li>
Groups: subtype <code>Cluster</code>, <code>Module</code>, <code>Tile</code>, <code>Compute Unit</code>, <code>Book</code> or <code>Drawer</code> for different architecture-specific groups of CPUs (see also <a class="el" href="a00359.php#faq_groups">What are these Group objects in my topology?</a>).  </li>
<li>
OS devices (see also <a class="el" href="a00348.php#iodevices_osdev">OS devices</a>): <ul>
<li>
...
<li>
PCI devices: subtype <code>NVSwitch</code> for NVLink switches (see also NVLinkBandwidth in <a class="el" href="a00351.php#topoattrs_distances">Distances</a>).  </li>
<li>
Misc devices: subtype <code>MemoryModule</code> (see also <a class="el" href="a00349.php#miscobjs_auto">Misc objects added by hwloc</a>)  </li>
</ul>
<p>Each object also contains an <code>attr</code> field that, if non NULL, points to a union <a class="el" href="a00201.php" title="Object type-specific Attributes.">hwloc_obj_attr_u</a> of type-specific attribute structures. For instance, a L2Cache object <code>obj</code> contains cache-specific information in <code>obj-&gt;attr-&gt;cache</code>, such as its size and associativity, cache type. See <a class="el" href="a00201.php" title="Object type-specific Attributes.">hwloc_obj_attr_u</a> for details.</p>
<h1><a class="anchor" id="attributes_info"></a>
Custom string infos</h1>
<p>Aside of these generic attribute fields, hwloc annotates many objects with info attributes made of name and value strings. Each object contains a list of such pairs that may be consulted manually (looking at the object <code>infos</code> array field) or using the <a class="el" href="a00145.php#ga415c4e44beada7ea34cd4bad9944b170" title="Search the given name in object infos and return the corresponding value.">hwloc_obj_get_info_by_name()</a>. The user may additionally add new name-value pairs to any object using <a class="el" href="a00145.php#gace7654bb8a9002caae1a4b8a59e7452e" title="Add the given name and value pair to the given object info attributes.">hwloc_obj_add_info()</a> or the <a class="el" href="a00345.php#cli_hwloc_annotate">hwloc-annotate</a> program.</p>
...
<dd><p class="startdd">Intel Xeon Phi processor configuration modes. Available if hwloc-dump-hwdata was used (see <a class="el" href="a00359.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for memory on Intel Xeon Phi processor?</a>) or if hwloc managed to guess them from the NUMA configuration.</p>
...
<dt>MemoryTiersNr </dt>
<dd>The number of different memory tiers in the topology, if any. See <a class="el" href="a00352.php">Heterogeneous Memory</a>.  </dd>
...
<dd>The rank of the memory tier of this node. Ranks start from 0 for highest bandwidth nodes. The attribute is only set if multiple tiers are found. See <a class="el" href="a00352.php">Heterogeneous Memory</a>.  </dd>
