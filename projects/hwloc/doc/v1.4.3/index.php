...
<p>Since it uses standard Operating System information, hwloc's support is mostly independant from the processor type (x86, powerpc, ...) and just relies on the Operating System support. The only exception to this is kFreeBSD, which does not support topology information, and hwloc thus uses an x86-only CPUID-based backend (which could be used for other OSes too).</p>
<p>To check whether hwloc works on a particular machine, just try to build it and run <code>lstopo</code>. If some things do not look right (e.g. bogus or missing cache information), see <a class="el" href="index.php#bugs">Questions and Bugs</a> below.</p>
...
<p>Perl bindings are available from Bernd Kallies <a href="http://search.cpan.org/~bka/Sys-Hwloc-0.10/">on CPAN:</a></p>
...
<div class="fragment"><div class="line">shell$ svn checkout http:<span class="comment">//svn.open-mpi.org/svn/hwloc/trunk hwloc-trunk</span></div>
<div class="line">shell$ cd hwloc-trunk</div>
<div class="line">shell$ ./autogen.sh</div>
</div><!-- fragment --><p>Note that GNU Autoconf &gt;=2.63, Automake &gt;=1.10 and Libtool &gt;=2.2.6 are required when building from a Subversion checkout.</p>
<p>Installation by itself is the fairly common GNU-based process:</p>
<div class="fragment"><div class="line">shell$ ./configure --prefix=...</div>
<div class="line">shell$ make</div>
<div class="line">shell$ make install</div>
</div><!-- fragment --><p>The hwloc command-line tool "lstopo" produces human-readable topology maps, as mentioned above. It can also export maps to the "fig" file format. Support for PDF, Postscript, and PNG exporting is provided if the "Cairo" development package can be found when hwloc is configured and build.</p>
<p>The hwloc core may also benefit from the following development packages: </p>
<ul>
<li>
libnuma for memory binding and migration support on Linux. </li>
<li>
libxml2 for full XML import/export support (otherwise, the internal minimalistic parser will only be able to import XML files that were exported by the same hwloc release). See <a class="el" href="a00007.php">Importing and exporting topologies from/to XML files</a> for details. </li>
<li>
hwloc can use one of two different libraries for I/O device discovery: <ol>
<li>
libpciaccess (BSD). </li>
<li>
libpci, from the pciutils package (GPL). </li>
</ol>
</li>
</ul>
...
<p>The basic interface is available in <a class="el" href="a00033_source.php" title="The hwloc API.">hwloc.h</a>. It essentially offers low-level routines for advanced programmers that want to manually manipulate objects and follow links between them. Documentation for everything in <a class="el" href="a00033_source.php" title="The hwloc API.">hwloc.h</a> are provided later in this document. Developers should also look at <a class="el" href="a00031_source.php" title="High-level hwloc traversal helpers.">hwloc/helper.h</a> (and also in this document, which provides good higher-level topology traversal examples).</p>
<p>To precisely define the vocabulary used by hwloc, a <a class="el" href="a00001.php">Terms and Definitions</a> section is available and should probably be read first.</p>
<p>Each hwloc object contains a cpuset describing the list of processing units that it contains. These bitmaps may be used for <a class="el" href="a00049.php">CPU binding</a> and <a class="el" href="a00050.php">Memory binding</a>. hwloc offers an extensive bitmap manipulation interface in <a class="el" href="a00027_source.php" title="The bitmap API, for use in hwloc itself.">hwloc/bitmap.h</a>.</p>
<p>Moreover, hwloc also comes with additional helpers for interoperability with several commonly used environments. See the <a class="el" href="a00008.php">Interoperability With Other Software</a> section for details.</p>
...
<div class="line"></div>
<div class="line"><span class="keyword">static</span> <span class="keywordtype">void</span> print_children(<a class="code" href="a00039.php#ga9d1e76ee15a7dee158b786c30b6a6e38" title="Topology context.">hwloc_topology_t</a> topology, <a class="code" href="a00016.php" title="Structure of a topology object.">hwloc_obj_t</a> obj, </div>
...
<div class="line">    <span class="keywordflow">for</span> (i = 0; i &lt; obj-&gt;<a class="code" href="a00016.php#aac3f6da35c9b57599909a44ce2b716c1" title="Number of children.">arity</a>; i++) {</div>
<div class="line">        print_children(topology, obj-&gt;<a class="code" href="a00016.php#a04d05403da37bfe17cd63b7c7dd07b1f" title="Children, children[0 .. arity -1].">children</a>[i], depth + 1);</div>
<div class="line">    }</div>
<div class="line">}</div>
<div class="line"></div>
...
<div class="line">    <a class="code" href="a00039.php#ga9d1e76ee15a7dee158b786c30b6a6e38" title="Topology context.">hwloc_topology_t</a> topology;</div>
<div class="line">    <a class="code" href="a00040.php#ga4bbf39b68b6f568fb92739e7c0ea7801" title="A CPU set is a bitmap whose bits are set according to CPU physical OS indexes.">hwloc_cpuset_t</a> cpuset;</div>
<div class="line">    <a class="code" href="a00016.php" title="Structure of a topology object.">hwloc_obj_t</a> obj;</div>
<div class="line"></div>
<div class="line">    <span class="comment">/* Allocate and initialize topology object. */</span></div>
<div class="line">    <a class="code" href="a00043.php#ga5c2d6f476af87005c7bd0811d4548b9f" title="Allocate a topology context.">hwloc_topology_init</a>(&amp;topology);</div>
<div class="line"></div>
...
<div class="line">    <a class="code" href="a00043.php#ga91e2e6427b95fb7339c99dbbef996e71" title="Build the actual topology.">hwloc_topology_load</a>(topology);</div>
<div class="line"></div>
...
<div class="line">    topodepth = <a class="code" href="a00046.php#ga8c30b0cec55074eb3ed34e4f2a1a9937" title="Get the depth of the hierarchical tree of objects.">hwloc_topology_get_depth</a>(topology);</div>
<div class="line"></div>
<div class="line">    <span class="comment">/*****************************************************************</span></div>
...
<div class="line">        <span class="keywordflow">for</span> (i = 0; i &lt; <a class="code" href="a00046.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth); </div>
<div class="line">             i++) {</div>
<div class="line">            <a class="code" href="a00048.php#ga5c6a61a83f4790b421e2f62e9088446f" title="Stringify a given topology object into a human-readable form.">hwloc_obj_snprintf</a>(<span class="keywordtype">string</span>, <span class="keyword">sizeof</span>(<span class="keywordtype">string</span>), topology,</div>
<div class="line">                       <a class="code" href="a00047.php#gaedd78240b0c1108355586a268ec5a697" title="Returns the topology object at logical index idx from depth depth.">hwloc_get_obj_by_depth</a>(topology, depth, i),</div>
...
<div class="line">    print_children(topology, <a class="code" href="a00053.php#gadbf58f6e187efbdb3cd9a8e30311b7d7" title="Returns the top-object of the topology-tree.">hwloc_get_root_obj</a>(topology), 0);</div>
<div class="line"></div>
<div class="line">    <span class="comment">/*****************************************************************</span></div>
...
<div class="line"><span class="comment">     *****************************************************************/</span></div>
<div class="line">    depth = <a class="code" href="a00046.php#gaea7c64dd59467f5201ba87712710b14d" title="Returns the depth of objects of type type.">hwloc_get_type_depth</a>(topology, <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55a1ac6e07775ae4324b3fe9dbd72c785ec" title="Socket, physical package, or chip. In the physical meaning, i.e. that you can add or remove physicall...">HWLOC_OBJ_SOCKET</a>);</div>
<div class="line">    <span class="keywordflow">if</span> (depth == <a class="code" href="a00046.php#ggaf4e663cf42bbe20756b849c6293ef575a0565ab92ab72cb0cec91e23003294aad" title="No object of given type exists in the topology.">HWLOC_TYPE_DEPTH_UNKNOWN</a>) {</div>
...
<div class="line">               <a class="code" href="a00046.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth));</div>
<div class="line">    }</div>
<div class="line"></div>
<div class="line">    <span class="comment">/*****************************************************************</span></div>
...
<div class="line">    <span class="keywordflow">for</span> (obj = <a class="code" href="a00047.php#ga9be4a03488cdd0fb431e4aa1cbdea895" title="Returns the topology object at logical index idx with type type.">hwloc_get_obj_by_type</a>(topology, <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55abca6887e80cb291353b0a0c1da83f661" title="Processing Unit, or (Logical) Processor. An execution unit (may share a core with some other logical ...">HWLOC_OBJ_PU</a>, 0);</div>
<div class="line">         obj;</div>
<div class="line">         obj = obj-&gt;<a class="code" href="a00016.php#adc494f6aed939992be1c55cca5822900" title="Parent, NULL if root (system object)">parent</a>)</div>
<div class="line">      <span class="keywordflow">if</span> (obj-&gt;<a class="code" href="a00016.php#acc4f0803f244867e68fe0036800be5de" title="Type of object.">type</a> == <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55a56ee0b7eca88f363b75b34fdde8c9ddc" title="Data cache. Can be L1, L2, L3, ...">HWLOC_OBJ_CACHE</a>) {</div>
<div class="line">        levels++;</div>
<div class="line">        size += obj-&gt;<a class="code" href="a00016.php#accd40e29f71f19e88db62ea3df02adc8" title="Object type-specific Attributes, may be NULL if no attribute value was found.">attr</a>-&gt;<a class="code" href="a00017.php#ab5a8ae3bf490e6b1071fea53f7382836">cache</a>.<a class="code" href="a00013.php#abe5e788943ed04302976740c829674c0" title="Size of cache in bytes.">size</a>;</div>
<div class="line">      }</div>
...
<div class="line"><span class="comment">     *****************************************************************/</span></div>
<div class="line">    depth = <a class="code" href="a00052.php#ga081be77905201e9f42318e9974456b45" title="Returns the depth of objects of type type or below.">hwloc_get_type_or_below_depth</a>(topology, <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55ac793958f330bca371aa1535de8aff45f" title="Core. A computation unit (may be shared by several logical processors).">HWLOC_OBJ_CORE</a>);</div>
<div class="line"></div>
<div class="line">    <span class="comment">/* Get last core. */</span></div>
<div class="line">    obj = <a class="code" href="a00047.php#gaedd78240b0c1108355586a268ec5a697" title="Returns the topology object at logical index idx from depth depth.">hwloc_get_obj_by_depth</a>(topology, depth,</div>
<div class="line">                   <a class="code" href="a00046.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth) - 1);</div>
...
<div class="line">        cpuset = <a class="code" href="a00065.php#gaaa4ed76211cd3694dfbea2109fc440be" title="Duplicate bitmap bitmap by allocating a new bitmap and copying bitmap contents.">hwloc_bitmap_dup</a>(obj-&gt;<a class="code" href="a00016.php#a67925e0f2c47f50408fbdb9bddd0790f" title="CPUs covered by this object.">cpuset</a>);</div>
<div class="line"></div>
...
<div class="line">        <a class="code" href="a00065.php#ga4630aa1b7e08eac5b41be126194e84a1" title="Keep a single index among those set in bitmap bitmap.">hwloc_bitmap_singlify</a>(cpuset);</div>
<div class="line"></div>
<div class="line">        <span class="comment">/* And try to bind ourself there. */</span></div>
<div class="line">        <span class="keywordflow">if</span> (<a class="code" href="a00049.php#gaf4cc194d5c0d38004a21b9f03522a7e3" title="Bind current process or thread on cpus given in physical bitmap set.">hwloc_set_cpubind</a>(topology, cpuset, 0)) {</div>
<div class="line">            <span class="keywordtype">char</span> *str;</div>
<div class="line">            <span class="keywordtype">int</span> error = errno;</div>
<div class="line">            <a class="code" href="a00065.php#gad3cf87ceb58aa91656756bbb58057320" title="Stringify a bitmap into a newly allocated string.">hwloc_bitmap_asprintf</a>(&amp;str, obj-&gt;<a class="code" href="a00016.php#a67925e0f2c47f50408fbdb9bddd0790f" title="CPUs covered by this object.">cpuset</a>);</div>
...
<div class="line">        <a class="code" href="a00065.php#ga8e7035fe555ef96921bfb98e08519bc7" title="Free bitmap bitmap.">hwloc_bitmap_free</a>(cpuset);</div>
<div class="line">    }</div>
<div class="line"></div>
<div class="line">    <span class="comment">/*****************************************************************</span></div>
...
<div class="line">    n = <a class="code" href="a00046.php#gaba821f84ef64282d14577066e6d6547e" title="Returns the width of level type type.">hwloc_get_nbobjs_by_type</a>(topology, <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55aaf0964881117bdedf1a5e9332cd120dd" title="NUMA node. A set of processors around memory which the processors can directly access.">HWLOC_OBJ_NODE</a>);</div>
...
<div class="line"></div>
<div class="line">        obj = <a class="code" href="a00047.php#ga9be4a03488cdd0fb431e4aa1cbdea895" title="Returns the topology object at logical index idx with type type.">hwloc_get_obj_by_type</a>(topology, <a class="code" href="a00041.php#ggacd37bb612667dc437d66bfb175a8dc55aaf0964881117bdedf1a5e9332cd120dd" title="NUMA node. A set of processors around memory which the processors can directly access.">HWLOC_OBJ_NODE</a>, n - 1);</div>
<div class="line">        m = <a class="code" href="a00050.php#gaeaa00714a9c4319bda0a74ca6f8720e8" title="Allocate some memory on the given physical nodeset nodeset.">hwloc_alloc_membind_nodeset</a>(topology, size, obj-&gt;<a class="code" href="a00016.php#a08f0d0e16c619a6e653526cbee4ffea3" title="NUMA nodes covered by this object or containing this object.">nodeset</a>,</div>
...
<div class="line">        <a class="code" href="a00050.php#ga986d9b4cc76da592c4b937c6cb7d9d56" title="Free memory that was previously allocated by hwloc_alloc() or hwloc_alloc_membind().">hwloc_free</a>(topology, m, size);</div>
<div class="line"></div>
<div class="line">        m = malloc(size);</div>
<div class="line">        <a class="code" href="a00050.php#gade5e2c28ea8475a479bf2b1df36c6ccd" title="Bind the already-allocated memory identified by (addr, len) to the NUMA node(s) in physical nodeset...">hwloc_set_area_membind_nodeset</a>(topology, m, size, obj-&gt;<a class="code" href="a00016.php#a08f0d0e16c619a6e653526cbee4ffea3" title="NUMA nodes covered by this object or containing this object.">nodeset</a>,</div>
<div class="line">                <a class="code" href="a00050.php#ggac9764f79505775d06407b40f5e4661e8a18675bb80ebc1bce5b652e9de8f3998c" title="Reset the memory allocation policy to the system default.">HWLOC_MEMBIND_DEFAULT</a>, 0);</div>
<div class="line">        free(m);</div>
<div class="line">    }</div>
<div class="line"></div>
<div class="line">    <span class="comment">/* Destroy topology object. */</span></div>
<div class="line">    <a class="code" href="a00043.php#ga6040925d3ee4bbb2647f2a321aca5f4b" title="Terminate and free a topology context.">hwloc_topology_destroy</a>(topology);</div>
<div class="line"></div>
...
<ul>
<li>
<a class="el" href="a00001.php">Terms and Definitions</a> </li>
<li>
<a class="el" href="a00002.php">Command-Line Tools</a> </li>
<li>
<a class="el" href="a00003.php">Environment Variables</a> </li>
<li>
<a class="el" href="a00004.php">CPU and Memory Binding Overview</a> </li>
<li>
<a class="el" href="a00005.php">I/O Devices</a> </li>
<li>
<a class="el" href="a00006.php">Multi-node Topologies</a> </li>
<li>
<a class="el" href="a00007.php">Importing and exporting topologies from/to XML files</a> </li>
<li>
<a class="el" href="a00008.php">Interoperability With Other Software</a> </li>
<li>
<a class="el" href="a00009.php">Thread Safety</a> </li>
<li>
<a class="el" href="a00010.php">Embedding hwloc in Other Software</a> </li>
<li>
<a class="el" href="a00011.php">Frequently Asked Questions</a> </li>
</ul>
