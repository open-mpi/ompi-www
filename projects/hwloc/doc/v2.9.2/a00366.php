...
<p >hwloc objects have many generic attributes in the <a class="el" href="a00218.php" title="Structure of a topology object.">hwloc_obj</a> structure, for instance their <code>logical_index</code> or <code>os_index</code> (see <a class="el" href="a00374.php#faq_indexes">Should I use logical or physical/OS indexes? and how?</a>), <code>depth</code> or <code>name</code>.</p>
...
<li>
Groups: subtype <code>Cluster</code>, <code>Module</code>, <code>Tile</code>, <code>Compute Unit</code>, <code>Book</code> or <code>Drawer</code> for different architecture-specific groups of CPUs (see also <a class="el" href="a00374.php#faq_groups">What are these Group objects in my topology?</a>).  </li>
<li>
OS devices (see also <a class="el" href="a00364.php#iodevices_osdev">OS devices</a>): <ul>
<li>
...
<li>
PCI devices: subtype <code>NVSwitch</code> for NVLink switches (see also NVLinkBandwidth in <a class="el" href="a00367.php#topoattrs_distances">Distances</a>).  </li>
<li>
Misc devices: subtype <code>MemoryModule</code> (see also <a class="el" href="a00365.php#miscobjs_auto">Misc objects added by hwloc</a>)  </li>
</ul>
<p >Each object also contains an <code>attr</code> field that, if non NULL, points to a union <a class="el" href="a00222.php" title="Object type-specific Attributes.">hwloc_obj_attr_u</a> of type-specific attribute structures. For instance, a L2Cache object <code>obj</code> contains cache-specific information in <code>obj-&gt;attr-&gt;cache</code>, such as its size and associativity, cache type. See <a class="el" href="a00222.php" title="Object type-specific Attributes.">hwloc_obj_attr_u</a> for details.</p>
<p > 
</div><div class="section" id="attributes_info">
 </p>
<h1><a class="anchor" id="attributes_info"></a>
Custom string infos</h1>
<p >Aside os these generic attribute fields, hwloc annotates many objects with string attributes that are made of a key and a value. Each object contains a list of such pairs that may be consulted manually (looking at the object <code>infos</code> array field) or using the <a class="el" href="a00166.php#ga415c4e44beada7ea34cd4bad9944b170" title="Search the given key name in object infos and return the corresponding value.">hwloc_obj_get_info_by_name()</a>. The user may additionally add new key-value pairs to any object using <a class="el" href="a00166.php#gace7654bb8a9002caae1a4b8a59e7452e" title="Add the given info name and value pair to the given object.">hwloc_obj_add_info()</a> or the <a class="el" href="a00361.php#cli_hwloc_annotate">hwloc-annotate</a> program.</p>
...
<dd><p class="startdd">Intel Xeon Phi processor configuration modes. Available if hwloc-dump-hwdata was used (see <a class="el" href="a00374.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for memory on Intel Xeon Phi processor?</a>) or if hwloc managed to guess them from the NUMA configuration.</p>
