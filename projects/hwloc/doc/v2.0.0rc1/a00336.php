...
<p>If any configuration function such as <a class="el" href="a00168.php#ga879439b7ee99407ee911b3ac64e9a25e" title="Enable XML-file based topology. ">hwloc_topology_set_xml()</a> is used before loading the topology, the corresponding component is enabled first. Then, as usual, hwloc enables any other component (based on priorities) that does not conflict.</p>
<p>Certain components that manage a virtual topology, for instance XML topology import or synthetic topology description, conflict with all other components. Therefore, one of them may only be loaded (e.g. with <code><a class="el" href="a00168.php#ga879439b7ee99407ee911b3ac64e9a25e" title="Enable XML-file based topology. ">hwloc_topology_set_xml()</a></code>) if no other component is enabled.</p>
...
<p>If no topology configuration functions such as <code><a class="el" href="a00168.php#ga4fab186bb6181a00bcf585825fddd38d" title="Enable synthetic topology. ">hwloc_topology_set_synthetic()</a></code> have been called, plugins may be selected with environment variables such as <code>HWLOC_XMLFILE</code>, <code>HWLOC_SYNTHETIC</code> or <code>HWLOC_FSROOT</code> (see <a class="el" href="a00327.php">Environment Variables</a>).</p>
...
<p>Plugins are built as independent dynamic libraries that are installed in <code>$libdir/hwloc</code>. All plugins found in this directory are loaded during <code>topology_init()</code> (unless blacklisted in <code>HWLOC_PLUGINS_BLACKLIST</code>, see <a class="el" href="a00327.php">Environment Variables</a>). A specific list of directories (colon-separated) to scan may be specified in the <code>HWLOC_PLUGINS_PATH</code> environment variable.</p>
...
<p>Also note that plugins should carefully be enabled and used when embedding hwloc in another project, see <a class="el" href="a00337.php">Embedding hwloc in Other Software</a> for details.</p>
<h1><a class="anchor" id="plugins_adding"></a>
Adding new discovery components and plugins</h1>
<p>The types and functions cited below are declared in the <a class="el" href="a00152_source.php" title="Public interface for building hwloc plugins. ">hwloc/plugins.h</a> header. Components are supposed to only use hwloc public headers (<a class="el" href="a00101_source.php" title="The hwloc API. ">hwloc.h</a> and anything under the <code>include/hwloc</code> subdirectory) and nothing from the <code>include/private</code> subdirectory in the source tree.</p>
<h2><a class="anchor" id="plugins_disc_basic"></a>
Basics of discovery components</h2>
<p>Each discovery component is defined by a <code><a class="el" href="a00315.php" title="Discovery component structure. ">hwloc_disc_component</a></code> structure which contains an <code>instantiate()</code> callback. This function is invoked when this component is actually used by a topology. It fills a new <code><a class="el" href="a00319.php" title="Discovery backend structure. ">hwloc_backend</a></code> structure that usually contains a <code>discover()</code> callback taking care of the actual topology discovery.</p>
...
<p>Registering components to the hwloc core relies on a <code><a class="el" href="a00323.php" title="Generic component structure. ">hwloc_component</a></code> structure. Its <code>data</code> field points to the previously defined <code><a class="el" href="a00315.php" title="Discovery component structure. ">hwloc_disc_component</a></code> structure while its <code>type</code> should be <a class="el" href="a00202.php#gga397a1bf7d21dd073320ad0045340f463a5d6e561f467fe9795a29e7368b613900" title="The data field must point to a struct hwloc_disc_component. ">HWLOC_COMPONENT_TYPE_DISC</a>. This structure should be named <code>hwloc_&lt;name&gt;_component</code>.</p>
...
<dl class="section note"><dt>Note</dt><dd>The symbol name of the <code><a class="el" href="a00323.php" title="Generic component structure. ">hwloc_component</a></code> structure is independent of the name of the discovery component mentioned in the previous section.</dd></dl>
...
<p>All components distributed within hwloc are listed below. The list of actually available components may be listed at running with the <code>HWLOC_COMPONENTS_VERBOSE</code> environment variable (see <a class="el" href="a00327.php">Environment Variables</a>).</p>
<dl>
...
<dt>bgq </dt>
<dd>This component is specific to IBM BlueGene/Q compute node (running CNK). It is built and enabled by default when <code>--host=powerpc64-bgq-linux</code> is passed to configure (see <a class="el" href="a00338.php#faq_bgq">How do I build hwloc for BlueGene/Q?</a>).  </dd>
<dt>no_os </dt>
...
<dt>pci </dt>
<dd>PCI object discovery uses the external pciaccess library (aka libpciaccess); see <a class="el" href="a00329.php">I/O Devices</a>. <b>It may be built as a plugin</b>.  </dd>
...
<dt>synthetic </dt>
<dd>Synthetic topology support (see <a class="el" href="a00333.php">Synthetic topologies</a>) is always built statically.  </dd>
<dt>xml </dt>
<dd>XML topology import (see <a class="el" href="a00332.php">Importing and exporting topologies from/to XML files</a>) is always built statically. It internally uses one of the XML backends (see <a class="el" href="a00332.php#xml_backends">libxml2 and minimalistic XML backends</a>). <ul>
<li>
