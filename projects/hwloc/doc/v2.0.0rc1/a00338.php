...
<p>For more information about the distance API, see <a class="el" href="a00184.php">Retrieve distances between objects</a> and <a class="el" href="a00186.php">Add or remove distances between objects</a>.</p>
...
<p>To understand how hwloc manages such cases, one should first remember the meaning of levels and cousin objects. All objects of the same type are gathered as horizontal levels with a given depth. They are also connected through the cousin pointers of the <a class="el" href="a00211.php" title="Structure of a topology object. ">hwloc_obj</a> structure. Object attribute (cache depth and type, group depth) are also taken in account when gathering objects as horizontal levels. To be clear: there will be one level for L1i caches, another level for L1d caches, another one for L2, etc.</p>
...
<p>Shared-memory topologies consist in one process exposing its topology in a shared-memory buffer so that other processes (running on the same machine) may use it directly. This has the advantage of reducing the memory footprint since a single topology is stored in physical memory for multiple processes. However it requires all processes to map this shared-memory buffer at the same virtual address, which may be difficult in some cases. This API is described in <a class="el" href="a00199.php">Sharing topologies between processes</a>.</p>
...
<p>The utility <code>hwloc-dump-hwdata</code> is also involved in gathering privileged information at boot time and making it available to non-privileged users (note that this may require a specific SELinux MLS policy module). However it only applies to Intel Knights Landing Xeon Phi for now (see <a class="el" href="a00338.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for memory on Intel Knights Landing Xeon Phi?</a>). See also <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00327.php">Environment Variables</a> for details about the location of dumped files.</p>
...
#endif
</pre><p>You should not try to remain compatible with very old releases such as 1.1.x or earlier because <code><a class="el" href="a00158.php#ga8f4dfb8eef138af55dd1a0fa802e5476" title="Indicate at build time which hwloc API version is being used. ">HWLOC_API_VERSION</a></code> was added in 1.0.0 and <code><a class="el" href="a00158.php#ga9c0b50c98add1adf57ed1ce85bb5190d" title="Indicate at runtime which hwloc API version was used at build time. ">hwloc_get_api_version()</a></code> came only in 1.1.1. Also do not use the old cpuset API since it was deprecated and superseded by the bitmap API in 1.1, and later removed in 1.5.</p>
...
<p>Intel Knights Landing Xeon Phi processors introduce a new memory architecture by possibly having two distinct local memories: some normal memory (DDR) and some high-bandwidth on-package memory (MCDRAM). Processors can be configured in various clustering modes to have up to 4 <em>Clusters</em>. Moreover, each <em>Cluster</em> (quarter, half or whole processor) of the processor may have its own local parts of the DDR and of the MCDRAM. This memory and clustering configuration may be probed by looking at MemoryMode and ClusterMode attributes, see <a class="el" href="a00331.php#attributes_info">Custom string infos</a> and doc/examples/get-knl-modes.c in the source directory.</p>
