...
</div><!-- fragment --><p>This will allocate some memory hopefully bound to the specified set. To do so, hwloc will possibly have to change the current memory binding policy in order to actually get the memory bound, if the OS does not provide any other way to simply allocate bound memory without changing the policy for all allocations. That is the difference with <a class="el" href="a00081.php#ga1058b0b9216bc6fd9c3734ea9dca1e54" title="Allocate some memory on memory nodes near the given physical cpuset cpuset. ">hwloc_alloc_membind()</a>, which will never change the current memory binding policy. Note that since HWLOC_MEMBIND_STRICT was not specified, failures to bind will not be reported &ndash; generally, only memory allocation failures will be reported (e.g., even a plain malloc() would have failed with ENOMEM).</p>
<p>Each hwloc memory binding function is available in two forms: one that takes a CPU set argument and another that takes a NUMA memory node set argument (see <a class="el" href="a00073.php">Object Sets (hwloc_cpuset_t and hwloc_nodeset_t)</a> and <a class="el" href="a00095.php">The bitmap API</a> for a discussion of CPU sets and NUMA memory node sets). The names of the latter form end with _nodeset. It is also possible to convert between CPU set and node set using <a class="el" href="a00092.php#ga8fbd641f284673a3e884e8e556044eba" title="Convert a CPU set into a NUMA node set and handle non-NUMA cases. ">hwloc_cpuset_to_nodeset()</a> or <a class="el" href="a00092.php#ga2ed57313ce5bf85f3ac156932ecf639d" title="Convert a NUMA node set into a CPU set and handle non-NUMA cases. ">hwloc_cpuset_from_nodeset()</a>.</p>
