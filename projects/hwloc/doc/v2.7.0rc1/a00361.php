...
<dt>HWLOC_CPUKINDS_MAXFREQ=0 </dt>
<dd><p class="startdd">change the use of the max frequency in the Linux backend. hwloc tries to read the base and max frequencies of each core on Linux. If base frequencies are found, max is ignored by default because max frequencies are less reliable for identifying hybrid CPUs due to features such as Intel Turbo Boost Max 3.0 (some cores may report slightly higher max frequencies than others in the same CPU package).</p>
<p class="interdd">If this variable is set to 1, the max frequency is not ignored anymore, it is exposed and some processors may appear hybrid because of this. </p>
<p class="enddd"></p>
</dd>
...
<p class="interdd">See <a class="el" href="a00371.php#plugins_select">Selecting which components to use</a> for details. </p>
<p class="enddd"></p>
</dd>
