...
<p> 
<div class="section">
 See also <a class="el" href="a00358.php#further_reading">Further Reading</a>  
...
<p>Since it uses standard Operating System information, hwloc's support is mostly independant from the processor type (x86, powerpc, ...) and just relies on the Operating System support. The main exception is BSD operating systems (NetBSD, FreeBSD, etc.) because they do not provide support topology information, hence hwloc uses an x86-only CPUID-based backend (which can be used for other OSes too, see the <a class="el" href="a00371.php">Components and plugins</a> section).</p>
...
<ul>
<li>
Symmetrical tree of resources generated from a list of level arities, see <a class="el" href="a00368.php">Synthetic topologies</a>. </li>
<li>
Remote machine simulation through the gathering of topology as XML files, see <a class="el" href="a00367.php">Importing and exporting topologies from/to XML files</a>. </li>
</ul>
<p>hwloc can display the topology in a human-readable format, either in graphical mode (X11), or by exporting in one of several different formats, including: plain text, LaTeX tikzpicture, PDF, PNG, and FIG (see <a class="el" href="a00358.php#cli_examples">Command-line Examples</a> below). Note that some of the export formats require additional support libraries.</p>
<p>hwloc offers a programming interface for manipulating topologies and objects. It also brings a powerful CPU bitmap API that is used to describe topology objects location on physical/logical processors. See the <a class="el" href="a00358.php#interface">Programming Interface</a> below. It may also be used to binding applications onto certain cores or memory nodes. Several utility programs are also provided to ease command-line manipulation of topology objects, binding of processes, and so on.</p>
...
<li>
the NVIDIA CUDA Toolkit for CUDA device discovery. See <a class="el" href="a00373.php#faq_cuda_build">How do I enable CUDA and select which CUDA version to use?</a>.  </li>
<li>
...
<li>
the AMD ROCm SMI library for RSMI device discovery. The relevant development package is usually <code>rocm-smi-lib64</code> or <code>librocm-smi-dev</code>. See <a class="el" href="a00373.php#faq_rocm_build">How do I enable ROCm SMI and select which version to use?</a>.  </li>
<li>
...
<p>PCI and XML support may be statically built inside the main hwloc library, or as separate dynamically-loaded plugins (see the <a class="el" href="a00371.php">Components and plugins</a> section).</p>
...
<pre class="fragment">Machine
  ...
  Package L#3 + L3 L#3 (4096KB)
    ...
    L2 L#7 (1024KB) + L1 L#7 (16KB) + Core L#7
      PU L#14 (P#7)
      PU L#15 (P#15)
</pre><p>Note that there is also an equivalent output in XML that is meant for exporting/importing topologies but it is hardly readable to human-beings (see <a class="el" href="a00367.php">Importing and exporting topologies from/to XML files</a> for details).</p>
...
<p>To precisely define the vocabulary used by hwloc, a <a class="el" href="a00359.php">Terms and Definitions</a> section is available and should probably be read first.</p>
<p>Each hwloc object contains a cpuset describing the list of processing units that it contains. These bitmaps may be used for <a class="el" href="a00166.php">CPU binding</a> and <a class="el" href="a00167.php">Memory binding</a>. hwloc offers an extensive bitmap manipulation interface in <a class="el" href="a00095_source.php">hwloc/bitmap.h</a>.</p>
<p>Moreover, hwloc also comes with additional helpers for interoperability with several commonly used environments. See the <a class="el" href="a00369.php">Interoperability With Other Software</a> section for details.</p>
...
<img src="ppc64-without-smt.png" alt=""/>
 ...
 <p>Notice that hwloc only sees half the PUs when SMT is disabled. PU L#6, for example, seems to change location from NUMA node #0 to #1. In reality, no PUs "moved" &ndash; they were simply re-numbered when hwloc only saw half as many (see also Logical index in <a class="el" href="a00359.php#termsanddefs_indexes">Indexes and Sets</a>). Hence, PU L#6 in the SMT-disabled picture probably corresponds to PU L#12 in the SMT-enabled picture.</p>
...
<div class="line"> </div>
<div class="line"><span class="keyword">static</span> <span class="keywordtype">void</span> print_children(<a class="code" href="a00162.php#ga9d1e76ee15a7dee158b786c30b6a6e38">hwloc_topology_t</a> topology, <a class="code" href="a00217.php">hwloc_obj_t</a> obj,</div>
...
<div class="line"> </div>
<div class="line">    <a class="code" href="a00164.php#gadb8765c260edea80c52cd06a76639ba4">hwloc_obj_type_snprintf</a>(type, <span class="keyword">sizeof</span>(type), obj, 0);</div>
<div class="line">    printf(<span class="stringliteral">&quot;%*s%s&quot;</span>, 2*depth, <span class="stringliteral">&quot;&quot;</span>, type);</div>
<div class="line">    <span class="keywordflow">if</span> (obj-&gt;<a class="code" href="a00217.php#a61a7a80a68eaccbaaa28269e678c81a9">os_index</a> != (<span class="keywordtype">unsigned</span>) -1)</div>
<div class="line">      printf(<span class="stringliteral">&quot;#%u&quot;</span>, obj-&gt;<a class="code" href="a00217.php#a61a7a80a68eaccbaaa28269e678c81a9">os_index</a>);</div>
<div class="line">    <a class="code" href="a00164.php#ga870e876931c282a1c7aee2f031912ce3">hwloc_obj_attr_snprintf</a>(attr, <span class="keyword">sizeof</span>(attr), obj, <span class="stringliteral">&quot; &quot;</span>, 0);</div>
...
<div class="line">    <span class="keywordflow">for</span> (i = 0; i &lt; obj-&gt;<a class="code" href="a00217.php#aac3f6da35c9b57599909a44ce2b716c1">arity</a>; i++) {</div>
<div class="line">        print_children(topology, obj-&gt;<a class="code" href="a00217.php#a04d05403da37bfe17cd63b7c7dd07b1f">children</a>[i], depth + 1);</div>
<div class="line">    }</div>
<div class="line">}</div>
<div class="line"> </div>
...
<div class="line">    <a class="code" href="a00162.php#ga9d1e76ee15a7dee158b786c30b6a6e38">hwloc_topology_t</a> topology;</div>
<div class="line">    <a class="code" href="a00159.php#ga4bbf39b68b6f568fb92739e7c0ea7801">hwloc_cpuset_t</a> cpuset;</div>
<div class="line">    <a class="code" href="a00217.php">hwloc_obj_t</a> obj;</div>
<div class="line"> </div>
<div class="line">    <span class="comment">/* Allocate and initialize topology object. */</span></div>
<div class="line">    <a class="code" href="a00162.php#ga03fd4a16d8b9ee1ffc32b25fd2f6bdfa">hwloc_topology_init</a>(&amp;topology);</div>
<div class="line"> </div>
...
<div class="line">    <a class="code" href="a00162.php#gabdf58d87ad77f6615fccdfe0535ff826">hwloc_topology_load</a>(topology);</div>
<div class="line"> </div>
...
<div class="line">    topodepth = <a class="code" href="a00163.php#gae54d1782ca9b54bea915f5c18a9158fa">hwloc_topology_get_depth</a>(topology);</div>
<div class="line"> </div>
<div class="line"><span class="comment">    /*****************************************************************</span></div>
...
<div class="line">        <span class="keywordflow">for</span> (i = 0; i &lt; <a class="code" href="a00163.php#ga1d5ceafe8130fe6e8657bf0bc666ba50">hwloc_get_nbobjs_by_depth</a>(topology, depth);</div>
<div class="line">             i++) {</div>
<div class="line">            <a class="code" href="a00164.php#gadb8765c260edea80c52cd06a76639ba4">hwloc_obj_type_snprintf</a>(<span class="keywordtype">string</span>, <span class="keyword">sizeof</span>(<span class="keywordtype">string</span>),</div>
<div class="line">                                    <a class="code" href="a00163.php#ga391f6b2613f0065673eaa4069b93d4e0">hwloc_get_obj_by_depth</a>(topology, depth, i), 0);</div>
...
<div class="line">    print_children(topology, <a class="code" href="a00163.php#ga2d4b12fc187dfc53b35f2fa21d21044d">hwloc_get_root_obj</a>(topology), 0);</div>
<div class="line"> </div>
<div class="line"><span class="comment">    /*****************************************************************</span></div>
...
<div class="line"><span class="comment">     *****************************************************************/</span></div>
<div class="line">    depth = <a class="code" href="a00163.php#ga8bec782e21be313750da70cf7428b374">hwloc_get_type_depth</a>(topology, <a class="code" href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55ab16ab8c0dbffc234921d86f3dfb63129">HWLOC_OBJ_PACKAGE</a>);</div>
<div class="line">    <span class="keywordflow">if</span> (depth == <a class="code" href="a00163.php#ggaf4e663cf42bbe20756b849c6293ef575a0565ab92ab72cb0cec91e23003294aad">HWLOC_TYPE_DEPTH_UNKNOWN</a>) {</div>
...
<div class="line">               <a class="code" href="a00163.php#ga1d5ceafe8130fe6e8657bf0bc666ba50">hwloc_get_nbobjs_by_depth</a>(topology, depth));</div>
<div class="line">    }</div>
<div class="line"> </div>
<div class="line"><span class="comment">    /*****************************************************************</span></div>
...
<div class="line">    <span class="keywordflow">for</span> (obj = <a class="code" href="a00163.php#ga6f414dd80a2b943967a0ac92da3181a2">hwloc_get_obj_by_type</a>(topology, <a class="code" href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55abca6887e80cb291353b0a0c1da83f661">HWLOC_OBJ_PU</a>, 0);</div>
<div class="line">         obj;</div>
<div class="line">         obj = obj-&gt;<a class="code" href="a00217.php#adc494f6aed939992be1c55cca5822900">parent</a>)</div>
<div class="line">      <span class="keywordflow">if</span> (<a class="code" href="a00174.php#ga2ed589bea28711e80b92066510a5607d">hwloc_obj_type_is_cache</a>(obj-&gt;<a class="code" href="a00217.php#acc4f0803f244867e68fe0036800be5de">type</a>)) {</div>
<div class="line">        levels++;</div>
<div class="line">        size += obj-&gt;<a class="code" href="a00217.php#accd40e29f71f19e88db62ea3df02adc8">attr</a>-&gt;<a class="code" href="a00221.php#a23f553f3252c9d13f2338231cd354ea9">cache</a>.<a class="code" href="a00233.php#abe5e788943ed04302976740c829674c0">size</a>;</div>
<div class="line">      }</div>
...
<div class="line"><span class="comment">     *****************************************************************/</span></div>
<div class="line">    depth = <a class="code" href="a00163.php#ga8125328e69eba709c33ea8055c12589b">hwloc_get_type_or_below_depth</a>(topology, <a class="code" href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55ac793958f330bca371aa1535de8aff45f">HWLOC_OBJ_CORE</a>);</div>
<div class="line"> </div>
<div class="line">    <span class="comment">/* Get last core. */</span></div>
<div class="line">    obj = <a class="code" href="a00163.php#ga391f6b2613f0065673eaa4069b93d4e0">hwloc_get_obj_by_depth</a>(topology, depth,</div>
<div class="line">                   <a class="code" href="a00163.php#ga1d5ceafe8130fe6e8657bf0bc666ba50">hwloc_get_nbobjs_by_depth</a>(topology, depth) - 1);</div>
...
<div class="line">        cpuset = <a class="code" href="a00181.php#gae679434c1a5f41d3560a8a7e2c1b0dee">hwloc_bitmap_dup</a>(obj-&gt;<a class="code" href="a00217.php#a67925e0f2c47f50408fbdb9bddd0790f">cpuset</a>);</div>
<div class="line"> </div>
...
<div class="line">        <a class="code" href="a00181.php#gaa611a77c092e679246afdf9a60d5db8b">hwloc_bitmap_singlify</a>(cpuset);</div>
<div class="line"> </div>
<div class="line">        <span class="comment">/* And try to bind ourself there. */</span></div>
<div class="line">        <span class="keywordflow">if</span> (<a class="code" href="a00166.php#ga80bc07473a8edf840cae17bd7ec21d48">hwloc_set_cpubind</a>(topology, cpuset, 0)) {</div>
<div class="line">            <span class="keywordtype">char</span> *str;</div>
<div class="line">            <span class="keywordtype">int</span> error = errno;</div>
<div class="line">            <a class="code" href="a00181.php#ga0fece972134fdecf2da9bc7a11dd827e">hwloc_bitmap_asprintf</a>(&amp;str, obj-&gt;<a class="code" href="a00217.php#a67925e0f2c47f50408fbdb9bddd0790f">cpuset</a>);</div>
...
<div class="line">        <a class="code" href="a00181.php#ga156130d85b3a0674d6e0e6770fe68fbe">hwloc_bitmap_free</a>(cpuset);</div>
<div class="line">    }</div>
<div class="line"> </div>
<div class="line"><span class="comment">    /*****************************************************************</span></div>
...
<div class="line">    n = <a class="code" href="a00163.php#ga789a3f65aedff644be64a18526a03065">hwloc_get_nbobjs_by_type</a>(topology, <a class="code" href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55a9d917a3e5497950c6d8948b8e183db5a">HWLOC_OBJ_NUMANODE</a>);</div>
<div class="line">    obj = <a class="code" href="a00163.php#ga6f414dd80a2b943967a0ac92da3181a2">hwloc_get_obj_by_type</a>(topology, <a class="code" href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55a9d917a3e5497950c6d8948b8e183db5a">HWLOC_OBJ_NUMANODE</a>, n - 1);</div>
<div class="line"> </div>
<div class="line">    size = 1024*1024;</div>
<div class="line">    m = <a class="code" href="a00167.php#ga04736461780fadcf193af218c0122273">hwloc_alloc_membind</a>(topology, size, obj-&gt;<a class="code" href="a00217.php#a08f0d0e16c619a6e653526cbee4ffea3">nodeset</a>,</div>
<div class="line">                            <a class="code" href="a00167.php#ggac9764f79505775d06407b40f5e4661e8ad811fa4b2a6002c4d63695a408ffde2c">HWLOC_MEMBIND_BIND</a>, <a class="code" href="a00167.php#ggab00475fd98815bf4fb9aaf752030e7d2a71f19fe4505f1c083dc8e6f7bdea6256">HWLOC_MEMBIND_BYNODESET</a>);</div>
<div class="line">    <a class="code" href="a00167.php#ga32dbd4f54e9e4a7179f2dde37ffe6ad7">hwloc_free</a>(topology, m, size);</div>
<div class="line"> </div>
<div class="line">    m = malloc(size);</div>
<div class="line">    <a class="code" href="a00167.php#gaf881faefe20701229f07dd7dbd0125ed">hwloc_set_area_membind</a>(topology, m, size, obj-&gt;<a class="code" href="a00217.php#a08f0d0e16c619a6e653526cbee4ffea3">nodeset</a>,</div>
<div class="line">                           <a class="code" href="a00167.php#ggac9764f79505775d06407b40f5e4661e8ad811fa4b2a6002c4d63695a408ffde2c">HWLOC_MEMBIND_BIND</a>, <a class="code" href="a00167.php#ggab00475fd98815bf4fb9aaf752030e7d2a71f19fe4505f1c083dc8e6f7bdea6256">HWLOC_MEMBIND_BYNODESET</a>);</div>
<div class="line">    free(m);</div>
<div class="line"> </div>
<div class="line">    <span class="comment">/* Destroy topology object. */</span></div>
<div class="line">    <a class="code" href="a00162.php#ga9f34a640b6fd28d23699d4d084667b15">hwloc_topology_destroy</a>(topology);</div>
<div class="line"> </div>
...
<div class="ttc" id="aa00160_php_ggacd37bb612667dc437d66bfb175a8dc55ab16ab8c0dbffc234921d86f3dfb63129"><div class="ttname"><a href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55ab16ab8c0dbffc234921d86f3dfb63129">HWLOC_OBJ_PACKAGE</a></div><div class="ttdeci">@ HWLOC_OBJ_PACKAGE</div><div class="ttdoc">Physical package. The physical package that usually gets inserted into a socket on the motherboard....</div><div class="ttdef"><b>Definition:</b> hwloc.h:191</div></div>
<div class="ttc" id="aa00160_php_ggacd37bb612667dc437d66bfb175a8dc55abca6887e80cb291353b0a0c1da83f661"><div class="ttname"><a href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55abca6887e80cb291353b0a0c1da83f661">HWLOC_OBJ_PU</a></div><div class="ttdeci">@ HWLOC_OBJ_PU</div><div class="ttdoc">Processing Unit, or (Logical) Processor. An execution unit (may share a core with some other logical ...</div><div class="ttdef"><b>Definition:</b> hwloc.h:201</div></div>
<div class="ttc" id="aa00160_php_ggacd37bb612667dc437d66bfb175a8dc55ac793958f330bca371aa1535de8aff45f"><div class="ttname"><a href="a00160.php#ggacd37bb612667dc437d66bfb175a8dc55ac793958f330bca371aa1535de8aff45f">HWLOC_OBJ_CORE</a></div><div class="ttdeci">@ HWLOC_OBJ_CORE</div><div class="ttdoc">Core. A computation unit (may be shared by several PUs, aka logical processors).</div><div class="ttdef"><b>Definition:</b> hwloc.h:197</div></div>
<div class="ttc" id="aa00162_php_ga03fd4a16d8b9ee1ffc32b25fd2f6bdfa"><div class="ttname"><a href="a00162.php#ga03fd4a16d8b9ee1ffc32b25fd2f6bdfa">hwloc_topology_init</a></div><div class="ttdeci">int hwloc_topology_init(hwloc_topology_t *topologyp)</div><div class="ttdoc">Allocate a topology context.</div></div>
...
<div class="ttc" id="aa00162_php_ga9f34a640b6fd28d23699d4d084667b15"><div class="ttname"><a href="a00162.php#ga9f34a640b6fd28d23699d4d084667b15">hwloc_topology_destroy</a></div><div class="ttdeci">void hwloc_topology_destroy(hwloc_topology_t topology)</div><div class="ttdoc">Terminate and free a topology context.</div></div>
<div class="ttc" id="aa00162_php_gabdf58d87ad77f6615fccdfe0535ff826"><div class="ttname"><a href="a00162.php#gabdf58d87ad77f6615fccdfe0535ff826">hwloc_topology_load</a></div><div class="ttdeci">int hwloc_topology_load(hwloc_topology_t topology)</div><div class="ttdoc">Build the actual topology.</div></div>
<div class="ttc" id="aa00163_php_ga1d5ceafe8130fe6e8657bf0bc666ba50"><div class="ttname"><a href="a00163.php#ga1d5ceafe8130fe6e8657bf0bc666ba50">hwloc_get_nbobjs_by_depth</a></div><div class="ttdeci">unsigned hwloc_get_nbobjs_by_depth(hwloc_topology_t topology, int depth)</div><div class="ttdoc">Returns the width of level at depth depth.</div></div>
<div class="ttc" id="aa00163_php_ga2d4b12fc187dfc53b35f2fa21d21044d"><div class="ttname"><a href="a00163.php#ga2d4b12fc187dfc53b35f2fa21d21044d">hwloc_get_root_obj</a></div><div class="ttdeci">static hwloc_obj_t hwloc_get_root_obj(hwloc_topology_t topology)</div><div class="ttdoc">Returns the top-object of the topology-tree.</div></div>
<div class="ttc" id="aa00163_php_ga391f6b2613f0065673eaa4069b93d4e0"><div class="ttname"><a href="a00163.php#ga391f6b2613f0065673eaa4069b93d4e0">hwloc_get_obj_by_depth</a></div><div class="ttdeci">hwloc_obj_t hwloc_get_obj_by_depth(hwloc_topology_t topology, int depth, unsigned idx)</div><div class="ttdoc">Returns the topology object at logical index idx from depth depth.</div></div>
<div class="ttc" id="aa00163_php_ga6f414dd80a2b943967a0ac92da3181a2"><div class="ttname"><a href="a00163.php#ga6f414dd80a2b943967a0ac92da3181a2">hwloc_get_obj_by_type</a></div><div class="ttdeci">static hwloc_obj_t hwloc_get_obj_by_type(hwloc_topology_t topology, hwloc_obj_type_t type, unsigned idx)</div><div class="ttdoc">Returns the topology object at logical index idx with type type.</div></div>
<div class="ttc" id="aa00163_php_ga789a3f65aedff644be64a18526a03065"><div class="ttname"><a href="a00163.php#ga789a3f65aedff644be64a18526a03065">hwloc_get_nbobjs_by_type</a></div><div class="ttdeci">static int hwloc_get_nbobjs_by_type(hwloc_topology_t topology, hwloc_obj_type_t type)</div><div class="ttdoc">Returns the width of level type type.</div></div>
<div class="ttc" id="aa00163_php_ga8125328e69eba709c33ea8055c12589b"><div class="ttname"><a href="a00163.php#ga8125328e69eba709c33ea8055c12589b">hwloc_get_type_or_below_depth</a></div><div class="ttdeci">static int hwloc_get_type_or_below_depth(hwloc_topology_t topology, hwloc_obj_type_t type)</div><div class="ttdoc">Returns the depth of objects of type type or below.</div></div>
...
<div class="ttc" id="aa00163_php_gae54d1782ca9b54bea915f5c18a9158fa"><div class="ttname"><a href="a00163.php#gae54d1782ca9b54bea915f5c18a9158fa">hwloc_topology_get_depth</a></div><div class="ttdeci">int hwloc_topology_get_depth(hwloc_topology_t restrict topology)</div><div class="ttdoc">Get the depth of the hierarchical tree of objects.</div></div>
<div class="ttc" id="aa00163_php_ggaf4e663cf42bbe20756b849c6293ef575a0565ab92ab72cb0cec91e23003294aad"><div class="ttname"><a href="a00163.php#ggaf4e663cf42bbe20756b849c6293ef575a0565ab92ab72cb0cec91e23003294aad">HWLOC_TYPE_DEPTH_UNKNOWN</a></div><div class="ttdeci">@ HWLOC_TYPE_DEPTH_UNKNOWN</div><div class="ttdoc">No object of given type exists in the topology.</div><div class="ttdef"><b>Definition:</b> hwloc.h:822</div></div>
<div class="ttc" id="aa00164_php_ga870e876931c282a1c7aee2f031912ce3"><div class="ttname"><a href="a00164.php#ga870e876931c282a1c7aee2f031912ce3">hwloc_obj_attr_snprintf</a></div><div class="ttdeci">int hwloc_obj_attr_snprintf(char *restrict string, size_t size, hwloc_obj_t obj, const char *restrict separator, int verbose)</div><div class="ttdoc">Stringify the attributes of a given topology object into a human-readable form.</div></div>
<div class="ttc" id="aa00164_php_gadb8765c260edea80c52cd06a76639ba4"><div class="ttname"><a href="a00164.php#gadb8765c260edea80c52cd06a76639ba4">hwloc_obj_type_snprintf</a></div><div class="ttdeci">int hwloc_obj_type_snprintf(char *restrict string, size_t size, hwloc_obj_t obj, int verbose)</div><div class="ttdoc">Stringify the type of a given topology object into a human-readable form.</div></div>
<div class="ttc" id="aa00166_php_ga80bc07473a8edf840cae17bd7ec21d48"><div class="ttname"><a href="a00166.php#ga80bc07473a8edf840cae17bd7ec21d48">hwloc_set_cpubind</a></div><div class="ttdeci">int hwloc_set_cpubind(hwloc_topology_t topology, hwloc_const_cpuset_t set, int flags)</div><div class="ttdoc">Bind current process or thread on cpus given in physical bitmap set.</div></div>
<div class="ttc" id="aa00167_php_ga04736461780fadcf193af218c0122273"><div class="ttname"><a href="a00167.php#ga04736461780fadcf193af218c0122273">hwloc_alloc_membind</a></div><div class="ttdeci">void * hwloc_alloc_membind(hwloc_topology_t topology, size_t len, hwloc_const_bitmap_t set, hwloc_membind_policy_t policy, int flags)</div><div class="ttdoc">Allocate some memory on NUMA memory nodes specified by set.</div></div>
<div class="ttc" id="aa00167_php_ga32dbd4f54e9e4a7179f2dde37ffe6ad7"><div class="ttname"><a href="a00167.php#ga32dbd4f54e9e4a7179f2dde37ffe6ad7">hwloc_free</a></div><div class="ttdeci">int hwloc_free(hwloc_topology_t topology, void *addr, size_t len)</div><div class="ttdoc">Free memory that was previously allocated by hwloc_alloc() or hwloc_alloc_membind().</div></div>
<div class="ttc" id="aa00167_php_gaf881faefe20701229f07dd7dbd0125ed"><div class="ttname"><a href="a00167.php#gaf881faefe20701229f07dd7dbd0125ed">hwloc_set_area_membind</a></div><div class="ttdeci">int hwloc_set_area_membind(hwloc_topology_t topology, const void *addr, size_t len, hwloc_const_bitmap_t set, hwloc_membind_policy_t policy, int flags)</div><div class="ttdoc">Bind the already-allocated memory identified by (addr, len) to the NUMA node(s) specified by set.</div></div>
<div class="ttc" id="aa00167_php_ggab00475fd98815bf4fb9aaf752030e7d2a71f19fe4505f1c083dc8e6f7bdea6256"><div class="ttname"><a href="a00167.php#ggab00475fd98815bf4fb9aaf752030e7d2a71f19fe4505f1c083dc8e6f7bdea6256">HWLOC_MEMBIND_BYNODESET</a></div><div class="ttdeci">@ HWLOC_MEMBIND_BYNODESET</div><div class="ttdoc">Consider the bitmap argument as a nodeset.</div><div class="ttdef"><b>Definition:</b> hwloc.h:1509</div></div>
<div class="ttc" id="aa00167_php_ggac9764f79505775d06407b40f5e4661e8ad811fa4b2a6002c4d63695a408ffde2c"><div class="ttname"><a href="a00167.php#ggac9764f79505775d06407b40f5e4661e8ad811fa4b2a6002c4d63695a408ffde2c">HWLOC_MEMBIND_BIND</a></div><div class="ttdeci">@ HWLOC_MEMBIND_BIND</div><div class="ttdoc">Allocate memory on the specified nodes.</div><div class="ttdef"><b>Definition:</b> hwloc.h:1421</div></div>
<div class="ttc" id="aa00174_php_ga2ed589bea28711e80b92066510a5607d"><div class="ttname"><a href="a00174.php#ga2ed589bea28711e80b92066510a5607d">hwloc_obj_type_is_cache</a></div><div class="ttdeci">int hwloc_obj_type_is_cache(hwloc_obj_type_t type)</div><div class="ttdoc">Check whether an object type is a CPU Cache (Data, Unified or Instruction).</div></div>
<div class="ttc" id="aa00181_php_ga0fece972134fdecf2da9bc7a11dd827e"><div class="ttname"><a href="a00181.php#ga0fece972134fdecf2da9bc7a11dd827e">hwloc_bitmap_asprintf</a></div><div class="ttdeci">int hwloc_bitmap_asprintf(char **strp, hwloc_const_bitmap_t bitmap)</div><div class="ttdoc">Stringify a bitmap into a newly allocated string.</div></div>
<div class="ttc" id="aa00181_php_ga156130d85b3a0674d6e0e6770fe68fbe"><div class="ttname"><a href="a00181.php#ga156130d85b3a0674d6e0e6770fe68fbe">hwloc_bitmap_free</a></div><div class="ttdeci">void hwloc_bitmap_free(hwloc_bitmap_t bitmap)</div><div class="ttdoc">Free bitmap bitmap.</div></div>
<div class="ttc" id="aa00181_php_gaa611a77c092e679246afdf9a60d5db8b"><div class="ttname"><a href="a00181.php#gaa611a77c092e679246afdf9a60d5db8b">hwloc_bitmap_singlify</a></div><div class="ttdeci">int hwloc_bitmap_singlify(hwloc_bitmap_t bitmap)</div><div class="ttdoc">Keep a single index among those set in bitmap bitmap.</div></div>
<div class="ttc" id="aa00181_php_gae679434c1a5f41d3560a8a7e2c1b0dee"><div class="ttname"><a href="a00181.php#gae679434c1a5f41d3560a8a7e2c1b0dee">hwloc_bitmap_dup</a></div><div class="ttdeci">hwloc_bitmap_t hwloc_bitmap_dup(hwloc_const_bitmap_t bitmap)</div><div class="ttdoc">Duplicate bitmap bitmap by allocating a new bitmap and copying bitmap contents.</div></div>
...
<div class="ttc" id="aa00217_php_a04d05403da37bfe17cd63b7c7dd07b1f"><div class="ttname"><a href="a00217.php#a04d05403da37bfe17cd63b7c7dd07b1f">hwloc_obj::children</a></div><div class="ttdeci">struct hwloc_obj ** children</div><div class="ttdoc">Normal children, children[0 .. arity -1].</div><div class="ttdef"><b>Definition:</b> hwloc.h:456</div></div>
<div class="ttc" id="aa00217_php_a08f0d0e16c619a6e653526cbee4ffea3"><div class="ttname"><a href="a00217.php#a08f0d0e16c619a6e653526cbee4ffea3">hwloc_obj::nodeset</a></div><div class="ttdeci">hwloc_nodeset_t nodeset</div><div class="ttdoc">NUMA nodes covered by this object or containing this object.</div><div class="ttdef"><b>Definition:</b> hwloc.h:540</div></div>
...
<div class="ttc" id="aa00217_php_aac3f6da35c9b57599909a44ce2b716c1"><div class="ttname"><a href="a00217.php#aac3f6da35c9b57599909a44ce2b716c1">hwloc_obj::arity</a></div><div class="ttdeci">unsigned arity</div><div class="ttdoc">Number of normal children. Memory, Misc and I/O children are not listed here but rather in their dedi...</div><div class="ttdef"><b>Definition:</b> hwloc.h:452</div></div>
<div class="ttc" id="aa00217_php_acc4f0803f244867e68fe0036800be5de"><div class="ttname"><a href="a00217.php#acc4f0803f244867e68fe0036800be5de">hwloc_obj::type</a></div><div class="ttdeci">hwloc_obj_type_t type</div><div class="ttdoc">Type of object.</div><div class="ttdef"><b>Definition:</b> hwloc.h:398</div></div>
...
<div class="ttc" id="aa00217_php_adc494f6aed939992be1c55cca5822900"><div class="ttname"><a href="a00217.php#adc494f6aed939992be1c55cca5822900">hwloc_obj::parent</a></div><div class="ttdeci">struct hwloc_obj * parent</div><div class="ttdoc">Parent, NULL if root (Machine object)</div><div class="ttdef"><b>Definition:</b> hwloc.h:446</div></div>
<div class="ttc" id="aa00221_php_a23f553f3252c9d13f2338231cd354ea9"><div class="ttname"><a href="a00221.php#a23f553f3252c9d13f2338231cd354ea9">hwloc_obj_attr_u::cache</a></div><div class="ttdeci">struct hwloc_obj_attr_u::hwloc_cache_attr_s cache</div></div>
<div class="ttc" id="aa00233_php_abe5e788943ed04302976740c829674c0"><div class="ttname"><a href="a00233.php#abe5e788943ed04302976740c829674c0">hwloc_obj_attr_u::hwloc_cache_attr_s::size</a></div><div class="ttdeci">hwloc_uint64_t size</div><div class="ttdoc">Size of cache in bytes.</div><div class="ttdef"><b>Definition:</b> hwloc.h:617</div></div>
...
<ul>
<li>
<a class="el" href="a00359.php">Terms and Definitions</a> </li>
<li>
<a class="el" href="a00360.php">Command-Line Tools</a> </li>
<li>
<a class="el" href="a00361.php">Environment Variables</a> </li>
<li>
<a class="el" href="a00362.php">CPU and Memory Binding Overview</a> </li>
<li>
<a class="el" href="a00363.php">I/O Devices</a> </li>
<li>
<a class="el" href="a00364.php">Miscellaneous objects</a> </li>
<li>
<a class="el" href="a00365.php">Object attributes</a> </li>
<li>
<a class="el" href="a00366.php">Topology Attributes: Distances, Memory Attributes and CPU Kinds</a> </li>
<li>
<a class="el" href="a00367.php">Importing and exporting topologies from/to XML files</a> </li>
<li>
<a class="el" href="a00368.php">Synthetic topologies</a> </li>
<li>
<a class="el" href="a00369.php">Interoperability With Other Software</a> </li>
<li>
<a class="el" href="a00370.php">Thread Safety</a> </li>
<li>
<a class="el" href="a00371.php">Components and plugins</a> </li>
<li>
<a class="el" href="a00372.php">Embedding hwloc in Other Software</a> </li>
<li>
<a class="el" href="a00373.php">Frequently Asked Questions (FAQ)</a> </li>
<li>
<a class="el" href="a00374.php">Upgrading to the hwloc 2.0 API</a> </li>
</ul>
