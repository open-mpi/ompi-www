...
<dd><p class="startdd">enforces the return value of <a class="el" href="a00050.php#ga0d109e33fc7990f62f665d336e5e5111" title="Does the topology context come from this system?">hwloc_topology_is_thissystem()</a>, as if HWLOC_TOPOLOGY_FLAG_IS_THISSYSTEM was set with <a class="el" href="a00047.php#ga6d11e53db143ac39c32cdb3912b71f99" title="Set OR&#39;ed flags to non-yet-loaded topology.">hwloc_topology_set_flags()</a>. It means that it makes hwloc assume that the selected backend provides the topology for the system on which we are running, even if it is not the OS-specific backend but the XML backend for instance. This means making the binding functions actually call the OS-specific system calls and really do binding, while the XML backend would otherwise provide empty hooks just returning success. This can be used for efficiency reasons to first detect the topology once, save it to an XML file, and quickly reload it later through the XML backend, but still having binding functions actually do bind. </p>
<p class="enddd"></p>
</dd>
...
<dd><p class="startdd">display verbose information about plugins. List which directories are scanned, which files are loaded, and which components are successfully loaded. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_COMPONENTS=list,of,components </dt>
<dd><p class="startdd">forces a list of components to enable. Enable the given comma-separated list of components (if they do not conflict with each other). Once the end of the list is reached, hwloc falls back to enabling the remaining components (sorted by priority) that do not conflict with the already enabled ones. If <code>stop</code> is met, the enabling loop immediately stops, no more component is enabled. If the variable is set to an empty string, no specific component is loaded first, all components are loaded in priority order, this is strictly identical to not specifying any variable. The <code>xml</code> component name may be followed by a XML file to load (<code>xml=file.xml</code>). The synthetic component may be followed by a synthetic topology description (<code>synthetic=node:2 pu:3</code>). See <a class="el" href="a00012.php#plugins">Components and plugins</a> for details. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_COMPONENTS_VERBOSE=1 </dt>
<dd><p class="startdd">display verbose information about components. Display messages when components are registered or enabled. This is the recommended way to list the available components with their priority (all of them are <em>registered</em> at startup). </p>
<p class="enddd"></p>
</dd>
</dl>
</div></div><!-- contents -->
