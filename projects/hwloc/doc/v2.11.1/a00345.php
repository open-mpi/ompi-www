...
<dd><p class="startdd">enforces the discovery from the given XML file as if <a class="el" href="a00148.php#ga879439b7ee99407ee911b3ac64e9a25e" title="Enable XML-file based topology.">hwloc_topology_set_xml()</a> had been called. This file may have been generated earlier with lstopo file.xml. For convenience, this backend provides empty binding hooks which just return success. To have hwloc still actually call OS-specific hooks, HWLOC_THISSYSTEM should be set 1 in the environment too, to assert that the loaded file is really the underlying system. See also <a class="el" href="a00352.php">Importing and exporting topologies from/to XML files</a>. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_SYNTHETIC=synthetic_description </dt>
<dd><p class="startdd">enforces the discovery through a synthetic description string as if <a class="el" href="a00148.php#ga4fab186bb6181a00bcf585825fddd38d" title="Enable synthetic topology.">hwloc_topology_set_synthetic()</a> had been called. For convenience, this backend provides empty binding hooks which just return success. See also <a class="el" href="a00353.php">Synthetic topologies</a>. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_XML_VERBOSE=1 </dt>
<dd></dd>
<dt>HWLOC_SYNTHETIC_VERBOSE=1 </dt>
<dd><p class="startdd">enables verbose messages in the XML or synthetic topology backends. hwloc XML backends (see <a class="el" href="a00352.php">Importing and exporting topologies from/to XML files</a>) can emit some error messages to the error output stream. Enabling these verbose messages within hwloc can be useful for understanding failures to parse input XML topologies. Similarly, enabling verbose messages in the synthetic topology backend can help understand why the description string is invalid. See also <a class="el" href="a00353.php">Synthetic topologies</a>. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_THISSYSTEM=1 </dt>
<dd><p class="startdd">enforces the return value of <a class="el" href="a00149.php#ga68ffdcfd9175cdf40709801092f18017" title="Does the topology context come from this system?">hwloc_topology_is_thissystem()</a>, as if <a class="el" href="a00149.php#ggada025d3ec20b4b420f8038d23d6e7bdea6ecb6abc6a0bb75e81564f8bca85783b" title="Assume that the selected backend provides the topology for the system on which we are running.">HWLOC_TOPOLOGY_FLAG_IS_THISSYSTEM</a> was set with <a class="el" href="a00149.php#gaaeed4df656979e5f16befea9d29b814b" title="Set OR&#39;ed flags to non-yet-loaded topology.">hwloc_topology_set_flags()</a>. It means that it makes hwloc assume that the selected backend provides the topology for the system on which we are running, even if it is not the OS-specific backend but the XML backend for instance. This means making the binding functions actually call the OS-specific system calls and really do binding, while the XML backend would otherwise provide empty hooks just returning success. This can be used for efficiency reasons to first detect the topology once, save it to a XML file, and quickly reload it later through the XML backend, but still having binding functions actually do bind. This also enables support for the variable HWLOC_THISSYSTEM_ALLOWED_RESOURCES. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_THISSYSTEM_ALLOWED_RESOURCES=1 </dt>
<dd><p class="startdd">Get the set of allowed resources from the native operating system even if the topology was loaded from XML or synthetic description, as if <a class="el" href="a00149.php#ggada025d3ec20b4b420f8038d23d6e7bdea1b66bbd66e900e5c837f71defb32ad89" title="Get the set of allowed resources from the local operating system even if the topology was loaded from...">HWLOC_TOPOLOGY_FLAG_THISSYSTEM_ALLOWED_RESOURCES</a> was set with <a class="el" href="a00149.php#gaaeed4df656979e5f16befea9d29b814b" title="Set OR&#39;ed flags to non-yet-loaded topology.">hwloc_topology_set_flags()</a>. This variable requires the topology to match the current system (see the variable HWLOC_THISSYSTEM). This is useful when the topology is not loaded directly from the local machine (e.g. for performance reason) and it comes with all resources, but the running process is restricted to only a part of the machine (for instance because of Linux Cgroup/Cpuset). </p>
<p class="enddd"></p>
</dd>
...
<dd><p class="startdd">Disable or enable all heuristics to guess memory subtypes and tiers. By default, hwloc only uses heuristics that are likely correct and disables those that are unlikely. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_MEMTIERS=0x0f=HBM;0xf=DRAM </dt>
<dd><p class="startdd">Enforce the memory tiers from the given semi-colon separated list. Each entry specifies a bitmask (nodeset) of NUMA nodes and their subtype. Nodes not listed in any entry are not placed in any tier.</p>
<p class="interdd">If an empty value or <code>none</code> is given, tiers are entirely disabled. </p>
<p class="enddd"></p>
</dd>
<dt>HWLOC_MEMTIERS_REFRESH=1 </dt>
<dd><p class="startdd">Force the rebuilding of memory tiers. This is mostly useful when importing a XML topology from an old hwloc version which was not able to guess memory subtypes and tiers. </p>
<p class="enddd"></p>
</dd>
...
<dd><p class="startdd">uniformize max frequency, base frequency and Linux capacity to force a single homogeneous kind of CPUs. This is enabled by default on NVIDIA Grace but may be disabled if set to 0 (or enabled on other platforms if set to 1). </p>
<p class="enddd"></p>
</dd>
...
<dd><p class="startdd">show or hide NUMA nodes that correspond to NVIDIA GPU memory. By default they are ignored on POWER platforms to avoid interleaved memory being allocated on GPU by mistake.</p>
<p class="interdd">Setting this environment variable to 0 hides the NUMA nodes (default on POWER). Setting to 1 exposes these NUMA nodes (default on non-POWER platforms such as NVIDIA Grace Hopper).</p>
<p class="interdd">These NUMA nodes may be recognized by the <em>GPUMemory</em> subtype. They also have a <em>PCIBusID</em> info attribute to identify the corresponding GPU. </p>
<p class="enddd"></p>
</dd>
...
<p class="interdd">This variable does not impact the querying of Windows processor groups using the dedicated API in <a class="el" href="a00098_source.php">hwloc/windows.h</a>, this feature is always supported. </p>
<p class="enddd"></p>
</dd>
...
<p class="interdd">See <a class="el" href="a00356.php#plugins_select">Selecting which components to use</a> for details. </p>
<p class="enddd"></p>
</dd>
