...
shell$ make
shell$ make install
</pre></div><p>The hwloc command-line tool "lstopo" produces human-readable topology maps, as mentioned above. It can also export maps to the "fig" file format. Support for PDF, Postscript, and PNG exporting is provided if the "Cairo" development package can be found when hwloc is configured and build.</p>
...
<li>
libxml2 for full XML import/export support (otherwise, the internal minimalistic parser will only be able to import XML files that were exported by the same hwloc release). See <a class="el" href="xml.php">Importing and exporting topologies from/to XML files</a> for details. </li>
</ul>
 
</div><div class="section" id="cli_examples">
 <h2><a class="anchor" id="cli_examples"></a>
CLI Examples</h2>
...
Machine (16GB)
  ...
  Socket L#3 + L3 L#3 (4096KB)
    ...
    L2 L#7 (1024KB) + L1 L#7 (16KB) + Core L#7
      PU L#14 (P#7)
      PU L#15 (P#15)
</pre></div><p>Finally, here's the equivalent output in XML. Long lines were artificially broken for document clarity (in the real output, each XML tag is on a single line), and only socket #0 is shown for brevity:</p>
<div class="fragment"><pre class="fragment">
...
&lt;topology&gt;
  ...
  &lt;/object&gt;
&lt;/topology&gt;
</pre></div><p>On a 4-socket 2-core Opteron NUMA machine, the <code>lstopo</code> tool may show the following graphical output:</p>
<div class="image">
...
</pre></div><p>And here's the equivalent output in XML. Similar to above, line breaks were added and only PU #0 is shown for brevity:</p>
<div class="fragment"><pre class="fragment">
&lt;?xml version="1.0" encoding="UTF-8"?&gt;
&lt;!DOCTYPE topology SYSTEM "hwloc.dtd"&gt;
&lt;topology&gt;
...
</pre></div><p>And the same output in XML (line breaks added, only PU #0 shown):</p>
<div class="fragment"><pre class="fragment">
&lt;?xml version="1.0" encoding="UTF-8"?&gt;
&lt;!DOCTYPE topology SYSTEM "hwloc.dtd"&gt;
&lt;topology&gt;
...
<p>The basic interface is available in <a class="el" href="hwloc_8h.php" title="The hwloc API.">hwloc.h</a>. It essentially offers low-level routines for advanced programmers that want to manually manipulate objects and follow links between them. Documentation for everything in <a class="el" href="hwloc_8h.php" title="The hwloc API.">hwloc.h</a> are provided later in this document. Developers should also look at <a class="el" href="helper_8h.php" title="High-level hwloc traversal helpers.">hwloc/helper.h</a> (and also in this document, which provides good higher-level topology traversal examples).</p>
<p>To precisely define the vocabulary used by hwloc, a <a class="el" href="termsanddefs.php">Terms and Definitions</a> section is available and should probably be read first.</p>
<p>Each hwloc object contains a cpuset describing the list of processing units that it contains. These bitmaps may be used for <a class="el" href="group__hwlocality__cpubinding.php">CPU binding</a> and <a class="el" href="group__hwlocality__membinding.php">Memory binding</a>. hwloc offers an extensive bitmap manipulation interface in <a class="el" href="bitmap_8h.php" title="The bitmap API, for use in hwloc itself.">hwloc/bitmap.h</a>.</p>
<p>Moreover, hwloc also comes with additional helpers for interoperability with several commonly used environments. See the <a class="el" href="interoperability.php">Interoperability With Other Software</a> section for details.</p>
...
<p><b>NOTE:</b> If you are building the documentation from a Subversion checkout, you will need to have Doxygen and pdflatex installed -- the documentation will be built during the normal "make" process. The documentation is installed during "make install" to $prefix/share/doc/hwloc/ and your systems default man page tree (under $prefix, of course).</p>
<h3><a class="anchor" id="portability"></a>
Portability</h3>
...
<img src="ppc64-without-smt.png" alt="ppc64-without-smt.png"/>
 ...
 <p>Notice that hwloc only sees half the PUs when SMT is disabled. PU #15, for example, seems to change location from NUMA node #0 to #1. In reality, no PUs "moved" -- they were simply re-numbered when hwloc only saw half as many. Hence, PU #15 in the SMT-disabled picture probably corresponds to PU #30 in the SMT-enabled picture.</p>
...
<h3><a class="anchor" id="interface_example"></a>
API Example</h3>
...
<span class="comment"> * Copyright © 2009-2010 INRIA.  All rights reserved.</span>
...
<span class="comment"> * See COPYING in top-level directory.</span>
<span class="comment"> *</span>
...
{
    <span class="keywordtype">int</span> depth;
    <span class="keywordtype">unsigned</span> i, n;
    ...
    <a class="code" href="group__hwlocality__sets.php#ga4bbf39b68b6f568fb92739e7c0ea7801" title="A CPU set is a bitmap whose bits are set according to CPU physical OS indexes.">hwloc_cpuset_t</a> cpuset;
    ...
    <a class="code" href="group__hwlocality__creation.php#ga5c2d6f476af87005c7bd0811d4548b9f" title="Allocate a topology context.">hwloc_topology_init</a>(&amp;topology);
...
<span class="comment">       Detection. */</span>
    ...
    <a class="code" href="group__hwlocality__creation.php#ga91e2e6427b95fb7339c99dbbef996e71" title="Build the actual topology.">hwloc_topology_load</a>(topology);
...
<span class="comment">       in case we need the topology depth later. */</span>
    topodepth = <a class="code" href="group__hwlocality__information.php#ga8c30b0cec55074eb3ed34e4f2a1a9937" title="Get the depth of the hierarchical tree of objects.">hwloc_topology_get_depth</a>(topology);
...
<span class="comment">     *****************************************************************/</span>
    <span class="keywordflow">for</span> (depth = 0; depth &lt; topodepth; depth++) {
        ...
        <span class="keywordflow">for</span> (i = 0; i &lt; <a class="code" href="group__hwlocality__information.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth); 
             i++) {
            <a class="code" href="group__hwlocality__conversion.php#ga5c6a61a83f4790b421e2f62e9088446f" title="Stringify a given topology object into a human-readable form.">hwloc_obj_snprintf</a>(<span class="keywordtype">string</span>, <span class="keyword">sizeof</span>(<span class="keywordtype">string</span>), topology,
                       <a class="code" href="group__hwlocality__traversal.php#gaedd78240b0c1108355586a268ec5a697" title="Returns the topology object at logical index idx from depth depth.">hwloc_get_obj_by_depth</a>(topology, depth, i),
...
<span class="comment">     *****************************************************************/</span>
    ...
    print_children(topology, <a class="code" href="group__hwlocality__helper__traversal__basic.php#ga632edae4a651996895ebde85ea2c1264" title="Returns the top-object of the topology-tree.">hwloc_get_root_obj</a>(topology), 0);

    <span class="comment">/*****************************************************************</span>
<span class="comment">     * Third example:</span>
<span class="comment">     * Print the number of sockets.</span>
<span class="comment">     *****************************************************************/</span>
    depth = <a class="code" href="group__hwlocality__information.php#gaea7c64dd59467f5201ba87712710b14d" title="Returns the depth of objects of type type.">hwloc_get_type_depth</a>(topology, <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55a1ac6e07775ae4324b3fe9dbd72c785ec" title="Socket, physical package, or chip. In the physical meaning, i.e. that you can add or remove physicall...">HWLOC_OBJ_SOCKET</a>);
    <span class="keywordflow">if</span> (depth == <a class="code" href="group__hwlocality__information.php#ggaf4e663cf42bbe20756b849c6293ef575a0565ab92ab72cb0cec91e23003294aad" title="No object of given type exists in the topology.">HWLOC_TYPE_DEPTH_UNKNOWN</a>) {
        printf(<span class="stringliteral">&quot;*** The number of sockets is unknown\n&quot;</span>);
    } <span class="keywordflow">else</span> {
        printf(<span class="stringliteral">&quot;*** %u socket(s)\n&quot;</span>, 
               <a class="code" href="group__hwlocality__information.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth));
    }
...
<span class="comment">     *****************************************************************/</span>
    levels = 0;
    size = 0;
    <span class="keywordflow">for</span> (obj = <a class="code" href="group__hwlocality__traversal.php#ga701f83b2cf0cb8e0acd58cd2dc1c67a2" title="Returns the topology object at logical index idx with type type.">hwloc_get_obj_by_type</a>(topology, <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55abca6887e80cb291353b0a0c1da83f661" title="Processing Unit, or (Logical) Processor. An execution unit (may share a core with some other logical ...">HWLOC_OBJ_PU</a>, 0);
         obj;
         obj = obj-&gt;<a class="code" href="structhwloc__obj.php#adc494f6aed939992be1c55cca5822900" title="Parent, NULL if root (system object)">parent</a>)
      <span class="keywordflow">if</span> (obj-&gt;<a class="code" href="structhwloc__obj.php#acc4f0803f244867e68fe0036800be5de" title="Type of object.">type</a> == <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55a56ee0b7eca88f363b75b34fdde8c9ddc" title="Data cache. Can be L1, L2, L3, ...">HWLOC_OBJ_CACHE</a>) {
        levels++;
        size += obj-&gt;<a class="code" href="structhwloc__obj.php#accd40e29f71f19e88db62ea3df02adc8" title="Object type-specific Attributes, may be NULL if no attribute value was found.">attr</a>-&gt;<a class="code" href="unionhwloc__obj__attr__u.php#ab5a8ae3bf490e6b1071fea53f7382836">cache</a>.<a class="code" href="structhwloc__obj__attr__u_1_1hwloc__cache__attr__s.php#abe5e788943ed04302976740c829674c0" title="Size of cache in bytes.">size</a>;
      }
...
<span class="comment">     *****************************************************************/</span>
    depth = <a class="code" href="group__hwlocality__helper__types.php#ga0ffafb4c0ae13b9a7541ca820ca34883" title="Returns the depth of objects of type type or below.">hwloc_get_type_or_below_depth</a>(topology, <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55ac793958f330bca371aa1535de8aff45f" title="Core. A computation unit (may be shared by several logical processors).">HWLOC_OBJ_CORE</a>);

    <span class="comment">/* Get last core. */</span>
    obj = <a class="code" href="group__hwlocality__traversal.php#gaedd78240b0c1108355586a268ec5a697" title="Returns the topology object at logical index idx from depth depth.">hwloc_get_obj_by_depth</a>(topology, depth,
                   <a class="code" href="group__hwlocality__information.php#ga20cfe2456f4cfdd789c9aca6d2fdd69f" title="Returns the width of level at depth depth.">hwloc_get_nbobjs_by_depth</a>(topology, depth) - 1);
    <span class="keywordflow">if</span> (obj) {
        <span class="comment">/* Get a copy of its cpuset that we may modify. */</span>
        cpuset = <a class="code" href="group__hwlocality__bitmap.php#gaaa4ed76211cd3694dfbea2109fc440be" title="Duplicate bitmap bitmap by allocating a new bitmap and copying bitmap contents.">hwloc_bitmap_dup</a>(obj-&gt;<a class="code" href="structhwloc__obj.php#a67925e0f2c47f50408fbdb9bddd0790f" title="CPUs covered by this object.">cpuset</a>);

        <span class="comment">/* Get only one logical processor (in case the core is</span>
<span class="comment">           SMT/hyperthreaded). */</span>
        <a class="code" href="group__hwlocality__bitmap.php#ga4630aa1b7e08eac5b41be126194e84a1" title="Keep a single index among those set in bitmap bitmap.">hwloc_bitmap_singlify</a>(cpuset);

        <span class="comment">/* And try to bind ourself there. */</span>
        <span class="keywordflow">if</span> (<a class="code" href="group__hwlocality__cpubinding.php#gaf4cc194d5c0d38004a21b9f03522a7e3" title="Bind current process or thread on cpus given in bitmap set.">hwloc_set_cpubind</a>(topology, cpuset, 0)) {
            <span class="keywordtype">char</span> *str;
            <span class="keywordtype">int</span> error = errno;
            <a class="code" href="group__hwlocality__bitmap.php#gad3cf87ceb58aa91656756bbb58057320" title="Stringify a bitmap into a newly allocated string.">hwloc_bitmap_asprintf</a>(&amp;str, obj-&gt;<a class="code" href="structhwloc__obj.php#a67925e0f2c47f50408fbdb9bddd0790f" title="CPUs covered by this object.">cpuset</a>);
            printf(<span class="stringliteral">&quot;Couldn&#39;t bind to cpuset %s: %s\n&quot;</span>, str, strerror(error));
            free(str);
        }

        <span class="comment">/* Free our cpuset copy */</span>
        <a class="code" href="group__hwlocality__bitmap.php#ga8e7035fe555ef96921bfb98e08519bc7" title="Free bitmap bitmap.">hwloc_bitmap_free</a>(cpuset);
    }

    <span class="comment">/*****************************************************************</span>
<span class="comment">     * Sixth example:</span>
<span class="comment">     * Allocate some memory on the last NUMA node, bind some existing</span>
<span class="comment">     * memory to the last NUMA node.</span>
<span class="comment">     *****************************************************************/</span>
    <span class="comment">/* Get last node. */</span>
    n = <a class="code" href="group__hwlocality__information.php#ga0131ab1051011fabfa69d7c1853e716c" title="Returns the width of level type type.">hwloc_get_nbobjs_by_type</a>(topology, <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55aaf0964881117bdedf1a5e9332cd120dd" title="NUMA node. A set of processors around memory which the processors can directly access.">HWLOC_OBJ_NODE</a>);
    <span class="keywordflow">if</span> (n) {
        <span class="keywordtype">void</span> *m;
        size = 1024*1024;

        obj = <a class="code" href="group__hwlocality__traversal.php#ga701f83b2cf0cb8e0acd58cd2dc1c67a2" title="Returns the topology object at logical index idx with type type.">hwloc_get_obj_by_type</a>(topology, <a class="code" href="group__hwlocality__types.php#ggacd37bb612667dc437d66bfb175a8dc55aaf0964881117bdedf1a5e9332cd120dd" title="NUMA node. A set of processors around memory which the processors can directly access.">HWLOC_OBJ_NODE</a>, n - 1);
        m = <a class="code" href="group__hwlocality__membinding.php#gaeaa00714a9c4319bda0a74ca6f8720e8" title="Allocate some memory on the given nodeset nodeset.">hwloc_alloc_membind_nodeset</a>(topology, size, obj-&gt;<a class="code" href="structhwloc__obj.php#a08f0d0e16c619a6e653526cbee4ffea3" title="NUMA nodes covered by this object or containing this object.">nodeset</a>,
                <a class="code" href="group__hwlocality__membinding.php#ggac9764f79505775d06407b40f5e4661e8a18675bb80ebc1bce5b652e9de8f3998c" title="Reset the memory allocation policy to the system default.">HWLOC_MEMBIND_DEFAULT</a>, 0);
        <a class="code" href="group__hwlocality__membinding.php#ga986d9b4cc76da592c4b937c6cb7d9d56" title="Free memory that was previously allocated by hwloc_alloc() or hwloc_alloc_membind().">hwloc_free</a>(topology, m, size);

        m = malloc(size);
        <a class="code" href="group__hwlocality__membinding.php#gade5e2c28ea8475a479bf2b1df36c6ccd" title="Bind the already-allocated memory identified by (addr, len) to the NUMA node(s) in nodeset...">hwloc_set_area_membind_nodeset</a>(topology, m, size, obj-&gt;<a class="code" href="structhwloc__obj.php#a08f0d0e16c619a6e653526cbee4ffea3" title="NUMA nodes covered by this object or containing this object.">nodeset</a>,
                <a class="code" href="group__hwlocality__membinding.php#ggac9764f79505775d06407b40f5e4661e8a18675bb80ebc1bce5b652e9de8f3998c" title="Reset the memory allocation policy to the system default.">HWLOC_MEMBIND_DEFAULT</a>, 0);
        free(m);
    }

    <span class="comment">/* Destroy topology object. */</span>
    <a class="code" href="group__hwlocality__creation.php#ga6040925d3ee4bbb2647f2a321aca5f4b" title="Terminate and free a topology context.">hwloc_topology_destroy</a>(topology);
...
<p>libtopology was initially developed by the INRIA Runtime Team-Project (<a href="http://runtime.bordeaux.inria.fr/">http://runtime.bordeaux.inria.fr/</a>) (headed by Raymond Namyst (<a href="http://dept-info.labri.fr/~namyst/">http://dept-info.labri.fr/~namyst/</a>). PLPA was initially developed by the Open MPI development team as a sub-project. Both are now deprecated in favor of hwloc, which is distributed as an Open MPI sub-project.</p>
 ...
 <h2><a class="anchor" id="further_read"></a>
Further Reading</h2>
<p>The documentation chapters include</p>
<ul>
<li>
<a class="el" href="termsanddefs.php">Terms and Definitions</a> </li>
<li>
<a class="el" href="tools.php">Command-Line Tools</a> </li>
<li>
<a class="el" href="envvar.php">Environment Variables</a> </li>
<li>
<a class="el" href="cpu_mem_bind.php">CPU and Memory Binding Overview</a> </li>
<li>
<a class="el" href="iodevices.php">I/O Devices</a> </li>
<li>
<a class="el" href="xml.php">Importing and exporting topologies from/to XML files</a> </li>
<li>
<a class="el" href="interoperability.php">Interoperability With Other Software</a> </li>
<li>
<a class="el" href="threadsafety.php">Thread Safety</a> </li>
<li>
<a class="el" href="embed.php">Embedding hwloc in Other Software</a> </li>
<li>
<a class="el" href="faq.php">Frequently Asked Questions</a> </li>
</ul>
