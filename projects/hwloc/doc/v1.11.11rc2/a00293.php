...
<p>lstopo options <code>-l</code> and <code>-p</code> may be used to switch between logical indexes (prefixed with <code>L#</code>) and physical/OS indexes (<code>P#</code>). Converting one into the other may also be achieved with hwloc-calc which may manipulate either logical or physical indexes as input or output. See also <a class="el" href="a00280.php#cli_hwloc_calc">hwloc-calc</a>.</p>
...
<p>The focus of hwloc is mainly of the structural modeling side. However, hwloc lets user adds performance information to the topology through distances (see <a class="el" href="a00214.php" title="Distances between objects. ">hwloc_distances_s</a> and <a class="el" href="a00124.php#ga90ed5733946b2e9aba90d2e2c1fbfbd7" title="Provide a distance matrix. ">hwloc_topology_set_distance_matrix()</a>) or even custom annotations (see <a class="el" href="a00293.php#faq_annotate">How do I annotate the topology with private notes?</a>). hwloc may also use such distance information for grouping objects together (see <a class="el" href="a00293.php#faq_onedim">hwloc only has a one-dimensional view of the architecture, it ignores distances?</a> and <a class="el" href="a00293.php#faq_groups">What are these Group objects in my topology?</a>).</p>
...
<p>For more information about the distance API, see <a class="el" href="a00214.php" title="Distances between objects. ">hwloc_distances_s</a> and <a class="el" href="a00124.php#ga90ed5733946b2e9aba90d2e2c1fbfbd7" title="Provide a distance matrix. ">hwloc_topology_set_distance_matrix()</a>.</p>
<h2><a class="anchor" id="faq_groups"></a>
What are these Group objects in my topology?</h2>
<p>hwloc comes with a set of predefined object types (Core, Package, NUMA node, Caches) that match the vast majority of hardware platforms. The <a class="el" href="a00121.php#ggacd37bb612667dc437d66bfb175a8dc55a5269ef95be72f88465559d35c9b7ad56" title="Group objects. Objects which do not fit in the above but are detected by hwloc and are useful to take...">HWLOC_OBJ_GROUP</a> type was designed for cases where this set is not sufficient. Groups may be used anywhere to add more structure information to the topology, for instance to show that 2 out of 4 NUMA nodes are actually closer than the others. When applicable, the <code>Type</code> info attribute describes why a Group was actually added (see also <a class="el" href="a00286.php#attributes_info">Custom string infos</a>).</p>
<p>hwloc currently uses Groups for the following reasons: </p><ul>
<li>
AMD dual-core compute units (<code>Type=ComputeUnit</code>, in the x86 backend), but these objects are usually merged with the L2 caches. </li>
<li>
...
<p>In practice, asymmetric topologies mostly appear when intermediate groups are added for I/O affinity: on a 4-package machine, an I/O bus may be connected to 2 packages. These packages are below an additional Group object, while the other packages are not (see also <a class="el" href="a00293.php#faq_groups">What are these Group objects in my topology?</a>).</p>
<p>Before hwloc v2.0, <a class="el" href="a00124.php#ga1f987bca941d6949faf7b1554dd7bc12" title="Ignore an object type if it does not bring any structure. ">hwloc_topology_ignore_type_keep_structure()</a> and <a class="el" href="a00124.php#ga7c9cf147442d65d755c664ccde3bb3ef" title="Ignore all objects that do not bring any structure. ">hwloc_topology_ignore_all_keep_structure()</a> may also make topologies assymetric by removing parts of levels, especially when part of the machine is disallowed by administrator restrictions (e.g. Linux cgroups).</p>
<p>To understand how hwloc manages such cases, one should first remember the meaning of levels and cousin objects. All objects of the same type are gathered as horizontal levels with a given depth. They are also connected through the cousin pointers of the <a class="el" href="a00174.php" title="Structure of a topology object. ">hwloc_obj</a> structure. Some types, such as Caches or Groups, are annotated with a depth or level attribute (for instance L2 cache or Group1). Moreover caches have a type attribute (for instance L1i or L1d). Such attributes are also taken in account when gathering objects as horizontal levels. To be clear: there will be one level for L1i caches, another level for L1d caches, another one for L2, etc.</p>
...
<p>See the diagram in <a class="el" href="a00279.php">Terms and Definitions</a> for a graphical representation of such topologies.</p>
...
<p>First, see <a class="el" href="a00293.php#faq_nosmt">What happens to my topology if I disable symmetric multithreading, hyper-threading, etc. in the system?</a> for more information about multithreading.</p>
...
<p>Loading a XML topology is usually much faster than querying multiple files or calling multiple functions of the operating system. It is also possible to manipulate such XML files with the C programming interface, and the import/export may also be directed to memory buffer (that may for instance be transmitted between applications through a package). See also <a class="el" href="a00287.php">Importing and exporting topologies from/to XML files</a>.</p>
<dl class="section note"><dt>Note</dt><dd>The environment variable HWLOC_THISSYSTEM_ALLOWED_RESOURCES may be used to load a XML topology that contains the entire machine and restrict it to the part that is actually available to the current process (e.g. when Linux Cgroup/Cpuset are used to restrict the set of resources). See <a class="el" href="a00281.php">Environment Variables</a>.</dd></dl>
...
<p>Sharing a single topology between threads is easy (see <a class="el" href="a00290.php">Thread Safety</a>) since the vast majority of accesses are read-only.</p>
<p>If multiple topologies of different (but similar) nodes are needed in your program, have a look at <a class="el" href="a00293.php#faq_diff">How to avoid memory waste when manipulating multiple similar topologies?</a>.</p>
...
<p><a class="el" href="a00113_source.php" title="Topology differences. ">hwloc/diff.h</a> (see also <a class="el" href="a00157.php">Topology differences</a>) offers the ability to compute topology differences, apply or unapply them, or export/import to/from XML. However this feature is limited to basic differences such as attribute changes. It does not support complex modifications such as adding or removing some objects.</p>
<h2><a class="anchor" id="faq_annotate"></a>
How do I annotate the topology with private notes?</h2>
<p>Each hwloc object contains a <code>userdata</code> field that may be used by applications to store private pointers. This field is only valid during the lifetime of these container object and topology. It becomes invalid as soon the topology is destroyed, or as soon as the object disappears, for instance when restricting the topology. The userdata field is not exported/imported to/from XML by default since hwloc does not know what it contains. This behavior may be changed by specifying application-specific callbacks with <code><a class="el" href="a00132.php#ga9d6ff0f7a8dd45be9aa8575ef31978cc" title="Set the application-specific callback for exporting object userdata. ">hwloc_topology_set_userdata_export_callback()</a></code> and <code><a class="el" href="a00132.php#ga5ac6917ea7289955fb1ffda4353af9b0" title="Set the application-specific callback for importing userdata. ">hwloc_topology_set_userdata_import_callback()</a></code>.</p>
<p>Each object may also contain some <em>info</em> attributes (key name and value) that are setup by hwloc during discovery and that may be extended by the user with <code><a class="el" href="a00127.php#ga7e90c5398a9d77df31d7d45faf0f316b" title="Add the given info name and value pair to the given object. ">hwloc_obj_add_info()</a></code> (see also <a class="el" href="a00286.php">Object attributes</a>). Contrary to the <code>userdata</code> field which is unique, multiple info attributes may exist for each object, even with the same name. These attributes are always exported to XML. However only character strings may be used as key names and values.</p>
<p>It is also possible to insert Misc objects with a custom name anywhere as a leaf of the topology (see <a class="el" href="a00284.php">Miscellaneous objects</a>). And Misc objects may have their own userdata and info attributes just like any other object.</p>
<p>The hwloc-annotate command-line tool may be used for adding Misc objects and info attributes.</p>
<p>There is also a topology-specific userdata pointer that can be used to recognize different topologies by storing a custom pointer. It may be manipulated with <code><a class="el" href="a00124.php#ga2cc7b7b155cba58dda203e54f1637b9c" title="Set the topology-specific userdata pointer. ">hwloc_topology_set_userdata()</a></code> and <code><a class="el" href="a00124.php#ga91f992f8d6c4905b2d3c4f43e509c2a3" title="Retrieve the topology-specific userdata pointer. ">hwloc_topology_get_userdata()</a></code>.</p>
<p> 
...
<p>The utility <code>hwloc-dump-hwdata</code> is also involved in gathering privileged information at boot time and making it available to non-privileged users (note that this may require a specific SELinux MLS policy module). However, it only applies to Intel Xeon Phi processors for now (see <a class="el" href="a00293.php#faq_knl_dump">Why do I need hwloc-dump-hwdata for memory on Intel Xeon Phi processor?</a>). See also <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00281.php">Environment Variables</a> for details about the location of dumped files.</p>
<h2><a class="anchor" id="faq_os_error"></a>
What should I do when hwloc reports "operating system" warnings?</h2>
<p>When the operating system reports invalid locality information (because of either software or hardware bugs), hwloc may fail to insert some objects in the topology because they cannot fit in the already built tree of resources. If so, hwloc will report a warning like the following. The object causing this error is ignored, the discovery continues but the resulting topology will miss some objects and may be asymmetric (see also <a class="el" href="a00293.php#faq_asymmetric">What happens if my topology is asymmetric?</a>).</p>
<pre class="fragment">****************************************************************************
...
...
#endif
</pre><p>To check for the API of release X.Y.Z at build time, you may compare <a class="el" href="a00119.php#ga8f4dfb8eef138af55dd1a0fa802e5476" title="Indicate at build time which hwloc API version is being used. ">HWLOC_API_VERSION</a> with <code>(X&gt;&gt;16)+(Y&gt;&gt;8)+Z</code>.</p>
...
#endif
</pre><p>You should not try to remain compatible with very old releases such as 1.1.x or earlier because <code><a class="el" href="a00119.php#ga8f4dfb8eef138af55dd1a0fa802e5476" title="Indicate at build time which hwloc API version is being used. ">HWLOC_API_VERSION</a></code> was added in 1.0.0 and <code><a class="el" href="a00119.php#ga9c0b50c98add1adf57ed1ce85bb5190d" title="Indicate at runtime which hwloc API version was used at build time. ">hwloc_get_api_version()</a></code> came only in 1.1.1. Also do not use the old cpuset API since it was deprecated and superseded by the bitmap API in 1.1, and later removed in 1.5.</p>
<p> 
...
<p>Intel Xeon Phi processors introduced a new memory architecture by possibly having two distinct local memories: some normal memory (DDR) and some high-bandwidth on-package memory (MCDRAM). Processors can be configured in various clustering modes to have up to 4 <em>Clusters</em>. Moreover, each <em>Cluster</em> (quarter, half or whole processor) of the processor may have its own local parts of the DDR and of the MCDRAM. This memory and clustering configuration may be probed by looking at MemoryMode and ClusterMode attributes, see <a class="el" href="a00286.php#attributes_info">Custom string infos</a> and doc/examples/get-knl-modes.c in the source directory.</p>
...
<p>See <code>HWLOC_DUMPED_HWDATA_DIR</code> in <a class="el" href="a00281.php">Environment Variables</a> for details about the location of dumped files.</p>
...
<p>The AIX operating system requires specific user capabilities for attaching processes to resource sets (CAP_NUMA_ATTACH). Otherwise functions such as <a class="el" href="a00128.php#ga80bc07473a8edf840cae17bd7ec21d48" title="Bind current process or thread on cpus given in physical bitmap set. ">hwloc_set_cpubind()</a> fail (return -1 with errno set to EPERM).</p>
